...
const OVER_MAX_WHITELIST = new Set([
    'src/host/main/perf/perf-hud.js',
]);
...
function layerOf(posixRepoRelativePath) {
    ...
    if (normalized.includes('/src/host/main/kernel/')) return 'kernel';
    if (normalized.includes('/src/host/main/ports/')) return 'ports';
    if (normalized.includes('/src/host/main/adapters/')) return 'adapters';
    if (normalized.includes('/src/host/main/services/')) return 'services';
    if (normalized.includes('/src/host/main/routes/')) return 'routes';
    return null;
}

function isInsideHostScope(posixRepoRelativePath) {
    return posixRepoRelativePath.startsWith('src/host/main/')
        || posixRepoRelativePath === 'src/init.js'
        || posixRepoRelativePath === 'src/host-main.js'
        || posixRepoRelativePath === 'src/host-bridge.js';
}
...
async function main() {
    ...
    for (const posixPath of hostFiles) {
        ...
        if (posixPath === 'src/host/main/routes/resource-routes.js') {
