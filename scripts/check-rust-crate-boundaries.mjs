...
const RUST_CRATES_ROOT = path.join(REPO_ROOT, 'crates');
const HOST_CRATE_ROOT = path.join(RUST_CRATES_ROOT, 'rusttavern');
const HOST_SRC_ROOT = path.join(HOST_CRATE_ROOT, 'src');
const WORKSPACE_MANIFEST = path.join(REPO_ROOT, 'Cargo.toml');
...
const MAIN_CRATE_SOURCE_RULES = [
    ...
    sourceRule('application provider auth boundary', path.join(REPO_ROOT, 'crates', 'tt-application', 'src'), [
        ['provider oauth client', /\byup_oauth2::/],
        ['concrete hyper client', /\bhyper_util::/],
        ['concrete rustls client', /\brustls::/],
        ['tls root store', /\bwebpki_roots::/],
