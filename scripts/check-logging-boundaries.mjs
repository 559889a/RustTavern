...
const SRC_ROOT = path.join(REPO_ROOT, 'crates', 'rusttavern', 'src');
const APPLICATION_SRC_ROOT = path.join(REPO_ROOT, 'crates', 'tt-application', 'src');
...
function collectLineViolations(relPath, line, lineNumber) {
    ...
    if (
        relPath.startsWith('crates/tt-application/src/')
        && stripped.includes('crate::infrastructure::logging')
    ) {
        violations.push('application -> infrastructure logging');
    }
    if (relPath.startsWith('crates/rusttavern/src/presentation/') && stripped.includes('crate::infrastructure::logging')) {
        violations.push('presentation -> infrastructure logging');
    }
    if (relPath === 'crates/rusttavern/src/presentation/commands/dev_logging_commands.rs' && stripped.includes('crate::infrastructure::')) {
        violations.push('dev_logging_commands -> infrastructure');
    }
    if (relPath === 'crates/rusttavern/src/presentation/commands/settings_commands.rs' && stripped.includes('LlmApiLogStore')) {
