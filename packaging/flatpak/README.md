...
The manifest builds RustTavern from source inside the GNOME SDK. JavaScript
...
flatpak-builder --run \
  .flatpak-build \
  packaging/flatpak/com.rusttavern.client.yml \
  rusttavern
```

Set `RUSTTAVERN_FLATPAK_USER=1` to install missing SDK dependencies into the
...
Set `RUSTTAVERN_BUILD_BRANCH` and `RUSTTAVERN_BUILD_REVISION` explicitly for
...
current with `packaging/flatpak/generate-sources.sh --check`.
