...
readonly BUILD_DIR="${RUSTTAVERN_FLATPAK_BUILD_DIR:-.flatpak-build}"
...
repo_root="$(git rev-parse --show-toplevel 2>/dev/null)" ||
    die "run this script from the RustTavern Git checkout"
cd "$repo_root"

[[ ! -e "$IDENTITY_FILE" ]] ||
    die "$IDENTITY_FILE already exists; remove it or preserve it elsewhere"

build_branch="${RUSTTAVERN_BUILD_BRANCH:-}"
build_revision="${RUSTTAVERN_BUILD_REVISION:-}"
if [[ -n "$build_branch" || -n "$build_revision" ]]; then
    [[ -n "$build_branch" && -n "$build_revision" ]] ||
        die "set both RUSTTAVERN_BUILD_BRANCH and RUSTTAVERN_BUILD_REVISION"
...
[[ "$build_branch" =~ ^[A-Za-z0-9._/-]+$ ]] ||
    die "cannot determine a valid build branch; set RUSTTAVERN_BUILD_BRANCH"
[[ "$build_revision" =~ ^[0-9A-Fa-f]{7,64}$ ]] ||
    die "cannot determine a valid Git revision; set RUSTTAVERN_BUILD_REVISION"
...
printf "RUSTTAVERN_BUILD_BRANCH='%s'\nRUSTTAVERN_BUILD_REVISION='%s'\n" \
    "$build_branch" "$build_revision" >"$IDENTITY_FILE"

installation_args=()
case "${RUSTTAVERN_FLATPAK_USER:-0}" in
    0) ;;
    1) installation_args+=(--user) ;;
    *) die "RUSTTAVERN_FLATPAK_USER must be 0 or 1" ;;
esac
...
printf '  flatpak-builder --run %q %q rusttavern\n' "$BUILD_DIR" "$MANIFEST"
