...
本节说明 RustTavern 处理需求和技术取舍的基本方法。它们适用于需求分析、设计、实现和评审，也帮助新维护者理解项目长期坚持什么。每次决定仍需结合用户需求、SillyTavern 生态、平台条件和能够验证的事实。
...
RustTavern 同时连接 SillyTavern 前端生态、浏览器客户端与原生宿主（HTTP 服务器形态）。问题显现的位置与原因所在的位置可能相距很远。因此，源码、测量和可复现实验应当优先于直觉。
...
# Contributing to RustTavern

Thank you for contributing to RustTavern.
...
RustTavern connects the SillyTavern frontend ecosystem, the browser client, and the native host (an HTTP server). A symptom may appear far from its cause. Source code, measurements, and reproducible experiments therefore carry more weight than intuition.
