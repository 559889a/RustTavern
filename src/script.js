...
import { getClientVersion as getBridgeClientVersion, getRustTavernSettings, invoke, updateRustTavernSettings } from './host-bridge.js';
import { SILLYTAVERN_COMPAT_VERSION } from './compat-version.js';
import { registerLifecycleFlushHandler } from './host/main/services/lifecycle/lifecycle-flush-service.js';
import {
    replaceMesTextHtmlWithRuntimePolicy,
    replaceTransientMesTextHtmlWithRuntimePolicy,
} from './scripts/app/message/mes-text-write.js';
import { getCodeHighlightCoordinator } from './scripts/app/perf/code-highlight-coordinator.js';
import { assertRequiredChatSurfaceParticipants } from './host/main/services/chat-surface/capability-gate.js';
import { installChatSurfaceRuntime } from './host/main/services/chat-surface/install.js';
import {
    initializeChatVirtualization,
    isChatVirtualizationEnabled,
} from './host/main/services/chat-surface/chat-virtualization-state.js';
import { isInlineDrawerContentOpen, setInlineDrawerContentOpen } from './scripts/app/perf/inline-drawer-motion.js';
import { getStreamingRenderInterval, normalizeStreamingFps, shouldCommitStreamingMessage } from './scripts/app/perf/streaming-render-policy.js';
import { CHAT_COMMIT_REASON } from './scripts/chat-payload-transport.js';
import { getActiveChatSnapshot } from './host/main/adapters/st/active-chat-ref.js';
import { extension_prompt_roles, extension_prompt_types } from './scripts/extension-prompts.js';
import { waitForTauriMainReady } from './scripts/extensions/runtime/host-ready.js';
import {
    ChatInputFocusIntent,
    focusChatInput,
    installChatInputFocusKeeper,
} from './scripts/chat-input-focus.js';
import { getActiveIosPolicyCapabilities } from './scripts/rusttavern/ios-policy.js';
import { applyIosPolicyUiProjection } from './scripts/rusttavern/ios-policy-ui.js';
import { getAgentGenerationOptions } from './scripts/rusttavern/agent/agent-generation-router.js';
import {
...
} from './scripts/rusttavern/agent/agent-run-controller.js';
import { agentErrorMessage } from './scripts/rusttavern/agent/agent-error-presenter.js';
import { normalizeAgentContextPolicy } from './scripts/rusttavern/agent/agent-context-policy.js';
import { normalizeAgentSystemPrompt } from './scripts/rusttavern/agent/agent-system-prompt.js';
import { readLegacyToolInvocations } from './scripts/rusttavern/tool-turn-projection.js';
import {
...
} from './scripts/app/setting/settings-delta-save.js';
...
import { createStartupStatusOverlay } from './scripts/app/startup/startup-status-overlay.js';
...
const TAURI_BACKEND_ERROR_QUEUE_KEY = '__RUSTTAVERN_BACKEND_ERROR_QUEUE__';
const TAURI_BACKEND_ERROR_READY_KEY = '__RUSTTAVERN_BACKEND_ERROR_CONSUMER_READY__';
...
export let displayVersion = 'RustTavern';
...
export let CLIENT_VERSION = 'SillyTavern:UNKNOWN:RustTavern'; // For Horde header
...
function offerChatVirtualizationRecovery(fault, { startup = false } = {}) {
    ...
    chatVirtualizationRecoveryPromise = (async () => {
        try {
            ...
            await updateRustTavernSettings({ chat_virtualization_enabled: false });
...
async function getClientVersion() {
    try {
        ...
        CLIENT_VERSION = data.agent || `SillyTavern:${SILLYTAVERN_COMPAT_VERSION}:RustTavern`;
        const productVersion = data.productVersion || data.pkgVersion;
        displayVersion = `RustTavern ${productVersion || 'UNKNOWN'}`;
        currentVersion = productVersion || '0.0.0';
...
async function firstLoadInit() {
    ...
    const perfEnabled = globalThis.__RUSTTAVERN_PERF_ENABLED__ === true;
    const perfMark = (name) => perfEnabled && globalThis.performance?.mark?.(name);

    const setStage = (stage, message) => {
        globalThis.__RUSTTAVERN_STARTUP_STAGE__ = stage;
        startupStatus.setText(message);
        perfMark(`tt:startup:${stage}`);
    };

    // requestAnimationFrame never fires while the renderer is occluded
    // (backgrounded mobile browser, minimized window, hidden PWA), which
    // would pause the whole boot chain at the first paint wait. Fall back
    // to a timer so startup proceeds even without an actual paint.
    ...
    try {
        ...
        const rustTavernSettings = await getRustTavernSettings();
        initializeChatVirtualization(rustTavernSettings);
        ...
        if (bootstrapSnapshot?.ios_policy?.scope === 'ios') {
            if (!window.__RUSTTAVERN__ || typeof window.__RUSTTAVERN__ !== 'object') {
                throw new Error('[RustTavern][iOSPolicy] Host ABI is unavailable (window.__RUSTTAVERN__).');
            }
            window.__RUSTTAVERN__.iosPolicy = bootstrapSnapshot.ios_policy;
        } else if (window.__RUSTTAVERN__ && typeof window.__RUSTTAVERN__ === 'object') {
            window.__RUSTTAVERN__.iosPolicy = bootstrapSnapshot.ios_policy;
        }
...
/**
 * Lists the chat files that exist on disk for a character.
 * @param {number} characterId
 * @returns {Promise<any[]>} Chat list entries, newest first is not guaranteed.
 */
async function getPersistedCharacterChats(characterId) {
    ...
    return entries.filter(entry => entry && typeof entry === 'object' && typeof entry.file_name === 'string' && entry.file_name);
}

/**
 * Aligns the character's chat pointer with the chats that actually exist.
 *
 * A card can point at a chat file that is not on disk: cards stored without a
 * `chat` field get a name generated from the current time whenever they are
 * loaded, so after a restart the pointer no longer matches the file the
 * previous session wrote. A pointer that does resolve is the chat the user last
 * had open and wins; a dangling one is replaced by the most recent chat and
 * persisted back onto the card so it stops dangling.
 *
 * @param {number} characterId
 * @returns {Promise<boolean>} True when the character has no chat yet and one must be created.
 */
async function resolveCharacterChatPointer(characterId) {
    const character = characters[characterId];
    ...
    const pointer = String(character?.chat ?? '');
    if (chats.some(chat => chat.file_name.replace('.jsonl', '') === pointer)) {
        return false;
    }

    const mostRecent = chats
        .slice()
        .sort((a, b) => sortMoments(timestampToMoment(a.last_mes), timestampToMoment(b.last_mes)))[0];

    // Persist through the targeted attribute merge, not the character form:
    // submitting the form regenerates the greeting of a chat that still holds
    // only its first message, which would overwrite the chat being opened.
    await updateRemoteChatName(characterId, mostRecent.file_name.replace('.jsonl', ''));
    return false;
}
...
export async function selectCharacterById(id, { switchMenu = true } = {}) {
    ...
    if (selected_group || String(this_chid) !== String(id)) {
        ...
        if (!is_send_press) {
            // Unshallow first: it replaces the cached card with the backend
            // copy, which would otherwise overwrite the chat pointer resolved
            // below and make getChat's stillActive guard drop the load.
            await unshallowCharacter(id);
            const allowNewChat = await resolveCharacterChatPointer(id);
...
export async function getOneCharacter(avatarUrl) {
    ...
    if (response.ok) {
        ...
        if (indexOf !== -1) {
            // The backend card may not carry a persisted chat pointer (fresh
            // import or default content). Keep the in-memory pointer so
            // getChat's stillActive guard survives unshallowing.
            getData.chat = String(getData.chat || characters[indexOf]?.chat || '');
...
// Media in a freshly rendered chat grows the scroller after layout, so the tail
// has to be re-pinned once it loads. `load` and `error` do not bubble but they do
// capture, so one listener on #chat covers every image/video/audio, including
// ones rendered later. Wiring a listener per element instead left one behind on
// every re-render, and since each invocation counted only the listeners it had
// just added, the duplicates overshot `media.length` and the scroll stopped
// happening at all.
const MEDIA_LOAD_WINDOW_MS = 1000;
let mediaLoadArmedUntil = 0;
let mediaLoadListenerInstalled = false;

function onChatMediaLoad(event) {
    if (Date.now() > mediaLoadArmedUntil) {
        return;
    }

    const target = event.target;
    const isMedia = target instanceof HTMLImageElement || target instanceof HTMLMediaElement;
    if (!isMedia || !target.closest('.mes_block')) {
        return;
    }
...
export function scrollOnMediaLoad() {
    if (!mediaLoadListenerInstalled && chatElement[0]) {
        for (const mediaLoadEvent of ['load', 'error', 'loadeddata']) {
            chatElement[0].addEventListener(mediaLoadEvent, onChatMediaLoad, { capture: true });
        }
        mediaLoadListenerInstalled = true;
    }
    mediaLoadArmedUntil = Date.now() + MEDIA_LOAD_WINDOW_MS;
}
...
async function maybeSubmitAgentGuidanceFromComposer() {
    ...
    const result = await Popup.show.confirm(
        t`Send as Agent guidance?`,
        t`<p>This message will be submitted as user guidance for the next Agent model request. It will not be saved as a chat message.</p>`,
        {
            okButton: t`Yes`,
            cancelButton: t`No`,
        },
    ...
    if (result !== POPUP_RESULT.AFFIRMATIVE) {
        // Declining must not silently swallow the send: without this the
        // composer looks dead while an Agent run is active.
        toastr.info(t`Message not sent. An Agent run is still active.`, t`Agent Mode`);
        syncAgentGuidanceComposerState();
        return true;
    }
...
export function updateMessageElement(mes, { messageId = chat.length - 1, messageElement = messageTemplate.clone(), adjustMediaScroll = SCROLL_BEHAVIOR.NONE, frontendSourceHandoffEvent = null } = {}) {
    ...
    // .off first: a swipe reuses the existing element, so without it every
    // swipe stacks another identical handler on the same <img>.
    messageElement.find('.avatar img').off('error').on('error', function () {
        $(this).hide();
 ...
 * @param {boolean} [options.respectScrollLock] If true, re-checks scrollLock when the animation frame runs, so a user scroll landing between the caller's check and the frame is not stomped
 */
export function scrollChatToBottom({ waitForFrame, respectScrollLock } = {}) {
    ...
    requestId = requestAnimationFrame(() => {
        if (respectScrollLock && scrollLock) {
            requestId = null;
            return;
        }
...
// Shared compiler + bounded template cache for `extractMessageBias`: this
// runs once per rendered message, so recompiling on every call (and creating
// a fresh Handlebars environment each time) is wasteful on the hot path.
// The `bias` helper accumulates into a per-invocation array, so the compiled
// templates are safe to reuse across calls.
const biasHandlebars = Handlebars.create();
const BIAS_TEMPLATE_CACHE_MAX = 256;
...
export function extractMessageBias(message) {
    ...
    try {
        let template = biasTemplateCache.get(message);
        if (!template) {
            template = biasHandlebars.compile(message);
            if (biasTemplateCache.size >= BIAS_TEMPLATE_CACHE_MAX) {
                const oldestKey = biasTemplateCache.keys().next().value;
                biasTemplateCache.delete(oldestKey);
            }
            biasTemplateCache.set(message, template);
        }
...
class StreamingProcessor {
    ...
    constructor(type, forceName2, timeStarted, continueMessage, promptReasoning) {
        ...
        /**
         * Source text and result of the last `messageFormatting` call, so a
         * render tick that carried no new text can skip reformatting the whole
         * message.
         * @type {{ source: string; html: string }|null}
         */
    ...
    async onProgressStreaming(messageId, text, isFinal) {
        ...
        if (isImpersonate) {
            ...
            // The render interval is fixed but chunks arrive at network pace, so
            // most ticks carry no new text; formatting the whole message again for
            // an unchanged string is pure cost. The final render always re-formats,
            // so the authoritative commit never uses a reused result.
            const reuseFormattedText = !isFinal
                && this.lastFormatted?.source === processedText;
            let formattedText;
            if (reuseFormattedText) {
                formattedText = this.lastFormatted.html;
                // Keep the exported showdown context pointing at this message: a
                // real parse of it would have assigned exactly this.
                mesForShowdownParse = processedText;
            } else {
                formattedText = messageFormatting(
                    processedText,
                    chat[messageId].name,
                ...
                this.lastFormatted = { source: processedText, html: formattedText };
            }
            if (
        ...
        if (!scrollLock) {
            scrollChatToBottom({ waitForFrame: true, respectScrollLock: true });
        }
    }
 ...
 * @property {boolean} [agentMode] Internal RustTavern Agent prompt snapshot mode.
...
function requireAgentPromptAssemblyApi() {
    const promptAssemblyApi = window.__RUSTTAVERN__?.api?.agent?.promptAssembly;
    if (!promptAssemblyApi || typeof promptAssemblyApi.prepare !== 'function' || typeof promptAssemblyApi.buildSnapshot !== 'function') {
        throw new Error('agent.prompt_assembly_api_unavailable: RustTavern Agent prompt assembly API is unavailable');
    }
...
async function pruneAgentPersistentStatesAfterDeletion(deletedStateIds) {
    ...
    const agentApi = window.__RUSTTAVERN__?.api?.agent;
...
export function getThumbnailUrl(type, file, t = false) {
    if (typeof window.__RUSTTAVERN_THUMBNAIL__ === 'function') {
        return window.__RUSTTAVERN_THUMBNAIL__(type, file, t);
    }
...
export async function getChat({ allowNewChat = false } = {}) {
    ...
    try {
        ...
        if (Array.isArray(data) && data.length > 0) {
            ...
            // Not chat.splice(0, chat.length, ...data): spreading the whole
            // array into the call throws RangeError on a very large chat.
            chat.length = 0;
...
async function getChatResult({ allowNewChat = false } = {}) {
    ...
    if (allowNewChat && chat.length === 0) {
        ...
        if (freshChat) {
            // Persist the chat pointer onto the card so later loads resolve the
            // same file instead of a freshly generated name that matches
            // nothing on disk. Merge the single attribute rather than saving
            // the character form, which would regenerate this greeting.
...
async function applySettingsSnapshot(data, initLoaderHandle = null) {
    if (data.result != 'file not find' && data.settings) {
        ...
        captureSettingsSaveBaseline(settings, data.rusttavern_settings_revision);
...
function messageEditAuto(div) {
    const { mesBlock } = updateMessage(div);

    // Note: this used to run messageFormatting on the full text twice per
    // keystroke and write the result with .val() into .mes_text — a no-op on
    // a div — plus a formatted .mes_bias append into the hidden display area.
    // The visible re-render happens in messageEditDone when the editor closes;
    // auto-save only needs the data update and the debounced save.
...
export async function deleteCharacter(characterKey, { deleteChats = true } = {}) {
    ...
    for (const key of characterKey) {
        ...
        const { clearCharacterSkillImportReminders } = await import('./scripts/app/agent-skills/reminders.js');
...
jQuery(async function () {
    ...
    // scrollLock means "the user moved away from the bottom". It is set from
    // user input events only: layout changes (message truncation collapse,
    // streaming growth) also move scrollTop via scroll anchoring, and those
    // adjustments must not stop the streaming follow.
    const markUserScroll = () => { scrollLock = true; };
    chatElementScroll.addEventListener('wheel', markUserScroll, { passive: true });
    chatElementScroll.addEventListener('touchmove', markUserScroll, { passive: true });
    ...
    const readTailGap = () => chatElementScroll.scrollHeight - chatElementScroll.clientHeight - chatElementScroll.scrollTop;

    // Everything above pins the tail once, at the moment a caller asks for it. A
    // message that is still laying out then (mount, marker class toggle, media box,
    // the viewport change on a return from the background) grows the scroller behind
    // that one frame, and nothing re-asserts the tail: the view stays parked short of
    // the bottom until the reader scrolls it. So re-assert on every content or box
    // change, and keep re-asserting while the scroller is still growing rather than
    // trusting a single frame — on a slow device the box lands several frames late.
    //
    // The gate stays `scrollLock`, which only wheel and touchmove set: it is a
    // statement about what the reader did, and content growth must never be able to
    // flip it (a position-derived "is at the bottom" reads a growing scroller as the
    // reader leaving the tail and stops following exactly when it is needed).
    const TAIL_SETTLE_PX = 2;
    // Growth that comes with its own mutation refills this budget, so only
    // mutation-less growth (a transition, a font swap, a media box) is bounded by
    // it; a slow device needs the window to be generous rather than a single frame.
    const TAIL_SETTLE_MAX_FRAMES = 90;
    let tailFollowFrame = null;
    let tailSettleFramesLeft = 0;
    let tailGaveUpLoggedAt = 0;

    const tailFollowTick = () => {
        tailFollowFrame = null;

        if (scrollLock || !power_user.auto_scroll_chat_to_bottom) {
            tailSettleFramesLeft = 0;
            return;
        }
        // An open editor owns the scroll position; never pull it out from under
        // whoever is typing in it.
        if (chatElementScroll.querySelector('textarea, [contenteditable="true"]')) {
            tailSettleFramesLeft = 0;
            return;
        }
        if (readTailGap() <= TAIL_SETTLE_PX) {
            // Settled. Anything landing after this comes with its own mutation,
            // load or resize, which starts a fresh run.
            tailSettleFramesLeft = 0;
            return;
        }

        scrollChatToBottom();
        tailSettleFramesLeft -= 1;
        if (tailSettleFramesLeft > 0) {
            tailFollowFrame = requestAnimationFrame(tailFollowTick);
            return;
        }

        // Ninety frames of growth that never reached the tail is not a slow layout,
        // it is something this code does not understand. Say so, rate-limited, and
        // send it to the server log too — a phone has no devtools, and a drift that
        // reports its own state is diagnosable where a silent one is not.
        const now = Date.now();
        if (now - tailGaveUpLoggedAt > 5000) {
            tailGaveUpLoggedAt = now;
            const detail = {
                gap: readTailGap(),
                scrollHeight: chatElementScroll.scrollHeight,
                clientHeight: chatElementScroll.clientHeight,
                floors: chatElementScroll.children.length,
            };
            console.warn('RustTavern: chat tail still unpinned after re-asserting', detail);
            void invoke('devlog_append_frontend_logs', {
                entries: [{
                    level: 'warn',
                    target: 'script.js',
                    message: `chat tail still unpinned after re-asserting ${JSON.stringify(detail)}`,
                }],
            }).catch(() => {});
        }
    };

    const scheduleTailFollow = () => {
        if (scrollLock || !power_user.auto_scroll_chat_to_bottom) {
            return;
        }
        tailSettleFramesLeft = TAIL_SETTLE_MAX_FRAMES;
        if (tailFollowFrame !== null) {
            return;
        }
        tailFollowFrame = requestAnimationFrame(tailFollowTick);
    };
    // ponytail: mutations and box changes cover the append/layout and the
    // viewport/URL-bar cases; growth with neither (a media element finishing its
    // load) is caught by the capture-phase load listener.
    new MutationObserver(scheduleTailFollow).observe(chatElementScroll, {
        childList: true,
        subtree: true,
        attributes: true,
    ...
    new ResizeObserver(scheduleTailFollow).observe(chatElementScroll);
    chatElementScroll.addEventListener('load', scheduleTailFollow, { capture: true, passive: true });
    ...
    // The preview re-render formats the full notes and rewrites the spoiler
    // DOM; on long notes that visibly lags each keystroke, so it is debounced.
    const creatorNotesPreviewDebounced = debounce(function () {
    ...
    $('#creator_notes_textarea').on('input', creatorNotesPreviewDebounced);
    ...
    $('#options [id]').on('click', async function (event, customData) {
        ...
        if (id == 'option_select_chat') {
            ...
            if (is_send_press == false) {
                if (selected_group) {
                    // Group regeneration never routes through Agent mode; asking
                    // for agent options first would fail-fast
                    // (agent.group_chat_unsupported) as an unhandled rejection
                    // and silently kill the click.
                    regenerateGroup();
                } else {
                    try {
                        const agentOptions = await getAgentGenerationOptions({
    ...
    sliders.forEach(slider => {
        // Direct binding: these sliders are static in index.html. Document-level
        // delegation taxed every input event on the page with a closest() match.
        $(slider.sliderId).on('input', function () {
            const value = $(this).val();
