...
globalThis.__RUSTTAVERN_PERF_ENABLED__ = PERF_ENABLED;
...
async function setupDevThirdPartyExtensionServiceWorker() {
    ...
    const installClientProxyBridge = (ttExtBaseUrl) => {
        ...
        const BRIDGE_KEY = '__RUSTTAVERN_DEV_SW_PROXY_BRIDGE__';
    ...
    try {
        ...
        console.warn('RustTavern: Failed to enable dev third-party extension endpoint:', error);
    }
}
...
async function importWithRetry(specifier, retries = 8, delay = 500) {
    ...
    for (let i = 0; i <= retries; i++) {
        try {
            ...
            console.warn(`RustTavern: import('${specifier}') attempt ${i + 1} failed, retrying in ${delay}ms…`);
...
async function initializeApplication() {
    ...
    try {
        ...
        if (PERF_ENABLED) {
            safePerfMark('tt:init:import:host-main:start');
        }
        await importWithRetry('./host-main.js');
        if (PERF_ENABLED) {
            safePerfMark('tt:init:import:host-main:end');
            safePerfMeasure('tt:init:import:host-main', 'tt:init:import:host-main:start', 'tt:init:import:host-main:end');
        }
        ...
        console.error('RustTavern: Failed to initialize application:', error);
    } finally {
        if (PERF_ENABLED) {
