...
async function viewSecrets() {
    ...
    if (response.status == 403) {
        const hint = globalThis.__TAURI_RUNNING__ === true
            ? t`To view your API keys here, enable Allow Keys Exposure in RustTavern Settings and restart the app.`
    ...
    async function renderSecretsList() {
        ...
        for (const secret of secrets) {
            ...
            itemTemplate.find('button[data-action="copy-secret"]').on('click', async function () {
                ...
                if (secretValue === null) {
                    const hint = globalThis.__TAURI_RUNNING__ === true
                        ? t`The key exposure is disabled. Enable Allow Keys Exposure in RustTavern Settings and restart the app.`
