...
/**
 * Handles node removal from MutationObserver - destroys autocomplete
 * instances whose textarea left the DOM (e.g. an expanded editor popup
 * closing), so their window listeners and observers don't accumulate.
 *
 * @param {Node} node - The removed node to check.
 */
function handleNodeRemoved(node) {
    if (node instanceof Element) {
        const ac = elementAutoCompleteMap.get(node);
        if (ac) {
            ac.destroy();
            elementAutoCompleteMap.delete(node);
            initializedElements.delete(node);
        }
        for (const child of node.querySelectorAll(`[${MACRO_AUTOCOMPLETE_ATTRIBUTE}]`)) {
            const childAc = elementAutoCompleteMap.get(child);
            if (childAc) {
                childAc.destroy();
                elementAutoCompleteMap.delete(child);
                initializedElements.delete(child);
            }
        }
    }
}
