...
export class AutoComplete {
    ...
    constructor(textarea, checkIfActivate, getNameAt, isFloating = false) {
        ...
        // Handlers are kept on the instance so `destroy()` can unbind them.
        this.handleTextareaInput = () => {
            this.selectionStart = this.textarea.selectionStart;
            if (this.text != this.textarea.value) this.show(true, this.wasForced);
        };
        this.handleTextareaKeydown = (evt) => this.handleKeyDown(evt);
        this.handleTextareaClick = () => {
            this.selectionStart = this.textarea.selectionStart;
            if (this.isActive) this.show();
        };
        this.handleTextareaBlur = () => this.hide();
    ...
    /**
     * Release every listener, observer, and DOM node owned by this instance.
     * Call when the owning editor is destroyed (e.g. a Quick Reply editor
     * clone being discarded) so instances don't accumulate per session.
     */
    destroy() {
        ...
        if (t) {
            t.removeEventListener('input', this.handleTextareaInput);
            t.removeEventListener('keydown', this.handleTextareaKeydown);
            t.removeEventListener('click', this.handleTextareaClick);
            t.removeEventListener('blur', this.handleTextareaBlur);
            if (this.isFloating) {
                t.removeEventListener('scroll', this.handleTextareaScroll);
            }
        }
        this.textarea = null;
    }
