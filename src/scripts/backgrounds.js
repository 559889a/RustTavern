...
import { consumeStartupPrefetch, startStartupPrefetch } from './app/startup/startup-prefetch-registry.js';
...
/** Upper bound for the thumbnail blob URL cache; beyond this, the oldest
 * entries are revoked and dropped so the map can't grow without limit. */
const THUMBNAIL_BLOBS_MAX_ENTRIES = 64;

/**
 * Store a thumbnail blob URL, evicting the oldest entry when the cache
 * exceeds its cap. Evicted blob URLs are revoked immediately.
 */
function setThumbnailBlob(bg, entry) {
    THUMBNAIL_BLOBS.set(bg, entry);
    while (THUMBNAIL_BLOBS.size > THUMBNAIL_BLOBS_MAX_ENTRIES) {
        const oldest = THUMBNAIL_BLOBS.keys().next().value;
        if (oldest === undefined) break;
        const evicted = THUMBNAIL_BLOBS.get(oldest);
        THUMBNAIL_BLOBS.delete(oldest);
...
export function getBackgroundPath(fileUrl) {
    if (typeof window.__RUSTTAVERN_BACKGROUND_PATH__ === 'function') {
        return window.__RUSTTAVERN_BACKGROUND_PATH__(fileUrl);
    }
...
async function getCustomBackgroundPreview(bg) {
    ...
    if (candidate) {
        ...
        if (candidate) {
            ...
            setThumbnailBlob(bg, { sourceUrl: url, recipe, etag: candidate.etag || null, blobUrl });
            return blobUrl;
        }
    }
    ...
    setThumbnailBlob(bg, { sourceUrl: url, recipe, etag, blobUrl });
    return blobUrl;
}
...
export function initBackgrounds() {
    ...
    // <base href="/"> resolves "#..." anchors against the site root, dropping the
    // query string. jQuery UI tabs then sees a "non-local" anchor (its _isLocal
    // compares the anchor URL with location.href verbatim), creates replacement
    // panels, and relocates page content into them — breaking the whole layout
    // whenever the app is opened with a query (e.g. the OpenRouter OAuth return
    // `?source=openrouter&query=...`). Re-pointing the anchors at the full
    // current URL keeps the hash links "local" for jQuery UI.
    for (const anchor of document.querySelectorAll('.bg_tabs_list a[href^="#"]')) {
        const hash = anchor.getAttribute('href');
        anchor.setAttribute('href', location.pathname + location.search + hash);
    }
