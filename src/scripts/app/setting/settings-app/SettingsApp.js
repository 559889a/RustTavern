...
function requireAction(actions, name) {
    ...
    if (typeof action !== 'function') {
        throw new Error(`RustTavern settings action is unavailable: ${name}`);
    }

    return action;
}

export function createRustTavernSettingsApp(options) {
    const { viewModel, actions, tr } = options || {};
    if (!viewModel?.capabilities || !viewModel?.values) {
        throw new Error('RustTavern settings view model is required');
    }
    if (typeof tr !== 'function') {
        throw new Error('RustTavern settings translator is required');
    }
