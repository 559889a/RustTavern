...
function requireMethods(source, names, label) {
    for (const name of names) {
        if (typeof source?.[name] !== 'function') {
            throw new Error(`RustTavern Sync ${label} is unavailable: ${name}`);
        }
    }
}
...
export function createRustTavernSyncApp(options) {
    const {
        client,
        actions,
