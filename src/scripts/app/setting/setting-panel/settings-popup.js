...
import {
    ...
    loadRustTavernSettingsViewModel,
} from './settings-view-model.js';
import { buildRustTavernSettingsUpdate } from './settings-patch.js';
import { applyRustTavernSettingsUpdateEffects } from './settings-effects.js';
import { callRustTavernPanelPopup } from '../panel-popup.js';

const SETTINGS_STYLE_ID = 'rusttavern-settings-style';
...
const HELP_TOPICS = {
    ...
    zstdCompression: {
        ...
        lines: [
            ...
            'RustTavern can read zstd backups directly. SillyTavern does not support this backup format.',
        ],
    },
    closeToTray: {
...
function readThemeOptions() {
    ...
    if (!(upstreamThemeSelect instanceof HTMLSelectElement)) {
        throw new Error('RustTavern settings: RustTavern theme selector not found');
    }
...
function getBackgroundThumbnailUrl(filename, isAnimated) {
    ...
    const url = typeof window.__RUSTTAVERN_THUMBNAIL__ === 'function'
        ? window.__RUSTTAVERN_THUMBNAIL__('bg', filename)
...
async function showHelpTopic(topicId) {
    ...
    if (!topic) {
        throw new Error(`Unknown RustTavern settings help topic: ${topicId}`);
    }
...
async function chooseDataRoot() {
    ...
    if (!normalized) {
        // Server mode has no native directory dialog (openDialog always
        // returns null there): explain the supported path instead of the
        // button silently doing nothing.
        const notice = createPopupColumn();
        const text = document.createElement('div');
        text.textContent = translate('Directory selection is unavailable in server mode. Start the server with --data-root <path> (or edit config.yaml) to change the data directory.');
        notice.append(text);
        await callGenericPopup(notice, POPUP_TYPE.TEXT, '', {
            okButton: translate('Close'),
            wide: false,
            large: false,
        });
...
export async function openRustTavernSettingsPopup() {
    ensureSettingsStyle();
    ...
    const appHandle = bundle.mountRustTavernSettingsApp(mount, {
        viewModel,
    ...
    try {
        const popupPromise = callRustTavernPanelPopup(mount, POPUP_TYPE.CONFIRM, '', {
            ...
            onClosing: async (popup) => {
                ...
                try {
                    pendingUpdate = buildRustTavernSettingsUpdate(viewModel.values, appHandle.getDraft());
                } catch (error) {
        ...
        const update = pendingUpdate
            ?? buildRustTavernSettingsUpdate(viewModel.values, appHandle.getDraft());
        if (!update.hasChanges) {
            return;
        }

        const updatedSettings = await updateRustTavernSettings(update.patch);
        applyRustTavernSettingsUpdateEffects(update, updatedSettings);
    } finally {
        appHandle.unmount();
    }
}
