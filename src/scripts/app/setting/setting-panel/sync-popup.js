...
const SYNC_STYLE_ID = 'rusttavern-sync-style';
...
async function editSyncScope(catalog, selection) {
    ...
    const appHandle = bundle.mountRustTavernSyncScopeApp(content, {
        catalog,
        selection,
        tr: translate,
    });
...
function canScanPairUri() {
    // QR-code pairing requires a native barcode scanner, which the server
    // form does not provide; manual URI entry is always available.
    return false;
}
...
export async function openSyncPopup() {
    ...
    const appHandle = bundle.mountRustTavernSyncApp(mount, {
        client,
