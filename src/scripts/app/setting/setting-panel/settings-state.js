...
} from '../../../../host/main/services/embedded-runtime/embedded-runtime-profile-state.js';
...
function requireChatBackupLimit(value, field) {
    if (!Number.isSafeInteger(value) || value < -1) {
        throw new Error(`RustTavern settings: invalid chat backup ${field}`);
    }
 ...
 */
export function createRustTavernSettingsState(settings, options = {}) {
    ...
    if (typeof chatVirtualizationEnabled !== 'boolean') {
        throw new Error('RustTavern settings: chat virtualization setting missing');
    }
    ...
    if (typeof avatarPersonaOriginalImagesEnabled !== 'boolean') {
        throw new Error('RustTavern settings: avatar/persona original images setting missing');
    }

    const dynamicTheme = settings.dynamic_theme;
    if (!dynamicTheme || typeof dynamicTheme !== 'object') {
        throw new Error('RustTavern settings: dynamic theme settings missing');
    }

    const chatBackups = settings.chat_backups;
    if (!chatBackups || typeof chatBackups !== 'object') {
        throw new Error('RustTavern settings: chat backup settings missing');
    }
    if (typeof chatBackups.automatic_enabled !== 'boolean') {
        throw new Error('RustTavern settings: automatic chat backup setting missing');
    }
    if (typeof chatBackups.zstd_compression_enabled !== 'boolean') {
        throw new Error('RustTavern settings: Zstandard chat backup setting missing');
    }
