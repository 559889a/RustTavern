...
export function createRustTavernDevLogsApp(options) {
    const {
        kind,
        title = '',
        initialEntries = [],
    ...
    if (typeof tr !== 'function') {
        throw new Error('RustTavern dev logs translator is required');
    }
    if (!client || typeof client !== 'object') {
        throw new Error('RustTavern dev logs client is required');
    }
    if (!actions || typeof actions !== 'object') {
        throw new Error('RustTavern dev logs actions are required');
    }
    if (kind !== 'live' && kind !== 'llm-api') {
        throw new Error(`Unsupported RustTavern dev logs panel: ${kind}`);
    }

    return {
        name: 'RustTavernDevLogsApp',
        components: {
            LiveLogPanel,
            LlmApiLogsPanel,
        },
