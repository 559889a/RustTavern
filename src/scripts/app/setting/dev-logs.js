...
import { trimFrontendLogEntriesInPlace } from '../../../host/main/services/dev-logging/frontend-log-retention.js';
...
const DEV_LOGS_STYLE_ID = 'rusttavern-dev-logs-style';

function getDevApi() {
    const api = window.__RUSTTAVERN__?.api?.dev;
    if (!api) {
        throw new Error('RustTavern host dev API is unavailable');
    }
    return api;
}
...
async function openDevLogsPopup(options) {
    ...
    const appHandle = bundle.mountRustTavernDevLogsApp(mount, {
        ...options,
    ...
    try {
        await callRustTavernPanelPopup(mount, POPUP_TYPE.TEXT, '', {
