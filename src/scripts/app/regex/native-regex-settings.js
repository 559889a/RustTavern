...
import { getRustTavernSettings, updateRustTavernSettings } from '../../../host-bridge.js';
...
export async function persistNativeRegexBackendEnabled(enabled) {
    const updatedSettings = await updateRustTavernSettings({
...
export function installNativeRegexBackendSetting() {
    const ready = getRustTavernSettings().then(syncNativeRegexBackendEnabledFromSettings);
    return { ready };
}
