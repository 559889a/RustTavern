...
import { SURFACE, applySurface } from '../../rusttavern/layout-kit.js';
import { characterStemFromAvatarFileName } from '../../../host/main/services/characters/character-identity.js';
...
function getSkillApi() {
    ...
    if (!skillApi || typeof skillApi !== 'object') {
        throw new Error('RustTavern Agent Skill API is not available');
    }

    if (typeof skillApi.previewImport !== 'function' || typeof skillApi.installImport !== 'function') {
        throw new Error('RustTavern Agent Skill API is incomplete');
    }
...
function buildPreviewPopupContent(previews, sourceLabel) {
    ...
    root.classList.add('rusttavern-agent-skill-import');
...
async function showImportPopup(previews, sourceLabel) {
    ...
    popup.dlg.classList.add('rusttavern-agent-skill-import-popup');
