...
function createRegexRefreshCoordinator() {
    ...
    function collectQueue() {
        ...
        for (const node of document.querySelectorAll('#chat > .mes[mesid]')) {
            ...
            if (!message) {
                // Chat switching clears the chat array before the DOM: nodes
                // without a backing message are about to be unmounted, so skip
                // them instead of aborting the whole refresh cycle.
                continue;
            }
