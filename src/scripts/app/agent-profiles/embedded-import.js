...
import { sanitizePortableAgentProfile } from '../../rusttavern/agent/agent-profile-portable.js';
...
function getAgentProfileApi() {
    ...
    if (!profileApi || typeof profileApi !== 'object') {
        throw new Error('RustTavern Agent Profile API is not available');
    }

    if (typeof profileApi.load !== 'function' || typeof profileApi.save !== 'function') {
        throw new Error('RustTavern Agent Profile API is incomplete');
    }
...
function buildPreviewPopupContent(previews, sourceLabel) {
    ...
    root.classList.add('rusttavern-agent-profile-import');
...
async function showImportPopup(previews, sourceLabel) {
    ...
    popup.dlg.classList.add('rusttavern-agent-profile-import-popup');
