...
import { WorldInfoCache } from './world-info-cache.js';
import { renderTemplateAsync } from './templates.js';
import { t } from './i18n.js';
import { accountStorage } from './util/AccountStorage.js';
import { getOrCreatePersonaDescriptor, setPersonaDescription, user_avatar } from './personas.js';
import { normalizeWorldInfoActivationBatch } from './rusttavern/agent/world-info-activation.js';
...
// Bumped every time the WI entry list starts rendering. Chunked rendering
// spans several frames, so a newer render (page change, page-size change,
// editor refresh) must be able to supersede an in-flight one instead of
// interleaving its appends with it.
let worldEntryRenderGeneration = 0;
 ...
 *
 * Evicts inactive books over a serialized-size cap (see `world-info-cache.js`);
 * books active for the current chat are always retained.
 *
 * @type {WorldInfoCache}
 * */
export const worldInfoCache = new WorldInfoCache({ cloneOnGet: true, cloneOnSet: false, isRetained: isWorldInfoRetained });
...
export async function loadWorldInfo(name) {
    ...
    const task = (async () => {
        ...
        if (response.ok) {
            // arrayBuffer + decode is what .json() does internally; the byte
            // length doubles as the cache-size hint for free.
            const buffer = await response.arrayBuffer();
            const data = JSON.parse(new TextDecoder().decode(buffer));
            worldInfoCache.set(name, data, { bytes: buffer.byteLength });
            return data;
        }
...
function updateWorldEntryKeyOptionsCache(keyOptions, { remove = false, reset = false } = {}) {
    ...
    // Drop keys no entry references any more: decrementing alone left them in
    // the cache forever, so it grew with every distinct key seen in a session.
    for (let index = worldEntryKeyOptionsCache.length - 1; index >= 0; index--) {
        if (worldEntryKeyOptionsCache[index].count <= 0) {
            worldEntryKeyOptionsCache.splice(index, 1);
        }
    }
...
async function displayWorldEntries(name, data, navigation = navigation_option.none, flashOnNav = true) {
    ...
    function appendEntryChunk(blocks, isCustomOrder) {
        if (!isCustomOrder) {
            blocks.forEach(block => block.find('.drag-handle').remove());
        }
        worldEntriesList.append(blocks);
    }

    function yieldToFrame() {
        // rAF never fires in hidden tabs; cap the yield so rendering completes.
        return Promise.race([
    ...
    $('#world_info_pagination').pagination({
        ...
        callback: async function (/** @type {object[]} */ page) {
            const generation = ++worldEntryRenderGeneration;
            try {
                clearEntryList(worldEntriesList);
                ...
                // Append in frame-yielding chunks: building a whole
                // 500/1000-entry page in one go blocks the tab for seconds.
                const CHUNK_SIZE = 10;
                let chunk = [];

                for (const entry of page) {
                    if (generation !== worldEntryRenderGeneration) {
                        return;
                    }
                    ...
                    if (chunk.length >= CHUNK_SIZE) {
                        appendEntryChunk(chunk, isCustomOrder);
                        chunk = [];
                        await yieldToFrame();
                    }
                }

                if (chunk.length > 0 && generation === worldEntryRenderGeneration) {
                    appendEntryChunk(chunk, isCustomOrder);
                }
            } catch (error) {
...
// The set of books that are active for the current chat: read on every
// generation by the lore collectors, so the cache must never evict them.
function collectWorldInfoNames() {
    const names = new Set(selected_world_info || []);

    for (const worldName of collectCharacterWorldsToSearch().worldsToSearch) {
        names.add(worldName);
    }
    ...
    const personaWorld = power_user.persona_description_lorebook;
    if (personaWorld) {
        names.add(personaWorld);
    }

    return names;
}

function isWorldInfoRetained(name) {
    return collectWorldInfoNames().has(name);
}

async function collectWorldInfoEntries() {
    await prefetchWorldInfos(collectWorldInfoNames());
...
export function initWorldInfo() {
    ...
    $('#WorldInfo').on('scroll', () => {
        // Cheap pre-filter: this handler only closes open autocomplete menus.
        // When none is open (the common case while inertia-scrolling at
        // 60-120 events/sec), skip the full per-entry scan entirely.
