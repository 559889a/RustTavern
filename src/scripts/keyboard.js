...
/**
 * Cached compound selector matching every entry of `interactableSelectors`.
 * Invalidation happens in `registerInteractableType` (the only push site).
 * @type {string}
 */
let compoundSelectorCache = interactableSelectors.join(', ');
...
const observer = new MutationObserver(mutations => {
    ...
    if (batch.length > 0) {
        handleNodeBatch(batch);
    }
});

/**
 * Processes a batch of added nodes with one compound-selector query per batch
 * instead of one query per node per selector (the per-message cost was
 * O(nodes × selectors) selector engine calls; now O(1) compound queries per batch).
 * @param {Element[]} nodes
 */
function handleNodeBatch(nodes) {
    const roots = nodes.filter(node => node.nodeType === Node.ELEMENT_NODE && node instanceof Element);
    if (roots.length === 0) return;

    // Scroll reset containers: one query per batch
    for (const node of roots) {
        if (node.classList.contains('scroll-reset-container')) {
            applyScrollResetBehavior(node);
        }
        if (node.classList.contains('scroll-reset-container') || node.querySelector('.scroll-reset-container')) {
            initializeScrollResetBehaviors(node);
        }
    }

    // Collect all candidate elements (the roots and their interactable descendants)
    // with a single compound selector per root, then deduplicate.
    const candidates = new Set();
    for (const node of roots) {
        if (node.matches(compoundSelectorCache)) {
            candidates.add(node);
        }
        for (const el of node.querySelectorAll(compoundSelectorCache)) {
            candidates.add(el);
        }
    }
    // Elements already carrying the marker class only need the tabindex sync,
    // but makeKeyboardInteractable is idempotent, so one pass over the
    // deduplicated set is both correct and cheap.
    makeKeyboardInteractable(...candidates);
}
...
export function registerInteractableType(interactableSelector, { disabledByDefault = false, notFocusableByDefault = false } = {}) {
    ...
    compoundSelectorCache = interactableSelectors.join(', ');
...
export function makeKeyboardInteractable(...interactables) {
    interactables.forEach(interactable => {
        ...
        if (!interactable.classList.contains(INTERACTABLE_CONTROL_CLASS) && !interactable.matches(compoundSelectorCache)) {
