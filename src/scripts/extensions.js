...
import { waitForTauriMainReady } from './extensions/runtime/host-ready.js';
...
// Extensions populate the menu asynchronously after load, so poll for a while —
// but stop either way. Previously only the "found items" branch cleared the
// interval, so a setup where every item fails to load polled this DOM query
// once a second for the whole session.
const MENU_CHECK_ATTEMPTS_MAX = 60;
...
function showHideExtensionsMenu() {
    ...
    // We have menu items, or we have waited long enough, so stop checking
    if (hasMenuItems || ++menuCheckAttempts >= MENU_CHECK_ATTEMPTS_MAX) {
