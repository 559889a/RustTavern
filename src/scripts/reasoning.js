...
import { shouldCommitStreamingMessage } from './app/perf/streaming-render-policy.js';
...
export class ReasoningHandler {
    ...
    constructor(timeStarted = null) {
        ...
        /**
         * Source text and result of the last reasoning `messageFormatting` call,
         * so a render tick that carried no new reasoning text can skip the parse.
    ...
    updateDom(messageId, { final = false } = {}) {
        ...
        // Update the reasoning message. The reasoning stream is rendered on the
        // same fixed interval as the message, so a tick that carried no new text
        // would otherwise re-parse the whole block; the final render re-parses.
        const reasoning = trimSpaces(this.reasoningDisplayText ?? this.reasoning);
        let displayReasoning;
        if (!final && this.lastFormatted?.source === reasoning) {
            displayReasoning = this.lastFormatted.html;
        } else {
            displayReasoning = messageFormatting(reasoning, '', false, false, messageId, {}, true);
            this.lastFormatted = { source: reasoning, html: displayReasoning };
        }
