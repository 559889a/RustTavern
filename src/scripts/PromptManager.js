...
class PromptManager {
    ...
    constructor() {
        ...
        /** Same dry run, but repaints the existing rows instead of rebuilding the list. */
        this.renderRowStateLatest = createLatestTaskScheduler(
            () => this.#renderAfterTryGenerate(() => this.#refreshPromptListState()),
            error => console.error('Prompt manager row refresh failed', error),
        );
    }
    ...
    init(moduleConfiguration, serviceSettings) {
        ...
        this.handleToggle = (event) => {
            const row = event.target.closest('.' + this.configuration.prefix + 'prompt_manager_prompt');
            ...
            // Flip the visible state right away: the authoritative re-render
            // goes through a full prompt dry-run and takes hundreds of ms on
            // large presets, which read as the toggle not responding.
            const toggleIcon = row.querySelector('.prompt-manager-toggle-action');
            if (toggleIcon) {
                toggleIcon.classList.toggle('fa-toggle-on', promptOrderEntry.enabled);
                toggleIcon.classList.toggle('fa-toggle-off', !promptOrderEntry.enabled);
            }
            row.classList.toggle(this.configuration.prefix + 'prompt_manager_prompt_disabled', !promptOrderEntry.enabled);
            // Only the toggled row and the token totals change, so refresh the
            // existing rows after the dry run instead of rebuilding the list.
            this.renderRowStateLatest();
    ...
    async #renderAfterTryGenerate(updateUi = () => this.#renderPromptManagerUi()) {
        ...
        try {
            ...
            // The error banner only exists in the full render, so a failed
            // dry run always falls back to it.
            await (this.error ? this.#renderPromptManagerUi() : updateUi());
        }
    }
    ...
    /**
     * Computes the token cell contents for one prompt row.
     *
     * Shared by the full list render and the in-place row refresh, so the
     * chat-history warning rule lives in exactly one place.
     *
     * @param {object} prompt
     * @returns {{ calculatedTokens: number|string, warningClass: string, warningTitle: string }}
     */
    #getPromptTokenDisplay(prompt) {
        ...
        return { calculatedTokens: tokens ? tokens : '-', warningClass, warningTitle };
    }

    /**
     * Repaints the state already shown by the existing prompt rows, without
     * rebuilding the list.
     *
     * A toggle only changes one row's enabled flag and the token totals, but
     * the full render tears down the container, rebuilds every row and
     * re-initializes the jQuery UI sortable - measured at 540ms on a 175-prompt
     * preset, 459ms of it the sortable re-init on freshly created rows. This
     * pass rewrites the affected text/classes of the rows already in the DOM
     * (~3ms) and leaves element identity alone, so the per-row click handlers
     * and the sortable instance both stay valid.
     */
    #refreshPromptListState() {
        const { prefix } = this.configuration;
        const list = this.listElement;
        if (!list) return;

        for (const row of list.querySelectorAll(`.${prefix}prompt_manager_prompt`)) {
            const prompt = this.getPromptById(row.dataset.pmIdentifier);
            if (!prompt) continue;

            const enabled = !!this.getPromptOrderEntry(this.activeCharacter, prompt.identifier)?.enabled;
            ...
            if (tokenCell) {
                const { calculatedTokens, warningClass, warningTitle } = this.#getPromptTokenDisplay(prompt);
                tokenCell.dataset.pmTokens = String(calculatedTokens);
                tokenCell.innerHTML = `<span class="${warningClass}" title="${warningTitle}"> </span>${calculatedTokens}`;
            }
        }

        const tokenTotalLabel = this.containerElement?.querySelector(`.${prefix}prompt_manager_header > div:last-child`);
        const tokenTotalNode = tokenTotalLabel && Array.from(tokenTotalLabel.childNodes).find(node => node.nodeType === Node.TEXT_NODE);
        if (tokenTotalNode) {
            tokenTotalNode.textContent = ` ${this.tokenUsage} `;
        }
    }
