...
export async function init() {
    ...
    // Skip the tick while the tab is hidden: nothing on screen can change, and
    // each tick forces a layout through jQuery's :visible, which is not worth
    // waking the CPU for on the phone. The next tick after the user returns
    // (at most UPDATE_INTERVAL later) picks up whatever changed meanwhile.
    setInterval(() => {
        if (document.hidden) {
            return;
        }
