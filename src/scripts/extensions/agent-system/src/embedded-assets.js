...
} from '../../../rusttavern/agent/agent-profile-portable.js';
import {
...
function buildCharacterJsonData(character, rusttavern) {
    const jsonData = character.json_data ? JSON.parse(character.json_data) : {};
    jsonData.data = jsonData.data || {};
    jsonData.data.extensions = jsonData.data.extensions || {};
    jsonData.data.extensions.tauritavern = rusttavern;
    return jsonData;
}

async function writeCharacterRustTavernPatch(target, patch) {
    const patchValue = clone(requirePlainObject(patch, 'rusttavern patch'));
    const current = clone(target.character?.data?.extensions?.tauritavern || {});
    const nextRustTavern = {
        ...current,
        ...patchValue,
    };
    const jsonData = buildCharacterJsonData(target.character, nextRustTavern);
    ...
    target.character.data.extensions.tauritavern = nextRustTavern;
...
async function writeProfiles(target, packageValue) {
    ...
    await writeCharacterRustTavernPatch(target, { agentProfiles: packageValue });
}
...
async function writeSkills(target, packageValue) {
    ...
    await writeCharacterRustTavernPatch(target, { skills: packageValue });
}
