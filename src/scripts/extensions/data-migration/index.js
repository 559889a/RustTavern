...
import { getActiveIosPolicyActivationReport } from '../../rusttavern/ios-policy.js';
...
const SILLYTAVERN_MIGRATION_COPY_KEY = 'Import a SillyTavern data archive (zip, tar, tar.gz, or tgz) and migrate it to RustTavern.';
const RUSTTAVERN_MIGRATION_COPY_KEY = 'Import a RustTavern data zip archive from another device and migrate it to this RustTavern.';
...
function refreshExportActions() {
    ...
    const visible = Boolean(savedPath);
...
jQuery(async () => {
    ...
    if (iosPolicy?.profile === 'ios_external_beta') {
        ...
        if (!(description instanceof HTMLElement)) {
            throw new Error('[RustTavern][iOSPolicy] Data migration description element not found');
        }

        description.dataset.i18n = RUSTTAVERN_MIGRATION_COPY_KEY;
        description.textContent = translate(RUSTTAVERN_MIGRATION_COPY_KEY);
    }
