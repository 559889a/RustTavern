...
class TtsWebuiProvider {
    ...
    async initAudioWorklet(wavSampleRate) {
        // One AudioContext per narrated message would pile up: browsers cap the
        // number of concurrent contexts (Chromium allows about six), after which
        // the constructor throws and playback stops working at all. Reuse the
        // existing graph while the sample rate matches, and tear the old one
        // down when it does not.
        if (this.audioContext && this.audioWorkletNode
            && this.audioContext.state !== 'closed'
            && this.audioContext.sampleRate === wavSampleRate) {
    ...
    /** Releases the streaming audio graph and its render thread. */
    async closeAudioWorklet() {
        ...
        try {
            ...
            console.warn('TTS WebUI: failed to disconnect the audio worklet node:', error);
        }
        ...
        if (context && context.state !== 'closed') {
            try {
                ...
                console.warn('TTS WebUI: failed to close the audio context:', error);
            }
        }
    }
    ...
    async previewTtsVoice(voiceId) {
        ...
        if (this.settings.streaming) {
            ...
            // Release the previous blob URL before replacing it (an
            // interrupted preview never fires `ended`).
