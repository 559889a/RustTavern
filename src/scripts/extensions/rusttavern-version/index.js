...
const MODULE_NAME = 'rusttavern-version';
...
let rustTavernSettingsCache = null;
let rustTavernSettingsPromise = null;
...
function buildVersionInfo(payload = null) {
    const agent = typeof payload?.agent === 'string' && payload.agent.trim()
        ? payload.agent.trim()
        : (String(CLIENT_VERSION || '').trim() || 'SillyTavern:UNKNOWN:RustTavern');

    const packageVersion = typeof payload?.productVersion === 'string' && payload.productVersion.trim()
        ? payload.productVersion.trim()
...
async function resolveVersionInfo() {
    ...
    if (!versionInfoPromise) {
        versionInfoPromise = getBridgeClientVersion()
            ...
            .catch((error) => {
                console.warn('RustTavern version extension fallback:', error);
...
function renderVersionInfo(info) {
    $('#rusttavern_version_number').text(info.packageVersion);
    $('#rusttavern_compat_version').text(info.compatBaseline);
    $('#rusttavern_git_info').text(info.gitInfo);
}

async function onExportDebugBundleClick() {
    const $btn = $('#rusttavern_export_debug_bundle');
    ...
    try {
        const devApi = window.__RUSTTAVERN__?.api?.dev;
        if (!devApi || typeof devApi.exportBundle !== 'function') {
            throw new Error('RustTavern host dev API exportBundle is unavailable');
        }
        ...
        console.error('RustTavern debug bundle export failed:', error);
...
function showUpdateResult(result) {
    ...
    const $result = $('#rusttavern_update_result');
    if (!$result.length) {
        return;
    }

    $('#rusttavern_update_version').text(getReleaseLabel(result));
    $('#rusttavern_update_changelog').html(renderChangelogHtml(release.body));
    $('#rusttavern_update_download').attr('href', release.html_url);
...
async function onCheckUpdateClick() {
    ...
    const $btn = $('#rusttavern_check_update');
    const $icon = $btn.find('i');
    const $text = $btn.find('span');
    const defaultText = String($text.data('defaultLabel') || $text.text()).trim();
...
async function getRustTavernSettingsState() {
    if (rustTavernSettingsCache) {
        return rustTavernSettingsCache;
    }

    if (!rustTavernSettingsPromise) {
        rustTavernSettingsPromise = getRustTavernSettings()
            .then((settings) => {
                rustTavernSettingsCache = settings;
                return settings;
            })
            .finally(() => {
                rustTavernSettingsPromise = null;
            });
    }

    return rustTavernSettingsPromise;
}
...
async function hasSeenStartupUpdate(result) {
    ...
    const settings = await getRustTavernSettingsState();
...
async function rememberStartupUpdate(result) {
    ...
    rustTavernSettingsCache = await updateRustTavernSettings({
        updates: {
            startup_popup: {
...
function buildStartupUpdatePopupContent(result) {
    ...
    meta.textContent = result.channel === 'canary'
        ...
        : `RustTavern ${result.current_version}  \u2192  ${release.version}`;
...
function renderChannel(channel) {
    const radio = document.querySelector(`input[name="rusttavern_update_channel"][value="${channel}"]`);
...
function setChannelInputsDisabled(disabled) {
    document.querySelectorAll('input[name="rusttavern_update_channel"]').forEach((input) => {
...
async function onUpdateChannelChange(event) {
    ...
    const previousChannel = rustTavernSettingsCache?.updates?.channel
...
jQuery(async () => {
    const container = $('#rusttavern_version_container');
    if (!container.length) {
        return;
    }
    ...
    $('#rusttavern_export_debug_bundle').on('click', () => void onExportDebugBundleClick());
    ...
    if (aboutCaps && aboutCaps.git_info === false) {
        ...
        if (!(gitRow instanceof HTMLElement)) {
            throw new Error('[RustTavern][iOSPolicy] ttv-git-row not found');
        }
        gitRow.hidden = true;
    }
    ...
    const updateCaps = resolveIosUpdateCapabilities();
    if (updateCaps && updateCaps.manual_check === false) {
        $('#rusttavern_check_update').remove();

        const channelRow = document.getElementById('ttv-channel-row');
        if (!(channelRow instanceof HTMLElement)) {
            throw new Error('[RustTavern][iOSPolicy] ttv-channel-row not found');
        }
    ...
    $('#rusttavern_update_dismiss').on('click', hideUpdateResult);
    ...
    container.find('input[name="rusttavern_update_channel"]').on('change', onUpdateChannelChange);
