...
/** Upper bound for the summary cache; the oldest entry is dropped beyond it. */
const CACHED_SUMMARIES_MAX_ENTRIES = 1000;
/**
...
async function summarize(hashedMessages, endpoint = 'main', { skipOnFailure = false } = {}) {
    ...
    for (const element of hashedMessages) {
        ...
        if (cachedSummaries.size > CACHED_SUMMARIES_MAX_ENTRIES) {
            cachedSummaries.delete(cachedSummaries.keys().next().value);
        }
    }
        ...
        function getErrorMessage(cause) {
            switch (cause) {
                ...
                case 'vector_endpoint_unavailable':
                    return 'Vector Storage backend is not implemented in the native RustTavern backend.';
                default:
...
/** Upper bound for the hash cache; the oldest entry is dropped beyond it. */
const HASH_CACHE_MAX_ENTRIES = 5000;
