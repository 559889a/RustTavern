...
export class QuickReply {
                ...
                const actions = document.createElement('div'); {
                    ...
                    const exp = document.createElement('div'); {
                        ...
                        exp.addEventListener('click', ()=>{
                            ...
                            // Release after a tick so the download has started.
    ...
    async showEditor() {
        ...
        if (response.ok) {
            ...
            let wasSyntax = null;
            /**@type {number|null}*/
            let syntaxFrame = null;
            const scheduleSyntaxLoop = ()=>{
                if (syntaxFrame !== null) return;
                syntaxFrame = requestAnimationFrame(updateSyntaxLoop);
            };
            const updateSyntaxLoop = ()=>{
                syntaxFrame = null;
                const now = Date.now();
                // fps limit
                if (now - lastSyntaxUpdate < fpsTime) return scheduleSyntaxLoop();
                // elements don't exist (yet?)
                if (!messageSyntaxInner || !message)  return scheduleSyntaxLoop();
                // elements no longer part of the document
                if (!messageSyntaxInner.closest('body')) return;
                // debugger is running
                if (this.isExecuting) {
                    lastMessageValue = null;
                    return scheduleSyntaxLoop();
                }
                // value hasn't changed
                if (wasSyntax == syntax.checked && lastMessageValue == message.value) return;
            ...
            // The editor is gone: release the autocomplete instance's window
            // listener and observers so each edit session doesn't accumulate one.
            ac?.destroy();
