...
import { CHAT_LAYOUT_CHANGED_EVENT } from '../host/main/services/chat-surface/install.js';
...
export const power_user = {
    ...
    chat_truncation: 100,
    // Streaming preview render frequency. Each render re-formats the full
    // accumulated text, so a lower rate keeps weak CPUs responsive during
    // generation without a visible quality change.
    streaming_fps: 18,
...
// Bounded cache of compiled story string templates: the template text is
// user-configured and changes rarely, while `renderStoryString` runs on
// every message send — recompiling each time is wasteful.
const storyStringTemplateCache = new Map();
const STORY_STRING_TEMPLATE_CACHE_MAX = 32;
...
export function renderStoryString(params, { customStoryString = null, customInstructSettings = null, customContextSettings = null } = {}) {
    try {
        ...
        let compiledTemplate = storyStringTemplateCache.get(storyString);
        if (!compiledTemplate) {
            compiledTemplate = Handlebars.compile(storyString, { noEscape: true });
            if (storyStringTemplateCache.size >= STORY_STRING_TEMPLATE_CACHE_MAX) {
                const oldestKey = storyStringTemplateCache.keys().next().value;
                storyStringTemplateCache.delete(oldestKey);
            }
            storyStringTemplateCache.set(storyString, compiledTemplate);
        }
...
function warnInvalidCharacterCreateDatesOnce(entities) {
    ...
    console.error('[RustTavern] Invalid character create_date detected; sorting falls back to date_added/filename.', {
