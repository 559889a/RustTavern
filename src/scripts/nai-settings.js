...
let novel_data = null;
let badWordsCache = {};
// Upper bound for `badWordsCache`: its key derives from the banned-tokens text
// plus the model, so every edit of that field would otherwise add a permanent
// entry holding a token-id array.
const BAD_WORDS_CACHE_MAX = 32;
...
function getBadWordIds(banned_tokens, tokenizerType) {
    ...
    const cacheKeys = Object.keys(badWordsCache);
    if (cacheKeys.length >= BAD_WORDS_CACHE_MAX) {
        delete badWordsCache[cacheKeys[0]];
    }
...
export function initNovelAISettings() {
    sliders.forEach(slider => {
        // Direct binding: these inputs are static in index.html. Document-level
        // delegation taxed every input event on the page with a closest() match
        // per entry (19 entries here).
