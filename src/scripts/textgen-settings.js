...
async function getStatusTextgen() {
    ...
    try {
        ...
        if (!response.ok) {
            // A failed status check carries no model list and no `result`, and
            // several branches below fall back to "Connected" when `result` is
            // missing. Fail loudly so the catch path reports the real state
...
export function initTextGenSettings() {
    ...
    for (const i of setting_names) {
        // Direct binding: these inputs are static in index.html, and document-level
        // delegation taxes every input event on the page with a closest() match per
        // entry (74 entries here alone). jQuery silently skips ids that don't exist.
        $(`#${i}_textgenerationwebui`).attr('x-setting-id', i).on('input', function () {
