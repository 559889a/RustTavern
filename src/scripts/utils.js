...
/**
 * Batch variant of resetScrollHeight: write/read/write phases are separated so
 * the browser flushes layout once for the whole batch instead of once per
 * element (N textareas opened in a drawer otherwise cost N forced layouts,
 * each re-measured against the full dirty chat subtree).
 * @param {JQuery<HTMLElement> | Iterable<HTMLElement>} elements Elements to resize.
 */
export function resetScrollHeights(elements) {
    const list = Array.from(elements);
    if (list.length === 0) return;
    for (const el of list) {
        el.style.height = 'auto';
    }
    const heights = list.map(el => el.scrollHeight + 3);
    for (let i = 0; i < list.length; i++) {
        list[i].style.height = heights[i] + 'px';
    }
}
...
/** Upper bound for the timestamp cache; beyond this the oldest entries are
 * dropped so the map can't grow without limit. */
const DATE_CACHE_MAX_ENTRIES = 1000;
...
export function timestampToMoment(timestamp) {
    ...
    if (dateCache.size > DATE_CACHE_MAX_ENTRIES) {
