...
function showBackupSecretsWarning() {
    const message = isTauriRuntime()
        ? 'The backup will not include secrets because Allow Keys Exposure is disabled in RustTavern Settings.'
...
jQuery(() => {
    ...
    setInterval(async () => {
        // Nobody is reading while the tab is hidden, so there is nothing to
        // keep alive and no reason to wake the network on the phone's battery.
