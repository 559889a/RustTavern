...
import { allowlistSettingAllows, getActiveIosPolicyCapabilities } from './rusttavern/ios-policy.js';
import { materializeInitialChatHistoryMessages } from './rusttavern/agent/agent-context-policy.js';
import { projectToolTurns } from './rusttavern/tool-turn-projection.js';
...
import { appendClaudeRefusalWarning, ClaudeNativeStreamAccumulator, getClaudeStopStatus, hasClaudeToolUse } from './rusttavern/claude-native-stream.js';
import {
    createThumbnail,
    delay,
    download,
...
export function getAwsBedrockModelMetadata(modelId = null) {
    ...
    return model?.rusttavern?.bedrock ?? null;
}
...
function addCustomModelForSource(source, modelId) {
    ...
    if (existingOption && !existingOption.closest('[data-rusttavern-custom-models="true"]')) {
...
function isCustomModelValueForSource(source, value) {
    ...
    return !option || Boolean(option.closest('[data-rusttavern-custom-models="true"]'));
}
...
function applyCustomModelOptionsForSource(source) {
    ...
    if (customGroupModels.length > 0) {
        ...
        customGroup.dataset.rusttavernCustomModels = 'true';
    ...
    actionGroup.dataset.rusttavernCustomModels = 'true';
...
function removeCustomModelOptionsForSource(source) {
    ...
    selectEntry.element.querySelectorAll('[data-rusttavern-custom-models="true"]').forEach(node => node.remove());
}
...
function saveModelList(data) {
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.AWS_BEDROCK) {
        ...
        // in index.html. The backend attaches RustTavern Bedrock metadata to
        ...
        const appendOption = (parent, model) => {
            ...
            if (metadata?.family) {
                option.dataset.rusttavernBedrockFamily = metadata.family;
            }
            parent.append(option);
        };
...
function enforceIosPolicyChatCompletionSourceSelector() {
    ...
    if (!(select instanceof HTMLSelectElement)) {
        throw new Error('[RustTavern][iOSPolicy] Chat completion source selector not found');
    }
    ...
    if (!fallback) {
        throw new Error('[RustTavern][iOSPolicy] No allowed chat completion source remains after applying policy');
    }
...
async function onPresetImportFileChange(e) {
    ...
    try {
        const { maybePromptForPresetEmbeddedProfiles } = await import('./app/agent-profiles/embedded-import.js');
        await maybePromptForPresetEmbeddedProfiles({ apiId: 'openai', name: data.name, preset: presetBody });
        const { maybePromptForPresetEmbeddedSkills } = await import('./app/agent-skills/embedded-import.js');
...
async function onDeletePresetClick() {
    ...
    if (!response.ok) {
        ...
        const { clearPresetSkillImportReminders } = await import('./app/agent-skills/reminders.js');
