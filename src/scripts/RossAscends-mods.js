...
export function dragElement($elmnt) {
    ...
    function dragMouseDown(e) {
        ...
        // Unbind before rebinding so a missed mouseup (drag leaving the
        // window, interrupted gesture) can't stack duplicate listeners.
        $(document).off('mouseup', closeDragElement).on('mouseup', closeDragElement);
        $(document).off('mousemove', elementDrag).on('mousemove', elementDrag);
    }
...
export function initRossMods() {
    ...
    document.addEventListener('swiped-left', function (e) {
        ...
        // jQuery's closest() never returns null: an empty set is the no-match
        // result, so the old `!== null` guard was always true and any diagonal
        // swipe anywhere in the chat triggered the last message's swipe.
        if (SwipeTargetMesClassParent.length > 0) {
    ...
    document.addEventListener('swiped-right', function (e) {
        ...
        var SwipeTargetMesClassParent = $(e.target).closest('.last_mes');
        // See the swiped-left handler: closest() never returns null.
        if (SwipeTargetMesClassParent.length > 0) {
