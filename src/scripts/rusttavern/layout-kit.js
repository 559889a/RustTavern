...
export function getHostAbi() {
    return getHostWindow()?.__RUSTTAVERN__ ?? null;
}
...
export function requireLayoutApi() {
    ...
    if (!api) {
        throw new Error('[RustTavern] Host layout API is unavailable.');
    }
    return api;
}
...
export async function waitForHostReady() {
    ...
    const readyPromise = hostWindow?.__RUSTTAVERN__?.ready ?? hostWindow?.__RUSTTAVERN_MAIN_READY__;
    if (readyPromise) {
        await readyPromise;
    }
}
