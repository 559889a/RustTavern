...
const AGENT_RUN_STATE_CHANGED = 'rusttavern-agent-run-state-changed';
const AGENT_RUN_EVENT = 'rusttavern-agent-run-event';
...
function requireAgentApi() {
    ...
    if (!agent) {
        throw new Error('RustTavern Agent API is unavailable');
    }
    return agent;
}
