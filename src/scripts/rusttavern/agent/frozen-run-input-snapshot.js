...
function requireCurrentModelConnectionApi() {
    const api = globalThis.window?.__RUSTTAVERN__?.api?.agent?.promptAssembly;
    if (
        typeof api?.buildCurrentModelConnectionSnapshot !== 'function'
        || typeof api?.applyCurrentModelConnectionSnapshot !== 'function'
    ) {
        throw new Error('agent.current_model_connection_api_unavailable: RustTavern Agent prompt assembly API is unavailable');
    }
    return api;
}
