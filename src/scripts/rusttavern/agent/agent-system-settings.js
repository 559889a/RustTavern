...
export const AGENT_SYSTEM_SETTINGS_CHANGED = 'rusttavern-agent-system-settings-changed';
...
function requireExtensionStore() {
    const store = window.__RUSTTAVERN__?.api?.extension?.store;
    if (!store) {
        throw new Error('RustTavern extension store API is unavailable');
    }
    return store;
}
...
export async function loadAgentSystemSettings() {
    ...
    if (typeof store.tryGetJson !== 'function') {
        throw new Error('RustTavern extension store tryGetJson API is unavailable');
    }
    ...
    if (typeof result?.found !== 'boolean') {
        throw new Error('RustTavern extension store tryGetJson returned an invalid response');
    }
