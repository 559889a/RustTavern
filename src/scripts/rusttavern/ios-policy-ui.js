...
function requireElement(selector, context = document) {
    ...
    if (!element) {
        throw new Error(`[RustTavern][iOSPolicy] Expected element not found: ${selector}`);
    }
    return element;
}
...
function hideElement(element) {
    if (!(element instanceof HTMLElement)) {
        throw new Error('[RustTavern][iOSPolicy] hideElement: element must be an HTMLElement');
    }
...
function filterChatCompletionSourceOptions({ allowlist, endpointOverridesAllowed }) {
    ...
    if (!(select instanceof HTMLSelectElement)) {
        throw new Error('[RustTavern][iOSPolicy] #chat_completion_source must be a <select>');
    }
    ...
    if (select.options.length === 0) {
        throw new Error('[RustTavern][iOSPolicy] Chat completion source list became empty after applying iOS policy');
    }
...
export function applyIosPolicyUiProjection() {
    ...
    if (!(document.body instanceof HTMLBodyElement)) {
        throw new Error('[RustTavern][iOSPolicy] document.body is unavailable');
    }

    document.body.dataset.ttIosPolicyScope = 'ios';
    if (typeof report.profile !== 'string' || !report.profile.trim()) {
        throw new Error('[RustTavern][iOSPolicy] Active policy profile is missing');
    }
    ...
    if (caps.prompts?.nsfw_prompt === false) {
        ...
        if (!(container instanceof HTMLElement)) {
            throw new Error('[RustTavern][iOSPolicy] NSFW prompt container not found');
        }
    ...
    if (caps.prompts?.jailbreak_prompt === false) {
        ...
        const container = textarea.closest('.range-block');
        if (!(container instanceof HTMLElement)) {
            throw new Error('[RustTavern][iOSPolicy] Jailbreak prompt container not found');
        }
        hideElement(container);
        ...
        if (!(label instanceof HTMLElement)) {
            throw new Error('[RustTavern][iOSPolicy] Jailbreak preference label not found');
        }
        hideElement(label);
    }
    ...
    if (!endpointOverridesAllowed) {
        ...
        if (!(reverseProxyDrawer instanceof HTMLElement)) {
            throw new Error('[RustTavern][iOSPolicy] Reverse proxy drawer not found');
        }
    ...
    if (caps.llm?.chat_completion_features?.web_search === false) {
        ...
        if (!(container instanceof HTMLElement)) {
            throw new Error('[RustTavern][iOSPolicy] Web search settings container not found');
        }
        hideElement(container);
    }
