...
import { getAgentGenerationOptions } from './rusttavern/agent/agent-generation-router.js';
import { agentErrorMessage } from './rusttavern/agent/agent-error-presenter.js';
...
export function initDefaultSlashCommands() {
    ...
    async function runRustTavernPanelSlashCommand(openPanel, reportError) {
        try {
            await openPanel();
        } catch (error) {
    ...
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        ...
        aliases: ['apilog'],
        callback: async () => {
            if (!window.__RUSTTAVERN__?.api?.dev) {
                toastr.error(t`LLM API logs are only available in RustTavern.`);
                return '';
            }

            return runRustTavernPanelSlashCommand(async () => {
                const { openLlmApiLogsPanel } = await import('./app/setting/dev-logs.js');
                await openLlmApiLogsPanel();
            }, (message) => toastr.error(t`Failed to open LLM API logs: ${message}`));
        },
        helpString: t`Open the LLM API log viewer (RustTavern only).`,
    }));
    ...
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        ...
        callback: async () => {
            if (!window.__RUSTTAVERN__?.api?.dev) {
                toastr.error(t`Frontend logs are only available in RustTavern.`);
                return '';
            }

            return runRustTavernPanelSlashCommand(async () => {
                const { openFrontendLogsPanel } = await import('./app/setting/dev-logs.js');
                await openFrontendLogsPanel();
            }, (message) => toastr.error(t`Failed to open frontend logs: ${message}`));
        },
        helpString: t`Open the frontend log viewer (RustTavern only).`,
    }));
    ...
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        ...
        callback: async () => {
            if (!window.__RUSTTAVERN__?.api?.dev) {
                toastr.error(t`Backend logs are only available in RustTavern.`);
                return '';
            }

            return runRustTavernPanelSlashCommand(async () => {
                const { openBackendLogsPanel } = await import('./app/setting/dev-logs.js');
                await openBackendLogsPanel();
            }, (message) => toastr.error(t`Failed to open backend logs: ${message}`));
        },
        helpString: t`Open the backend log viewer (RustTavern only).`,
    }));
    ...
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        ...
        aliases: ['lansync'],
        callback: async () => {
            if (typeof window.__RUSTTAVERN__?.invoke?.safeInvoke !== 'function') {
                toastr.error(t`LAN Sync panel is only available in RustTavern.`);
                return '';
            }
            ...
            return runRustTavernPanelSlashCommand(async () => {
                const { openSyncPopup } = await import('./app/setting/setting-panel/sync-popup.js');
                await openSyncPopup();
            }, (message) => toastr.error(t`Failed to open LAN Sync panel: ${message}`));
        },
        helpString: t`Open the LAN Sync panel (RustTavern only).`,
    }));
