...
/** Upper bound for stored token-count entries per chat bucket; beyond this
 * only the most recently inserted half is kept. */
const TOKEN_CACHE_MAX_ENTRIES_PER_CHAT = 2000;
/** Upper bound for the number of stored per-chat buckets. */
const TOKEN_CACHE_MAX_BUCKETS = 100;
...
async function flushTokenCacheState(state) {
    ...
    const cacheObject = state.cache;
    const keys = Object.keys(cacheObject);
    if (keys.length > TOKEN_CACHE_MAX_ENTRIES_PER_CHAT) {
        // Keep only the most recently inserted half: a very long chat can't
        // grow its stored cache without bound; dropped entries simply
        // re-tokenize on demand.
        const keep = new Set(keys.slice(keys.length - TOKEN_CACHE_MAX_ENTRIES_PER_CHAT / 2));
    ...
    await objectStore.setItem(getTokenCacheStorageKey(state.chatId), cacheObject);
    state.dirty = false;

    // Bound the number of per-chat buckets: when the store grows past the
    // cap, drop arbitrary other buckets (never the current one). Chat ids
    // carry no recency ordering, so any victim is fine for bounding purposes.
    try {
        const allKeys = await objectStore.keys();
        const bucketKeys = allKeys.filter(key => key.startsWith('tokenCache:'));
        const overflow = bucketKeys.length - TOKEN_CACHE_MAX_BUCKETS;
        if (overflow > 0) {
            const current = getTokenCacheStorageKey(state.chatId);
            const victims = bucketKeys.filter(key => key !== current).slice(0, overflow);
            await Promise.all(victims.map(key => objectStore.removeItem(key)));
        }
    } catch (err) {
        console.warn('Token cache bucket pruning failed:', err);
    }
}
...
function callTokenizer(type, str) {
    // No remote count endpoint exists in the native server; estimate locally.
...
function callTokenizerAsync(type, str) {
    // No remote count endpoint exists in the native server; estimate locally.
    return Promise.resolve(guesstimate(str));
}
...
export async function getTokenPrefixCountsAsync(base, suffixes, padding = undefined, stopAt = undefined) {
    ...
    if (main_api === 'openai' && padding !== power_user.token_padding && globalThis.__RUSTTAVERN__) {
...
export async function warmTokenizerCache() {
    ...
    try {
        ...
        console.warn('RustTavern tokenizer warm-up failed:', error);
    }
}
...
/**
 * Builds the jQuery `error` handler for a tokenizer request.
 *
 * Every tokenizer helper below drives a promise through the `resolve` callback
 * that jQuery only invokes on success. Without an error handler a failed
 * request (unrouted endpoint, offline backend, malformed response) leaves that
 * promise pending forever, and every `await getTokenCountAsync(...)` in the
 * generation path hangs with it. Resolve with the documented fallback instead.
 *
 * `createFallback` stays lazy: `apiFailureTokenCount` warns the user and may
 * switch the active tokenizer, so it must not run on the success path.
 *
 * @param {string} endpoint Endpoint that failed, for the log line.
 * @param {function} [resolve] Promise resolve function.
 * @param {() => any} createFallback Produces the value to resolve with.
 * @returns {function} jQuery error handler.
 */
function tokenizerRequestFailureHandler(endpoint, resolve, createFallback) {
    return function (xhr, status, errorThrown) {
        console.warn(`Tokenizer request to ${endpoint} failed (${xhr?.status ?? status}):`, errorThrown || status);
...
function getTextTokensFromServer(endpoint, str, resolve) {
    ...
    jQuery.ajax({
        ...
        error: tokenizerRequestFailureHandler(endpoint, resolve, () => []),
    });
    return ids;
}
...
function getTextTokensFromTextgenAPI(str, resolve) {
    ...
    jQuery.ajax({
        ...
        error: tokenizerRequestFailureHandler(
...
function decodeTextTokensFromServer(endpoint, ids, resolve) {
    ...
    jQuery.ajax({
        ...
        error: tokenizerRequestFailureHandler(endpoint, resolve, () => ({ text: '', chunks: [] })),
    });
    return { text, chunks };
}
