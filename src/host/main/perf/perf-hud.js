const GLOBAL_KEY = '__RUSTTAVERN_PERF__';
const EMBEDDED_RUNTIME_KEY = '__RUSTTAVERN_EMBEDDED_RUNTIME__';
const PANEL_RUNTIME_KEY = '__RUSTTAVERN_PANEL_RUNTIME__';
...
function readAutoEnableFlag() {
    // sessionStorage, not localStorage: while this flag lives the HUD keeps a
    // per-frame rAF sampler, a 100ms lag timer and a 750ms DOM rebuild running.
    // Persisted in localStorage, one debugging session left all three running
    // on the phone forever, long after anyone was looking at them.
    try {
        if (globalThis.sessionStorage?.getItem(STORAGE_ENABLED_KEY) === '1') {
            return true;
        }
    } catch {
...
function persistEnabledFlag(enabled) {
    try {
        globalThis.sessionStorage?.setItem(STORAGE_ENABLED_KEY, enabled ? '1' : '0');
    } catch {
...
function createPerfController(options = {}) {
    ...
    function renderHud() {
        ...
        const importTauriMain = readMeasureDuration('tt:init:import:host-main');
        ...
        if (state.expanded) {
            ...
            lines.push(`Import host-main: ${importTauriMain === null ? '-' : formatMs(importTauriMain)}`);
            ...
            lines.push('Commands: __RUSTTAVERN_PERF__.exportJson() / downloadReport() / copyReport() / saveReport()');
            lines.push('DOM report: __RUSTTAVERN_PERF__.domTop()');
    ...
    function snapshot() {
        ...
        return {
            ...
            init: {
                ...
                importTauriMainMs: readMeasureDuration('tt:init:import:host-main'),
    ...
    function printReport(options = {}) {
        const payload = report(options);
        console.log('[RustTavern Perf Report]', payload);
        return payload;
    }
    ...
    function downloadReport(options = {}) {
        ...
        const filename = String(options.filename || `rusttavern-perf-${suffix}.json`);
