...
export function createChatDomAdapter({
    ...
    function setMessageId(element, messageId) {
        // Commits re-assert every entry, but only the appended/renumbered ones can
        // differ. A redundant `setAttribute` still queues a MutationObserver record
        // and invalidates style, and writing the same `textContent` replaces the
        // text node outright -- both are O(N) work per appended message otherwise.
        const value = String(messageId);
        if (element.getAttribute('mesid') === value) {
            return;
        }
        element.setAttribute('mesid', value);
