    ...
    const invoke = async (command, args = {}) => {
        ...
        if (policy.kind === 'dedupe') {
            ...
            const promise = task()
                ...
                .finally(() => {
                    // Conditional: `invalidate` may have already dropped this
                    // entry and a successor request re-registered the same key.
                    // Deleting unconditionally would orphan that successor's
                    // in-flight entry and let the next caller duplicate it.
