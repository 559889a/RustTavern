...
export function installDynamicTheme() {
    const ready = getRustTavernSettings().then((settings) => {
        ...
        eventSource.on(event_types.APP_READY, () => {
            ...
            // Registered before the server-mode early return below: this resync is
            // the only catch-up for a theme change that arrived while the tab was
            // hidden, and server mode needs it as much as the native shell does.
