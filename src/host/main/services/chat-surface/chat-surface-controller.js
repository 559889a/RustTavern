...
// Residency assertions walk every mounted message per commit (O(N) DOM reads).
// They are self-checks for ChatSurface development, not part of the runtime
// contract: the DOM adapter's mutation guard still rejects unauthorized root
// mutations in production. Enable with localStorage 'tt:chatSurfaceAssert' = '1'
// (tests set this per fixture; see tests/chat-surface-controller.test.mjs).
const RESIDENCY_ASSERT_STORAGE_KEY = 'tt:chatSurfaceAssert';

/**
 * Reads the residency self-check flag once per controller instance (not per
 * commit): module import happens before a page's localStorage exists in some
 * hosts, and per-controller reads keep test fixtures independent.
 * @returns {boolean}
 */
function readResidencyAssertFlag() {
    try {
        return globalThis.localStorage?.getItem(RESIDENCY_ASSERT_STORAGE_KEY) === '1';
    } catch {
        return false;
    }
}
...
export function createChatSurfaceController({
    ...
    const residencyAssertsEnabled = readResidencyAssertFlag();

    /**
     * Runs the residency self-checks only when this controller instance
     * enabled them (dev flag or tests).
     * @param {() => void} assertion
     */
    function runResidencyAssert(assertion) {
        if (residencyAssertsEnabled) {
            assertion();
        }
    }
    /** @type {string | null} */
    ...
    function commitProjection({
        ...
        if (sameProjection) {
            try {
                runResidencyAssert(() => assertCommittedResidencies(residencies, nextProjection, domAdapter));
            } catch (error) {
        ...
        try {
            ...
            // Single post-commit self-check (was two identical walks, before and
            // after participant notification): this is the final state, so it also
            // catches divergence introduced by the participant hooks above.
            runResidencyAssert(() => assertCommittedResidencies(residencies, projection, domAdapter));
            assertHealthy();
        } catch (error) {
            throw setFault(error);
        }
