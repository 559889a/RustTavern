...
export function createUploadService({ safeInvoke, invoke } = {}) {
    ...
    async function invokeHostUploadChunk(filePath, offset, chunk) {
        ...
        const invokeApi = typeof invoke === 'function' ? invoke : nativeInvoke;
        if (typeof invokeApi !== 'function') {
            throw new Error('Tauri invoke API is unavailable');
        }

        // Server mode (and the Android Tauri base64 path): the chunk travels
        // as a JSON `data` field. The legacy raw-bytes+headers form only
        // exists inside the desktop Tauri WebView.
        if (isAndroidRuntime() || typeof nativeInvoke !== 'function') {
            return invokeApi('stage_upload_chunk', {
                file_path: filePath,
                offset,
