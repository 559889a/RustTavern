...
import { DEFAULT_AGENT_PROFILE_ID } from '../../../scripts/rusttavern/agent/agent-system-settings.js';
import { ensureModelTargetLlmConnectionForProfile } from '../../../scripts/rusttavern/agent/model-target-llm-connection.js';
...
async function normalizePromptSnapshotRunInput(input, { safeInvoke, ensureModelTargetConnection = true, runProfile = null }) {
    ...
    const chatRef = input.chatRef || window.__RUSTTAVERN__?.api?.chat?.current?.ref?.();
