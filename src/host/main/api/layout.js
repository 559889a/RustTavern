...
function requireRoot() {
    ...
    if (!(root instanceof HTMLElement)) {
        throw new Error('[RustTavern] documentElement is unavailable while reading layout snapshot.');
    }
    return root;
}
...
function resolveActiveImeSurface() {
    ...
    if (!(active instanceof Element)) {
        throw new Error('[RustTavern] IME active surface is not an Element.');
    }

    const kind = String(active.getAttribute(IME_ATTR.Surface) || '').trim();
    if (!kind) {
        throw new Error('[RustTavern] IME active surface is missing data-tt-ime-surface.');
    }

    if (![IME_KIND.Composer, IME_KIND.FixedShell, IME_KIND.Dialog].includes(kind)) {
        throw new Error(`[RustTavern] Unsupported IME surface kind: ${kind}`);
    }
...
function createLayoutEmitter() {
    ...
    const flush = () => {
        ...
        for (const handler of subscribers) {
            try {
                ...
                console.error('[RustTavern] layout subscriber failed.', error);
            }
        }
    };
    ...
    const start = () => {
        ...
        if (typeof MutationObserver !== 'function') {
            throw new Error('[RustTavern] MutationObserver is unavailable while subscribing layout.');
        }
