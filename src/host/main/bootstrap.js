import { invoke, isTauri as isTauriRuntime } from '../../host-bridge.js';
...
import { initializeTauriIntegration } from './bootstrap/initialize-host-integration.js';
import {
    getMethod,
    getMethodHint,
    jsonResponse,
...
function isPerfHudEnabled() {
    try {
        const flag = globalThis.__RUSTTAVERN_PERF_ENABLED__;
...
function installHostAbi(context) {
    window.__RUSTTAVERN__ = {
        ...
        assets: {
            thumbnailUrl: window.__RUSTTAVERN_THUMBNAIL__,
            backgroundPath: window.__RUSTTAVERN_BACKGROUND_PATH__,
        },
    };
}
...
export function bootstrapTauriMain() {
    ...
    if (perfEnabled) {
        perfReadyPromise = import('./perf/perf-hud.js')
            ...
            .catch((error) => {
                console.warn('RustTavern: Failed to load perf HUD:', error);
                return null;
            });
        window.__RUSTTAVERN_PERF_READY__ = perfReadyPromise;
    }
    ...
    const routeRequest = async (url, input, init, _targetWindow) => {
        ...
        try {
            ...
            finalResponse.headers.set(DEFAULT_TRACE_HEADER, traceId);
            const durationMs = (globalThis.performance?.now?.() ?? Date.now()) - startedAt;
            console.error('RustTavern route handler failed', {
                traceId,
                method,
                path: url.pathname,
                durationMs,
    ...
    if (window.__RUSTTAVERN__) {
        window.__RUSTTAVERN__.ready = readyPromise;
    }

    runAfterTauriReady(() => setFrontendLogBackendForwardingEnabled(true));

    runAfterTauriReady(() => import('../../scripts/app/setting/setting-panel.js')
        .then(({ installRustTavernSettingsPanel }) => installRustTavernSettingsPanel())
        .catch((error) => { console.warn('RustTavern: Failed to load settings panels:', error); }));
    runAfterTauriReady(() => import('../../scripts/app/regex/native-regex-settings.js')
        .then(({ installNativeRegexBackendSetting }) => installNativeRegexBackendSetting())
        .catch((error) => { console.warn('RustTavern: Failed to load native regex settings:', error); }));
    runAfterTauriReady(() => import('./services/dynamic-theme/install.js')
        .then(({ installDynamicTheme }) => installDynamicTheme())
        .catch((error) => { console.warn('RustTavern: Failed to load dynamic theme:', error); }));
    if (!isEmbeddedRuntimeTakeoverDisabled()) {
        runAfterTauriReady(() => import('./services/embedded-runtime/install.js')
            ...
            .catch((error) => { console.warn('RustTavern: Failed to load embedded runtime:', error); }));
    }
    runAfterTauriReady(() => import('./services/panel-runtime/install.js')
        .then(({ installPanelRuntime }) => installPanelRuntime())
        .catch((error) => { console.warn('RustTavern: Failed to load panel runtime:', error); }));
