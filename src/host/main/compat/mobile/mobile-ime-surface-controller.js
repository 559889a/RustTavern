...
/**
 * The native Android insets bridge existed only in the removed Tauri mobile
 * app shell. In the B/S form the bridge is intentionally absent; the IME
 * layout is handled by `interactive-widget=resizes-content` plus the
 * `--doc-height` chain. Surface marking below still runs, but routing to a
 * bridge must be skipped instead of throwing inside focus listeners.
 */
function getInsetsBridgeIfAvailable() {
    const bridge = window.__RUSTTAVERN_INSETS__;
    return bridge && typeof bridge.setImeTarget === 'function' ? bridge : null;
}

function applyImeTarget(target) {
    const bridge = getInsetsBridgeIfAvailable();
    if (bridge) {
        bridge.setImeTarget(target);
    }
}
...
function requireShed() {
    ...
    if (!(sheld instanceof HTMLElement)) {
        throw new Error('[RustTavern] #sheld unavailable while routing IME target.');
    }
    return sheld;
}
