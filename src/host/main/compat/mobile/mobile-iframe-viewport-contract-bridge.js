const CONTROLLER_KEY = '__RUSTTAVERN_MOBILE_IFRAME_VIEWPORT_CONTRACT_BRIDGE__';
...
function requireRoot() {
    ...
    if (!(root instanceof HTMLElement)) {
        throw new Error('[RustTavern] documentElement unavailable while bridging iframe viewport contract.');
    }
    return root;
}
...
export function installMobileIframeViewportContractBridge() {
    ...
    if (typeof MutationObserver !== 'function') {
        throw new Error('[RustTavern] MutationObserver unavailable while installing iframe viewport contract bridge.');
    }
