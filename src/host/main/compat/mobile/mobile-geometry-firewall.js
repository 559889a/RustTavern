...
const FIREWALL_CSS = `
/* [RustTavern] Mobile geometry firewall (host-last) */
...
@media screen and (min-width: 1001px) {
  ...
  body dialog.popup.tt-rusttavern-panel-popup[data-tt-mobile-surface="fullscreen-window"][data-tt-mobile-surface][data-tt-mobile-surface] {
...
function requireHead() {
    ...
    if (!(head instanceof HTMLHeadElement)) {
        throw new Error('[RustTavern] document.head unavailable while installing mobile geometry firewall.');
    }
    return head;
}
...
function requireStyleElement() {
    ...
    if (!(existing instanceof HTMLStyleElement)) {
        throw new Error(`[RustTavern] #${STYLE_ID} is not a <style> element.`);
    }
...
export function installMobileGeometryFirewall() {
    if (typeof MutationObserver !== 'function') {
        throw new Error('[RustTavern] MutationObserver unavailable while installing mobile geometry firewall.');
    }
