...
const RUNTIME_SENTINEL = '__RUSTTAVERN_DIALOG_POLYFILL_COVERAGE_RUNTIME__';
const BRIDGE_SENTINEL = '__RUSTTAVERN_DIALOG_POLYFILL_COVERAGE_BRIDGE__';
...
export function installDialogPolyfillCoverage(targetWindow = window) {
    ...
    if (targetWindow === window) {
        void installDialogPolyfillCoverageRuntime().catch((error) => {
            console.error('RustTavern: Failed to install dialog polyfill coverage:', error);
        });
        return;
    }
...
function ensureDialogPolyfillCssLoaded() {
    ...
    link.addEventListener('error', (event) => {
        console.error('RustTavern: Failed to load dialog polyfill stylesheet', { href: cssHref, event });
    });
