...
import { createChannel } from '../../../host-bridge.js';
...
const DEFAULT_COMPLETION_MODEL = 'rusttavern-error';
...
const ANDROID_GENERATION_BRIDGE_NAME = 'RustTavernAndroidAiBridge';
...
const CAPTION_UNAVAILABLE_MESSAGE = 'Image captioning is not implemented in the RustTavern native backend.';
const GENERATION_UNAVAILABLE_ROUTES = Object.freeze([
    '/api/backends/text-completions/generate',
    '/api/backends/kobold/generate',
    '/api/backends/koboldhorde/generate',
    '/api/novelai/generate',
]);
const GENERATION_UNAVAILABLE_MESSAGE = 'This text-completion backend is not implemented in the RustTavern native server yet. Use a Chat Completion API instead.';
const i18nNotificationKeys = Object.freeze({
    successTitle: 'rusttavern_ai_notification_success_title',
    successBody: 'rusttavern_ai_notification_success_body',
    failureTitle: 'rusttavern_ai_notification_failure_title',
    failureBody: 'rusttavern_ai_notification_failure_body',
});
const i18nNotificationFallbacks = Object.freeze({
    successTitle: 'AI reply is ready',
    successBody: 'Tap to return to RustTavern',
    failureTitle: 'AI reply failed',
    failureBody: 'Generation failed. Tap to return to RustTavern',
});
...
function buildOpenAiStyleErrorChunk(error, payload) {
    const timestamp = Math.floor(Date.now() / 1000);
    return {
        id: `rusttavern-error-chunk-${timestamp}`,
...
async function createChatCompletionStreamResponse(context, payload, signal, lifecycle) {
    // The stream id comes from `createChannel`: in server mode the channel
    // subscribes to `/__tt/stream/{id}` (SSE) BEFORE the command is invoked,
    // so the id must be the one the channel subscribed to. Cancellation only
    // runs after the channel exists (see cancelAfterStart below).
    let streamId = null;
    const encoder = new TextEncoder();
    ...
    const closeStream = async ({
        ...
        if (channel) {
            channel.onmessage = () => {};
            // Abort the SSE read loop so the connection and its closures are
            // released immediately instead of lingering until the server
            // finishes the stream.
...
export function registerAiRoutes(router, context, { jsonResponse }) {
    ...
    // Text-completion style generation endpoints have no Rust backend yet.
    // Without an explicit route these fall through to the SPA static handler,
    // which answers 200 text/html and makes response.json() fail with a
    // confusing SyntaxError. Fail fast with a JSON body the generate error
    // path (throw await response.json()) can surface.
    for (const route of GENERATION_UNAVAILABLE_ROUTES) {
        router.post(route, async () => jsonResponse({
            error: true,
