...
test('Stable Diffusion backend request uses explicit credentials instead of provider-specific loose fields', async () => {
    const repositorySource = await readFile(
        path.join(REPO_ROOT, 'crates', 'tt-ports', 'src', 'repositories', 'stable_diffusion_repository.rs'),
        'utf8',
    );
    const serviceSource = await readFile(
        path.join(REPO_ROOT, 'crates', 'tt-application', 'src', 'services', 'stable_diffusion_service.rs'),
        'utf8',
    );
