...
test('RustTavern keeps SETTINGS_LOADED in auto-fire events', async () => {
    const source = await readFile(path.join(REPO_ROOT, 'src/scripts/events.js'), 'utf8');
...
test('chat rename events use backend-committed normalized file names', async () => {
    const [script, welcomeScreen, route, chatCommand, groupCommand, chatService, groupService] = await Promise.all([
        readFile(path.join(REPO_ROOT, 'src/script.js'), 'utf8'),
        readFile(path.join(REPO_ROOT, 'src/scripts/welcome-screen.js'), 'utf8'),
        readFile(path.join(REPO_ROOT, 'src/host/main/routes/chat-routes.js'), 'utf8'),
        readFile(path.join(REPO_ROOT, 'crates/rusttavern/src/presentation/commands/chat_commands.rs'), 'utf8'),
        readFile(path.join(REPO_ROOT, 'crates/rusttavern/src/presentation/commands/group_chat_commands.rs'), 'utf8'),
        readFile(path.join(REPO_ROOT, 'crates/tt-application/src/services/chat_service.rs'), 'utf8'),
        readFile(path.join(REPO_ROOT, 'crates/tt-application/src/services/group_chat_service.rs'), 'utf8'),
    ]);
