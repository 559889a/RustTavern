...
test('dynamic appearance runtime applies SillyTavern appearance through the ST adapter', async () => {
    const runtime = await readRepoFile('src/host/main/services/dynamic-theme/install.js');
    const adapter = await readRepoFile('src/host/main/adapters/st/appearance.js');
