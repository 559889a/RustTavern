...
test('core message roots reconcile through ChatSurface instead of direct DOM mutation', async () => {
    ...
    const bounded = await readFile(
        path.join(REPO_ROOT, 'src/host/main/services/chat-surface/bounded-chat-surface.js'),
        'utf8',
    );
...
test('projection lifecycle does not import or emit SillyTavern business events', async () => {
    const controller = await readFile(
        path.join(REPO_ROOT, 'src/host/main/services/chat-surface/chat-surface-controller.js'),
        'utf8',
    );
    const lifecycle = await readFile(
        path.join(REPO_ROOT, 'src/host/main/services/chat-surface/participant-lifecycle.js'),
        'utf8',
    );
