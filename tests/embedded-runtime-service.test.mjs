...
test('EmbeddedRuntimeService fails fast when #chat root is missing', async () => {
    ...
    try {
        const { createEmbeddedRuntimeService } = await importFresh(
            path.join(REPO_ROOT, 'src/host/main/services/embedded-runtime/embedded-runtime-service.js'),
        );
...
test('EmbeddedRuntimeService exposes profile+manager and publishes a global handle', async () => {
    ...
    try {
        ...
        assert.equal(globalThis.__RUSTTAVERN_EMBEDDED_RUNTIME__, service.manager);
    } finally {
        delete globalThis.__RUSTTAVERN_EMBEDDED_RUNTIME__;
        dom.cleanup();
    }
});
