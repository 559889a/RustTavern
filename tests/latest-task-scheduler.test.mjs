...
test('PromptManager keeps dry-run rendering wired through the latest-task scheduler', async () => {
    ...
    const dryRunStart = source.indexOf('    async #renderAfterTryGenerate(updateUi');
    ...
    assert.match(
        source,
        /this\.renderRowStateLatest = createLatestTaskScheduler\(\s*\(\) => this\.#renderAfterTryGenerate\(\(\) => this\.#refreshPromptListState\(\)\)/,
    );
    ...
    const renderUi = dryRunSource.indexOf('this.#renderPromptManagerUi()', rethrowError);
    assert.ok(clearError >= 0 && clearError < tryGenerate);
    assert.ok(tryGenerate < exposeError && exposeError < rethrowError);
    // The error banner only exists in the full render, so a failed dry run must
    // fall back to it even on the row-refresh path.
    assert.ok(rethrowError < renderUi);
    assert.match(dryRunSource, /this\.error \? this\.#renderPromptManagerUi\(\) : updateUi\(\)/);
