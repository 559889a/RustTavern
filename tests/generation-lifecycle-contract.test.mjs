...
test('stable chat-history reasons cover provider, final, transport, and agent commits', async () => {
    const [script, groups, transport, commit, routes, agentBridge] = await Promise.all([
        ...
        readFile(path.join(REPO_ROOT, 'src/scripts/app/chat/transport.js'), 'utf8'),
        readFile(path.join(REPO_ROOT, 'src/scripts/app/chat/commit.js'), 'utf8'),
        readFile(path.join(REPO_ROOT, 'src/host/main/routes/chat-routes.js'), 'utf8'),
        readFile(path.join(REPO_ROOT, 'src/host/main/api/agent-chat-commit-bridge.js'), 'utf8'),
    ]);
