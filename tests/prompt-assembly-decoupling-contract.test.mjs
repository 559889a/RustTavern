...
test('connectionRef prompt assembly overlays model binding instead of validating preset source', async () => {
    const source = await readProjectFile('crates/tt-application/src/services/prompt_assembly_service.rs');
...
test('currentPromptSnapshot prompt assembly overlays frozen current model connection', async () => {
    const source = await readProjectFile('crates/tt-application/src/services/prompt_assembly_service.rs');
    const frontendSource = await readProjectFile('src/scripts/rusttavern/agent/frozen-run-input-snapshot.js');
    const agentApiSource = await readProjectFile('src/host/main/api/agent-prompt-assembly-run.js');
...
test('frontend prompt assembly normalizes effective settings without resolving model from defaults', async () => {
    const [brokerSource, openaiSource] = await Promise.all([
        readProjectFile('src/host/main/api/agent-prompt-assembly.js'),
