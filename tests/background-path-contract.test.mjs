...
test('Tauri resource helpers expose only stable Host Resource URLs', async () => {
    ...
    const { installAssetPathHelpers } = await importFresh(
        path.join(REPO_ROOT, 'src/host/main/context/asset-path-helpers.js'),
    );
    ...
    const backgroundPathFn = windowMock.__RUSTTAVERN_BACKGROUND_PATH__;
    const thumbnailPathFn = windowMock.__RUSTTAVERN_THUMBNAIL__;
    ...
    assert.equal(windowMock.__RUSTTAVERN_THUMBNAIL_BLOB_URL__, undefined);
    assert.equal(windowMock.__RUSTTAVERN_AVATAR_PATH__, undefined);
    assert.equal(windowMock.__RUSTTAVERN_PERSONA_PATH__, undefined);
});
