...
test('SillyTavern compatibility baseline stays aligned across frontend and backend', async () => {
    ...
    const backendSource = await readText('crates/rusttavern/src/presentation/commands/bridge.rs');
...
test('RustTavern product version stays aligned across release manifests', async () => {
    const packageJson = JSON.parse(await readText('package.json'));
    const appCargoToml = await readText('crates/rusttavern/Cargo.toml');
