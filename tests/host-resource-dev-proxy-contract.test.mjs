...
test('dev service worker proxies the same browser host resources as production', async () => {
    const [sw, init, endpoint, router] = await Promise.all([
        readRepoFile('src/tt-ext-sw.js'),
        readRepoFile('src/init.js'),
        readRepoFile('crates/rusttavern/src/server/host_resources.rs'),
        readRepoFile('crates/rusttavern/src/server/router.rs'),
    ]);
    ...
    assert.match(endpoint, /try_serve/);
    assert.match(endpoint, /HTTP_DELIVERY/);
    assert.match(endpoint, /not_found_response/);
    // The server serves the same host resource routes directly (the old dev
    // proxy dev-server is gone; the axum router owns them in all modes).
    assert.match(router, /serve_host_resource_any/);
    assert.match(router, /\/User Avatars\/\{\*path\}/);
});
