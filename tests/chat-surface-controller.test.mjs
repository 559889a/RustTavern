...
import { createChatDomAdapter } from '../src/host/main/adapters/chat-surface/chat-dom-adapter.js';
import { createChatScrollAdapter } from '../src/host/main/adapters/chat-surface/chat-scroll-adapter.js';
import { createChatSurfaceController } from '../src/host/main/services/chat-surface/chat-surface-controller.js';
import { createChatSurfaceParticipantRegistry } from '../src/host/main/services/chat-surface/participant-registry.js';
...
function createFixture({ messages, registry, guardUnauthorizedMutations = false }) {
    ...
    const faults = [];
    let controller;
    // Enable the residency self-checks for the fixture: the controller reads
    // this flag once per instance, and tests below assert that the checks
    // catch DOM drift (they are off by default in production).
    localStorage.setItem('tt:chatSurfaceAssert', '1');
...
test('residency self-checks are opt-in per controller instance', () => {
    // Flag off (default): a mesid drift that the enabled checks would fault on
    // goes through without error, proving the production path skips the walks.
    const dom = installFakeDom();
    try {
        const messages = [{ mes: 'zero' }];
        const fixture = createFixture({ messages, registry: createChatSurfaceParticipantRegistry() });
        localStorage.removeItem('tt:chatSurfaceAssert');
        const offController = createChatSurfaceController({
            getMessages: () => messages,
            materializeMessage({ message, messageId }) {
                return createMessageElement(messageId, message);
            },
            domAdapter: createChatDomAdapter({ root: fixture.root, onUnauthorizedMutation: () => {} }),
            scrollAdapter: createChatScrollAdapter(fixture.root),
            participantRegistry: createChatSurfaceParticipantRegistry(),
        });
        reconcile(offController, [0]);
        offController.getMessageElement(0).removeAttribute('mesid');
        assert.doesNotThrow(() => reconcile(offController, [0]));
        assert.equal(offController.snapshot().fault, null);
    } finally {
        dom.cleanup();
    }
});
