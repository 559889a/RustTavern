...
import { createRustTavernSettingsState } from '../src/scripts/app/setting/setting-panel/settings-state.js';
import { buildRustTavernSettingsUpdate } from '../src/scripts/app/setting/setting-panel/settings-patch.js';
import { createRustTavernSettingsApp } from '../src/scripts/app/setting/settings-app/SettingsApp.js';
...
test('buildRustTavernSettingsUpdate returns an empty patch for unchanged settings', () => {
    const initial = createRustTavernSettingsState(createSettings(), {
    ...
    const update = buildRustTavernSettingsUpdate(initial, createDraft(initial, {
...
test('buildRustTavernSettingsUpdate persists the chat virtualization switch', () => {
    const initial = createRustTavernSettingsState(createSettings());
    const update = buildRustTavernSettingsUpdate(initial, createDraft(initial, {
...
test('createRustTavernSettingsState requires the canonical chat virtualization switch', () => {
    const settings = createSettings();
    delete settings.chat_virtualization_enabled;

    assert.throws(
        () => createRustTavernSettingsState(settings),
        /chat virtualization setting missing/,
    );
});

test('createRustTavernSettingsState requires the canonical Zstandard backup switch', () => {
    const settings = createSettings();
...
test('buildRustTavernSettingsUpdate preserves minimal nested patch semantics', () => {
    const initial = createRustTavernSettingsState(createSettings(), {
        nativeRegexBackendEnabled: true,
    });
...
test('buildRustTavernSettingsUpdate persists dynamic wallpaper settings with theme settings', () => {
    const initial = createRustTavernSettingsState(createSettings(), {
        nativeRegexBackendEnabled: true,
    });
...
test('buildRustTavernSettingsUpdate persists embedded runtime legacy migration', () => {
    const initial = createRustTavernSettingsState(createSettings({
    ...
    const update = buildRustTavernSettingsUpdate(legacyEffectiveInitial, createDraft(legacyEffectiveInitial, {
        embeddedRuntimeProfile: 'compat',
    }));
...
test('buildRustTavernSettingsUpdate preserves minimal chat backup patch semantics', () => {
    const initial = createRustTavernSettingsState(createSettings(), {
        nativeRegexBackendEnabled: true,
    });
...
test('buildRustTavernSettingsUpdate persists only the Zstandard backup switch', () => {
    const initial = createRustTavernSettingsState(createSettings(), {
        nativeRegexBackendEnabled: true,
    });
...
test('buildRustTavernSettingsUpdate flags the destructive zero limit transition', () => {
    const initial = createRustTavernSettingsState(createSettings(), {
        nativeRegexBackendEnabled: true,
    });
...
test('buildRustTavernSettingsUpdate accepts unlimited chat backup sentinels', () => {
    const initial = createRustTavernSettingsState(createSettings(), {
        nativeRegexBackendEnabled: true,
    });
...
test('buildRustTavernSettingsUpdate rejects invalid chat backup limits', () => {
    const initial = createRustTavernSettingsState(createSettings(), {
        nativeRegexBackendEnabled: true,
    });

    assert.throws(
        () => buildRustTavernSettingsUpdate(initial, createDraft(initial, {
            chatBackups: {
                maxFilesPerPrefix: -2,
            },
        })),
...
test('MiB and GiB chat backup inputs save the same byte limit', () => {
    ...
    const fromMiB = buildRustTavernSettingsUpdate(initial, createDraft(initial, {
        chatBackups: {
            maxTotalValue: 1536,
            maxTotalUnit: 'MiB',
        },
    }));
    const fromGiB = buildRustTavernSettingsUpdate(initial, createDraft(initial, {
        chatBackups: {
            maxTotalValue: 1.5,
            maxTotalUnit: 'GiB',
        },
...
test('chat backup storage unit selector converts the displayed value without changing bytes', () => {
    const values = createRustTavernSettingsState(createSettings(), {
        nativeRegexBackendEnabled: true,
    });
    const component = createRustTavernSettingsApp({
        viewModel: {
            capabilities: {},
            dataRoot: null,
            values,
...
test('zstd backup hint updates when aggregate storage stats arrive later', () => {
    ...
    const values = createRustTavernSettingsState(settings);
    const component = createRustTavernSettingsApp({
        viewModel: {
            capabilities: {},
            dataRoot: null,
