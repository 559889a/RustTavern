...
async function installHarness() {
    ...
    const { installSkillApi } = await import(pathToFileURL(path.join(REPO_ROOT, 'src/host/main/api/skill.js')));
    installSkillApi({
    ...
    return {
        calls,
        skill: globalThis.window.__RUSTTAVERN__.api.skill,
    };
}
...
test('api.skill downloads remote SKILL.md through host command', async () => {
    ...
    const input = await globalThis.window.__RUSTTAVERN__.api.skill.downloadImport({
...
test('api.skill picks import archives through the host dialog', async () => {
    ...
    const input = await globalThis.window.__RUSTTAVERN__.api.skill.pickImportArchive();
