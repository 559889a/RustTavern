...
function createCustomPreviewHarness(source, sourceResponse) {
    ...
    const getPreview = customPreviewFactory(source)(
        ...
        // Sandbox stand-in for the bounded cache helper in backgrounds.js.
        (bg, entry) => memory.set(bg, entry),
    );
...
test('background previews use stable Host Resource representations without a server Blob cache', async () => {
    const [source, settingsSource] = await Promise.all([
        ...
        readProjectFile('src/scripts/app/setting/setting-panel/settings-popup.js'),
    ]);
...
test('persona paths are stable, exactly-once encoded, and re-demanded through the DOM', async () => {
    ...
    assert.doesNotMatch(personaSource, /forFetch|__RUSTTAVERN_PERSONA_PATH__|cache\s*:\s*['"]reload['"]/);
...
test('Host ABI stays v1 after removing internal cache bypasses', async () => {
    const [bootstrap, context, types, upstreamFacade, frontendCommands, rustRegistry] = await Promise.all([
        readProjectFile('src/host/main/bootstrap.js'),
        readProjectFile('src/host/main/context/index.js'),
        readProjectFile('src/types.d.ts'),
        readProjectFile('src/script.js'),
        readProjectFile('src/host/main/kernel/invokes/host-commands.js'),
        readProjectFile('crates/rusttavern/src/presentation/commands/registry.rs'),
    ]);
    ...
    assert.match(
        upstreamFacade,
        /if \(typeof window\.__RUSTTAVERN_THUMBNAIL__ === 'function'\) \{\s*return window\.__RUSTTAVERN_THUMBNAIL__\(type, file, t\);\s*\}/,
    );
