...
import { registerProviderRoutes } from '../src/host/main/routes/provider-routes.js';
import { registerSettingsRoutes } from '../src/host/main/routes/settings-routes.js';
import { registerVectorRoutes } from '../src/host/main/routes/vector-routes.js';
...
test('native vector endpoints fail fast while backend is not implemented', async () => {
    ...
    assert.deepEqual(await response.json(), {
        ...
        message: 'Vector Storage backend is not implemented in the native RustTavern backend yet.',
    });
...
test('top-level native routes register vector fail-fast before provider routes', async () => {
    const source = await readFile(new URL('../src/host/main/routes/index.js', import.meta.url), 'utf8');
