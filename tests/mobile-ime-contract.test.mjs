...
function createFocusHarness({ android = true } = {}) {
    ...
    Object.defineProperty(globalThis, 'navigator', {
        value: android
            ? {
                userAgent: 'Mozilla/5.0 (Linux; Android 14) RustTavern',
                maxTouchPoints: 5,
                platform: 'Android',
            }
            : {
...
test('mobile IME surface controller is focus-driven (no observers) and writes host-private attrs', async () => {
    const controllerPath = path.join(
        REPO_ROOT,
        'src/host/main/compat/mobile/mobile-ime-surface-controller.js',
    );
...
test('fixed-shell IME contract stays geometry-only (no pseudo-element spacer)', async () => {
    const firewallPath = path.join(
        REPO_ROOT,
        'src/host/main/compat/mobile/mobile-geometry-firewall.js',
    );
...
test('focus routing degrades to surface marking when the insets bridge is missing', async () => {
    const dom = createFocusHarness();
    delete dom.windowMock.__RUSTTAVERN_INSETS__;
    ...
    // The native insets bridge existed only in the removed Tauri mobile shell.
    // In the B/S form it is intentionally absent, so a focus event must keep
    // marking the IME surface instead of throwing inside the focus listener.
    dom.emit('focusin', { target: editorTextarea });
    assert.equal(dom.calls.length, 0, 'no bridge routing happens without a bridge');
    assert.equal(characterPopup.getAttribute('data-tt-ime-surface'), 'fixed-shell');
    assert.equal(sheld.getAttribute('data-tt-ime-surface'), null);
