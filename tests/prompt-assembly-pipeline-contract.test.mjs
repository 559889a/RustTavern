...
const RUSTTAVERN_OPENAI_PATH = path.join(REPO_ROOT, 'src', 'scripts', 'openai.js');
...
test('RustTavern OpenAI prompt assembly module parses before app bootstrap', () => {
    const result = spawnSync(process.execPath, ['--check', RUSTTAVERN_OPENAI_PATH], {
        encoding: 'utf8',
    });
...
test('Yuan ultranova preset prompt assembly matches SillyTavern for every prompt order in the preset', async () => {
    ...
    for (const promptOrder of preset.prompt_order) {
        ...
        const rustTavernPlan = buildPromptAssemblyPlan(preset, orderId, 'tauritavern');

        assert.deepEqual(rustTavernPlan, sillyTavernPlan, `prompt order ${orderId} diverged`);
    }
});
