...
function createDomHarness() {
    ...
    const reset = () => {
        ...
        delete windowMock.__RUSTTAVERN_MOBILE_OVERLAY_COMPAT__;
...
test('geometry firewall surface selectors keep high specificity (>= Vue scoped)', async () => {
    const firewallPath = path.join(REPO_ROOT, 'src/host/main/compat/mobile/mobile-geometry-firewall.js');
...
test('geometry firewall keeps wide mobile fullscreen surfaces safe-area pinned', async () => {
    ...
    assert.match(
        source,
        /dialog\.popup\.tt-rusttavern-panel-popup\[data-tt-mobile-surface="fullscreen-window"\][\s\S]*width:\s*min\(980px,\s*var\(--tt-panel-popup-wide-width\)\)\s*!important[\s\S]*height:\s*min\(760px,\s*var\(--tt-panel-popup-wide-height\)\)\s*!important/,
    );
});
...
test('geometry firewall stays last in <head> (keep-last)', async () => {
    ...
    const firewallModulePath = path.join(REPO_ROOT, 'src/host/main/compat/mobile/mobile-geometry-firewall.js');
...
test('bootstrap wires mobile geometry firewall + overlay classifier (no old controller)', async () => {
    const bootstrapPath = path.join(REPO_ROOT, 'src/host/main/bootstrap.js');
...
test('overlay surface classifier is stable across revalidate (fullscreen-window)', async () => {
    ...
    const overlayModulePath = path.join(
        REPO_ROOT,
        'src/host/main/compat/mobile/mobile-overlay-compat-controller.js',
    );
...
test('iframe viewport contract bridge syncs inset vars into same-origin iframe documents', async () => {
    ...
    const bridgeModulePath = path.join(
        REPO_ROOT,
        'src/host/main/compat/mobile/mobile-iframe-viewport-contract-bridge.js',
    );
