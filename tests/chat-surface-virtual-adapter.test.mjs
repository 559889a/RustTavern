...
import { createTanStackVirtualAdapter } from '../src/host/main/adapters/chat-surface/tanstack-virtual-adapter.js';
import {
    CHAT_VIRTUAL_ESTIMATE_PX,
    CHAT_VIRTUAL_MAX_VIEWPORT_ITEMS,
} from '../src/host/main/kernel/chat-surface/virtualization-config.js';
...
test('TanStack private lifecycle is isolated to the pinned adapter', async () => {
    const adapterSource = await readFile(
        path.join(REPO_ROOT, 'src/host/main/adapters/chat-surface/tanstack-virtual-adapter.js'),
        'utf8',
    );
    ...
    const sourceFiles = [
        'src/host/main/services/chat-surface/bounded-chat-surface.js',
        'src/host/main/services/chat-surface/chat-surface-controller.js',
        'src/script.js',
    ];
...
test('composition invalidates global layout changes and offers explicit fault recovery', async () => {
    ...
    const install = await readFile(
        path.join(REPO_ROOT, 'src/host/main/services/chat-surface/install.js'),
        'utf8',
    );
    ...
    assert.match(source, /updateRustTavernSettings\(\{ chat_virtualization_enabled: false \}\)/);
