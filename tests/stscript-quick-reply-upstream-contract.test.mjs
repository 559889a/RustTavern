...
test('RustTavern panel slash commands stay wrapper-based', async () => {
    const source = await readFile(path.join(REPO_ROOT, 'src/scripts/slash-commands.js'), 'utf8');

    const llmLog = extractCommandBlock(source, 'llmlog');
    assert.match(llmLog, /aliases:\s*\[\s*['"]apilog['"]\s*\]/);
    assert.match(llmLog, /import\(['"]\.\/app\/setting\/dev-logs\.js['"]\)/);
    ...
    assert.match(frontendLog, /import\(['"]\.\/app\/setting\/dev-logs\.js['"]\)/);
    assert.match(frontendLog, /\bopenFrontendLogsPanel\(\)/);

    const backendLog = extractCommandBlock(source, 'backendlog');
    assert.match(backendLog, /import\(['"]\.\/app\/setting\/dev-logs\.js['"]\)/);
    ...
    // Host-gated via the __RUSTTAVERN__ ABI (server mode has no window.__TAURI__).
    assert.match(syncPanel, /window\.__RUSTTAVERN__\?\.invoke\?\.safeInvoke/);
    assert.match(syncPanel, /getActiveIosPolicyCapabilities\(\)\?\.sync\?\.lan\s*===\s*false/);
    assert.match(syncPanel, /import\(['"]\.\/app\/setting\/setting-panel\/sync-popup\.js['"]\)/);
    assert.match(syncPanel, /\bopenSyncPopup\(\)/);

    for (const block of [llmLog, frontendLog, backendLog, syncPanel]) {
        assert.doesNotMatch(block, /devlog_|lan_sync_|tt_sync_|dist\/|mountRustTavern/);
    }
});
