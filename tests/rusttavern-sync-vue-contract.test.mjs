...
async function importSyncState() {
    return import(pathToFileURL(path.join(
        REPO_ROOT,
        'src/scripts/app/setting/setting-panel/sync-state.js',
    )).href);
}
...
async function importSyncApp() {
    return import(pathToFileURL(path.join(
        REPO_ROOT,
        'src/scripts/app/setting/sync-app/SyncApp.js',
    )).href);
}
...
async function importSyncJobEvents() {
    return import(pathToFileURL(path.join(
        REPO_ROOT,
        'src/scripts/app/setting/setting-panel/sync-job-events.js',
    )).href);
}
...
test('RustTavern Sync popup wrapper owns host-only capabilities', async () => {
    const source = await readRepoFile('src/scripts/app/setting/setting-panel/sync-popup.js');

    // Host invoke comes from the host-bridge transport (HTTP in server mode).
    assert.match(source, /import \{ invoke \} from '\.\.\/\.\.\/\.\.\/\.\.\/host-bridge\.js'/);
    // The native barcode scanner bridge is gone; QR pairing stays disabled.
    assert.match(source, /scanPairUri: \(\) => Promise\.resolve\(null\)/);
    assert.doesNotMatch(source, /barcodeScanner/);
    assert.match(source, /callGenericPopup/);
    assert.match(source, /sync\.bundle\.js/);
    assert.match(source, /mountRustTavernSyncApp/);
    assert.match(source, /mountRustTavernSyncScopeApp/);
...
test('RustTavern Sync listeners keep event contract while delegating progress UI', async () => {
    const source = await readRepoFile('src/scripts/app/setting/setting-panel/sync-listeners.js');
    ...
    assert.match(source, /mountRustTavernSyncProgressApp/);
    assert.match(source, /window\.location\.reload\(\)/);
    assert.doesNotMatch(source, /from\s+['"]vue(?:\/|['"])/);
});

test('RustTavern Sync job event routing handles origin and status payloads', async () => {
...
test('RustTavern Sync automation status events refresh status only while toasts refresh panel data', async () => {
    const constants = await readRepoFile('src/scripts/app/setting/setting-panel/constants.js');
    const listeners = await readRepoFile('src/scripts/app/setting/setting-panel/sync-listeners.js');
    const popup = await readRepoFile('src/scripts/app/setting/setting-panel/sync-popup.js');
    const entry = await readRepoFile('src/scripts/app/setting/sync-app/index.js');
...
test('RustTavern Sync automation success toasts include next run time', async () => {
    const listeners = await readRepoFile('src/scripts/app/setting/setting-panel/sync-listeners.js');
    const service = await readRepoFile('crates/tt-application/src/services/sync_automation_service.rs');
    const model = await readRepoFile('crates/tt-contracts/src/sync_automation.rs');
...
test('RustTavern Sync automation draft survives background refreshes', async () => {
    const source = await readRepoFile('src/scripts/app/setting/sync-app/SyncApp.js');
...
test('RustTavern Sync automation status shows the newest success-like timestamp', async () => {
    const { createRustTavernSyncApp } = await importSyncApp();
    const noopFunctions = new Proxy({}, { get: () => () => {} });
    const component = createRustTavernSyncApp({
...
test('Rspack exposes a dedicated RustTavern Sync Vue entry', async () => {
    const source = await readRepoFile('rspack.config.js');

    assert.match(source, /sync:\s*['"]\.\/src\/scripts\/app\/setting\/sync-app\/index\.js['"]/);
    assert.match(source, /listJavaScriptFiles\(['"]src\/scripts\/app\/setting\/sync-app['"]\)/);
    assert.match(source, /src\/scripts\/app\/setting\/dist/);
});

test('RustTavern Sync bundled UI does not contain legacy LAN v1 affordances', async () => {
    if (!await repoFileExists('src/scripts/app/setting/dist/sync.bundle.js')) {
        return;
    }

    const source = await readRepoFile('src/scripts/app/setting/dist/sync.bundle.js');
...
test('RustTavern Sync dataset selection migrates legacy localStorage key once', async () => {
...
test('RustTavern Sync dataset selection rejects invalid current key before legacy fallback', async () => {
    const { getSyncDatasetSelection } = await importSyncState();
    const catalog = {
        policyVersion: 1,
        supportedDatasetIds: ['characters', 'chats'],
...
test('RustTavern Sync dataset selection rejects invalid legacy key', async () => {
    const { getSyncDatasetSelection } = await importSyncState();
    const catalog = {
        policyVersion: 1,
        supportedDatasetIds: ['characters', 'chats'],
...
test('RustTavern Sync Vue app stays presentation-only', async () => {
    const files = await listJsFiles('src/scripts/app/setting/sync-app');
    assert.ok(files.includes('src/scripts/app/setting/sync-app/index.js'));
    assert.ok(files.includes('src/scripts/app/setting/sync-app/SyncApp.js'));
    assert.ok(files.includes('src/scripts/app/setting/sync-app/SyncScopeApp.js'));
    assert.ok(files.includes('src/scripts/app/setting/sync-app/SyncProgressApp.js'));
    ...
    assert.match(entry, /export\s+function\s+mountRustTavernSyncApp/);
    assert.match(entry, /export\s+function\s+mountRustTavernSyncScopeApp/);
    assert.match(entry, /export\s+function\s+mountRustTavernSyncProgressApp/);
});

test('RustTavern Sync sends the selected overwrite policy through every operation', async () => {
    const popup = await readRepoFile('src/scripts/app/setting/setting-panel/sync-popup.js');
    const source = await readRepoFile('src/scripts/app/setting/sync-app/SyncApp.js');
    const { createRustTavernSyncApp } = await importSyncApp();
    const noopFunctions = new Proxy({}, { get: () => () => {} });
...
test('RustTavern Sync pure state helpers keep pair URI validation explicit', async () => {
    const source = await readRepoFile('src/scripts/app/setting/setting-panel/sync-state.js');
