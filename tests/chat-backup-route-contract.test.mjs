...
import { jsonResponse, textResponse } from '../src/host/main/http-utils.js';
import { createRouteRegistry } from '../src/host/main/router.js';
import { registerBackupsRoutes } from '../src/host/main/routes/backups-routes.js';
import { registerChatRoutes } from '../src/host/main/routes/chat-routes.js';
...
test('chat backup browser views through a stream and restores by logical backup name', async () => {
    ...
    const routeSource = await readFile(new URL('../src/host/main/routes/backups-routes.js', import.meta.url), 'utf8');
    const commandSource = await readFile(new URL('../src/host/main/kernel/invokes/host-commands.js', import.meta.url), 'utf8');
...
test('host startup scopes chat staging for portable and custom data roots', async () => {
    const [adapter, fsResources] = await Promise.all([
        readFile(
            new URL('../crates/tt-adapter-storage-core/src/repositories/file_chat_repository/mod.rs', import.meta.url),
            'utf8',
        ),
        readFile(new URL('../crates/rusttavern/src/server/fs_resources.rs', import.meta.url), 'utf8'),
    ]);

    // Chat commit staging lives inside the data root (`.staging/chat-commits`),
    // so a custom data root scopes it automatically.
    assert.match(adapter, /\.staging/);
    assert.match(adapter, /chat-commits/);

    // Server-side file access (plugin:fs|*, /__tt/file) is validated against
    // the canonical data root (plus the transient upload staging root).
    assert.match(fsResources, /validate_server_path/);
    assert.match(fsResources, /data_root/);
    assert.match(fsResources, /starts_with/);
    assert.match(fsResources, /upload_staging_root/);
});
