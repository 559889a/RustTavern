...
function installCurrentChatRef(chatRef) {
    ...
    globalThis.window.__RUSTTAVERN__ = {
        api: {
            chat: {
                current: {
                    ref: () => chatRef,
...
async function installHarness(options = {}) {
    ...
    ensureCustomEvent();
    globalThis.window = new EventTarget();
    globalThis.window.__RUSTTAVERN__ = { api: {} };
    const safeInvoke = options.safeInvoke || (async (command, args) => {
        calls.push({ command, args });
        return { command, args };
    });

    const { installAgentApi } = await import(pathToFileURL(path.join(REPO_ROOT, 'src/host/main/api/agent.js')));
    installAgentApi({
        safeInvoke,
    });
    ...
    return {
        calls,
        agent: globalThis.window.__RUSTTAVERN__.api.agent,
    };
}
...
test('api.agent.profiles publishes profile change events after successful mutations', async () => {
    ...
    const { subscribeAgentProfilesChanged } = await import(pathToFileURL(path.join(
        REPO_ROOT,
        'src/scripts/rusttavern/agent/agent-profile-events.js',
    )));
    const events = [];
...
test('api.agent.startRunWithPromptSnapshot refreshes Model Target LLM connection before starting run', async () => {
    ...
    globalThis.window.__RUSTTAVERN__.api.llmConnections = {
...
test('api.agent.retention forwards settings and prune contracts', async () => {
    ...
    const { agent } = await installHarness({
        safeInvoke: async (command, args) => {
            calls.push({ command, args });
            if (command === 'get_rusttavern_settings') {
                return {
                    agent: {
                        retention: {
            ...
            if (command === 'update_rusttavern_settings') {
                return {
                    agent: {
                        retention: {
    ...
    assert.deepEqual(calls, [
        {
            command: 'get_rusttavern_settings',
            args: undefined,
        },
        {
            command: 'update_rusttavern_settings',
            args: {
                dto: {
                    agent: {
                        retention: {
...
test('agent chat commit bridge detaches on partial success terminal event', async () => {
    const moduleUrl = pathToFileURL(path.join(REPO_ROOT, 'src/host/main/api/agent-chat-commit-bridge.js'));
...
test('agent prompt assembly bridge reads pending request by assembly id', async () => {
    const moduleUrl = pathToFileURL(path.join(REPO_ROOT, 'src/host/main/api/agent-prompt-assembly-bridge.js'));
...
test('shared agent run event subscription fans out over one backend poller', async () => {
    const moduleUrl = pathToFileURL(path.join(REPO_ROOT, 'src/host/main/api/agent-run-event-subscription.js'));
