...
test('Agent System settings use the extension store and publish changes', async () => {
    ...
    const settings = await importFresh('src/scripts/rusttavern/agent/agent-system-settings.js');
...
test('Agent run retention panel uses the host retention facade', async () => {
    ...
    assert.doesNotMatch(source, /safeInvoke|plan_agent_run_prune|apply_agent_run_prune|update_rusttavern_settings|localStorage/);
...
test('Embedded Agent Skill import isolates per-item preview and install failures', async () => {
    const source = await readFile(path.join(
        REPO_ROOT,
        'src/scripts/app/agent-skills/embedded-import.js',
    ), 'utf8');
...
test('Agent generation router uses the global toggle for normal regenerate and swipe', async () => {
    ...
    const router = await importFresh('src/scripts/rusttavern/agent/agent-generation-router.js');
...
test('Agent context policy windows latest-first prompt history without mutating frozen input', async () => {
    const contextPolicy = await importFresh('src/scripts/rusttavern/agent/agent-context-policy.js');
    const chat = [
...
test('Agent history window is applied at PromptManager assembly boundary', async () => {
    const [scriptSource, openaiSource, brokerSource] = await Promise.all([
        ...
        readFile(path.join(REPO_ROOT, 'src/host/main/api/agent-prompt-assembly.js'), 'utf8'),
    ]);
...
test('FrozenRunInputSnapshot stores materialized extension prompts and macro context', async () => {
    const frozen = await importFresh('src/scripts/rusttavern/agent/frozen-run-input-snapshot.js');
...
test('Agent run helpers keep main runtime imports native for the extension bundle', async () => {
    const controllerSource = await readFile(path.join(
        REPO_ROOT,
        'src/scripts/rusttavern/agent/agent-run-controller.js',
    ), 'utf8');
    const retrySource = await readFile(path.join(
        REPO_ROOT,
        'src/scripts/rusttavern/agent/agent-run-retry.js',
    ), 'utf8');
    ...
    assert.match(retrySource, /import\(['"]\/scripts\/rusttavern\/agent\/agent-generation-router\.js['"]\s*\/\*\s*webpackIgnore:\s*true\s*\*\/\)/);
});
...
test('Agent run controller tracks active runs until terminal events', async () => {
    ...
    const controller = await importFresh('src/scripts/rusttavern/agent/agent-run-controller.js');
    const stateChanges = [];
...
test('Agent guidance composer intercepts active-run sends before busy generation gates', async () => {
    ...
    assert.match(scriptSource, /Popup\.show\.confirm\(\s*t`Send as Agent guidance\?`/);
...
test('Agent error presenter surfaces userRetryable from run_failed payload', async () => {
    const presenter = await importFresh('src/scripts/rusttavern/agent/agent-error-presenter.js');
...
test('Agent error presenter translation keys exist in global locales', async () => {
    const source = await readFile(path.join(
        REPO_ROOT,
        'src/scripts/rusttavern/agent/agent-error-presenter.js',
    ), 'utf8');
...
test('Agent retry resolves typed generation intent instead of clicking regenerate UI', async () => {
    const retry = await importFresh('src/scripts/rusttavern/agent/agent-run-retry.js');
...
test('Rollback helper deletes drift messages back-to-front and dedupes targets', async () => {
    const { rollbackAgentRunDriftMessages } = await importFresh('src/scripts/rusttavern/agent/agent-run-message-rollback.js');
