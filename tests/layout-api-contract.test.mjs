...
test('api.layout installs and returns snapshots (safe-area + IME)', async () => {
    ...
    const { installLayoutApi } = await import(pathToFileURL(path.join(REPO_ROOT, 'src/host/main/api/layout.js')));
    installLayoutApi({});

    assert.ok(windowMock.__RUSTTAVERN__?.api?.layout);
    const layout = windowMock.__RUSTTAVERN__.api.layout;
...
test('layout-kit.js exists and exposes stable surface strings', async () => {
    const source = await readFile(path.join(REPO_ROOT, 'src/scripts/rusttavern/layout-kit.js'), 'utf8');
