...
test('extension store tryGetJson uses optional lookup command', async () => {
    ...
    const { installExtensionStoreApi } = await importFresh('src/host/main/api/extension-store.js');
    ...
    const result = await window.__RUSTTAVERN__.api.extension.store.tryGetJson({
        namespace: 'my-ext',
        table: 'state',
        key: 'settings',
    });
