...
test('session persistence uses the host lifecycle registry and flushes pending work only', async () => {
    ...
    const contextSource = await readFile(path.join(REPO_ROOT, 'src/host/main/context/index.js'), 'utf8');
    const invokeSource = await readFile(path.join(REPO_ROOT, 'src/host/main/services/invokes/invoke-service.js'), 'utf8');
