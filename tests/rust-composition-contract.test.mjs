...
test('Rust composition keeps repository sharing explicit', async () => {
    const source = await readRepoFile('crates/rusttavern/src/app/composition/repositories.rs');
...
test('Rust sync composition keeps one shared coordinator and pairing approval', async () => {
    const source = await readRepoFile('crates/rusttavern/src/app/composition/services/sync.rs');
...
test('Rust agent composition keeps one complete runtime behind the chat completion service gateway', async () => {
    const source = await readRepoFile('crates/rusttavern/src/app/composition/services/agent.rs');
