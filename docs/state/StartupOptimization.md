...
- **Host Ready 显式等待**：在首次 `/api/*` 访问前等待 `__RUSTTAVERN_MAIN_READY__`，确保拦截器/路由已安装，且 Rust 后端已进入可接收 `AppState` 命令的 readiness 状态。
...
- `src/index.html`
  ...
  - 约束：不使用 modulepreload（移动端浏览器上曾有缓存/预加载失败问题）。

### 2.2 Bootloader：分层 import + 移动端 import 重试

- `src/init.js`
  - 设置：`window.__TAURI_RUNNING__ = true`，并计算 `globalThis.__RUSTTAVERN_PERF_ENABLED__`
  - 通过 `importWithRetry()` 依次加载：
    1) `./lib.js`（静态依赖 `src/dist/lib.core.bundle.js`）
    2) `./host-main.js`（安装 Host Kernel：路由/拦截器/ABI）
    3) `./script.js`（SillyTavern 主前端）
  - 目的：在移动端浏览器首启 I/O 抖动场景下提高模块加载确定性。

### 2.3 Host Kernel：把同源 /api 变成可路由端点

- `src/host/main/bootstrap.js`
  - 安装 `window.__RUSTTAVERN__`（稳定 Host ABI）：invoke broker / 资源路径 helpers 等
  - Patch：
    - `fetch` 拦截（同源、命中路由表则走 `router.handle(...)`）
    - `jQuery.ajax` 拦截（同源、命中路由表则转发）
    - same-origin iframe/window 的补丁（保证扩展/脚本在 iframe 内也能命中路由/下载桥）
  - 设置 readiness：
    - `window.__RUSTTAVERN_MAIN_READY__ = readyPromise`
...
- `globalThis.__RUSTTAVERN_STARTUP_STAGE__`：由 `firstLoadInit()` 写入（shell/core/full）
- `event_types.APP_READY`：作为“主应用可交互”的稳定语义；晚加载的扩展依赖其 auto-fire 行为完成 ready 钩子

### 3.2 Host 就绪（拦截器、路由与 BackendReadiness）

- `window.__RUSTTAVERN_MAIN_READY__`：在 `src/host/main/bootstrap.js` 设置
- `waitForTauriMainReady()`：前端轮询等待该 promise 注册并 resolve（`src/scripts/extensions/runtime/host-ready.js`）
- 宿主运行时下，Host 初始化链会在任何需要 `AppState` 的首批工作前调用 `wait_for_backend_ready`，避免依赖 “state not managed” 错误文本重试。
...
- `src/host/main/routes/bootstrap-routes.js`
...
- `crates/rusttavern/src/presentation/commands/bootstrap_commands.rs:get_bootstrap_snapshot`
...
> 详细扩展兼容链路见：`docs/state/ThirdPartyExtensions.md`
...
- `src/scripts/app/perf/code-highlight-coordinator.js`
...
- `src/host/main/adapters/panel-runtime/extensions-subtree-gates.js`
...
- `src/host/main/adapters/panel-runtime/top-settings-panel-parking.js`
...
- 标记（部分）：
  - `src/init.js`：`tt:init:*`
  - `src/host/main/bootstrap.js`：`tt:tauri:*`
  - `src/script.js`：`tt:startup:shell/core/full` + `tt:startup:ready`
- 运行时提示：
  - `src/scripts/app/startup/startup-status-overlay.js`：右下角非阻塞启动状态 overlay（`APP_READY` 后移除）
