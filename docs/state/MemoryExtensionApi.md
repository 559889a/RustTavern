...
本文档记录 RustTavern 记忆、数据库与检索类扩展 API 的当前状态。

RustTavern 遵循 SillyTavern 1.18.0 契约：`getContext().chat` 是当前聊天的完整、有序消息数组，数组下标就是 0-based 绝对消息索引。增强 API 不替代这一契约，而是把有界读取、定位、搜索和扩展状态持久化交给 Rust 后端。
...
- Chat payload：`docs/contracts/ChatPayload.md`
