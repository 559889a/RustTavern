...
> 兼容性的关键不是继续在前端 runtime 里“解释扩展代码”，而是把 third-party 资源路径重新做成浏览器可原生加载的真实端点。
...
2. `src/host/main/routes/extensions-routes.js` 将请求转给 Rust 命令 `get_extensions`
...
- 只有旧浏览器不支持 CSS `@layer` 时，`third-party-runtime.js` 才会为样式 URL 附加 `ttCompat=layer`，并由 Rust 端点返回展平后的 CSS bytes
...
- `src/host/main/compat/mobile/mobile-runtime-compat.js` 负责旧浏览器缺失 JS API 的 polyfills（仅移动端旧浏览器，兼容保留）
- 第三方浮层/窗口 mobile surface compat（仅移动端旧浏览器，兼容保留）：
  ...
  - 同源 iframe contract bridge：`src/host/main/compat/mobile/mobile-iframe-viewport-contract-bridge.js`
- `src/scripts/browser-fixes.js` 保持与上游同步（不再承载移动端 compat）

### 2.4 后端资源提供

生产/打包运行时：

- `crates/rusttavern/src/server/router.rs` 挂载 Host Resource 路由（`/css/user.css`、`/scripts/extensions/third-party/*`、`/thumbnail`、用户静态资源）
- `crates/tt-application/src/services/host_resource_service/user_css.rs` 处理 `/css/user.css`
- `crates/tt-application/src/services/host_resource_service/third_party.rs` 处理 `/scripts/extensions/third-party/*`
- `crates/tt-application/src/services/host_resource_service/thumbnail.rs` 处理 `/thumbnail`
- `crates/tt-application/src/services/host_resource_service/user_data.rs` 处理用户数据静态资源：`/characters/*`、`/User Avatars/*`、`/backgrounds/*`、`/assets/*`、`/user/images/*`、`/user/files/*`

请求处理步骤：

1. 校验请求方法，只接受 `GET` / `HEAD` / `OPTIONS`
2. 通过 `crates/tt-contracts/src/client_asset_paths.rs` 解析并校验路径，再由 `crates/tt-application/src/services/host_resource_service/route_classifier.rs` 分类到具体资源处理器
3. 通过 `crates/tt-adapter-media/src/host_resources.rs` 一次完成 local/global 选源、打开文件和 metadata/revision 构造
4. 返回真实 bytes、正确 `Content-Type`、`Cache-Control: private, no-cache`、weak ETag 和 Last-Modified；条件命中返回 `304 Not Modified`（见 `docs/contracts/HostResourceCaching.md`）
   - 对用户静态资源端点（如 `/backgrounds/*`）若请求携带 `Range`，支持单范围并返回 `206 + Content-Range`（见 `docs/contracts/MediaAssetContract.md`）
5. 未命中时返回真正 `404`，不回退到 `index.html`

Host Resource 只校验浏览器 URL 的路径段，不禁止 data root 内部 symlink 指向外部目录或文件。这样用户可以让 SillyTavern 与 RustTavern 共享同一套数据目录布局。
...
- 开发态与生产态共用同一个 HTTP 服务器（`pnpm run dev` = Rspack watch + cargo run 并行），third-party 路径语义一致。
- 旧 Tauri 形态的 `tt-ext` Service Worker 代理链路（`src/init.js` + `src/tt-ext-sw.js`）在服务器形态下自然回退：注册守卫要求 `window.__TAURI__` 的 `convertFileSrc` 存在，而浏览器页面中该符号不存在，因此不会注册 SW，也不会把资源请求转发到 custom scheme。`src/dev-sw-bootstrap.js` / `src/tt-ext-sw.js` 作为兼容保留，不再参与资源交付。

因此，开发态与生产态使用同一 HTTP 入口，third-party 路径语义保持一致。
...
- 若调整路径规则，至少同步检查：
  ...
  - `crates/tt-contracts/src/client_asset_paths.rs`
  - `crates/tt-application/src/services/host_resource_service/route_classifier.rs`
  - `crates/tt-application/src/services/host_resource_service/third_party.rs`
  - 相关测试
- 若改动开发态链路，也必须同步验证 `src/dev-sw-bootstrap.js`、`src/init.js` 与 `src/tt-ext-sw.js`（兼容保留，不应重新激活）
