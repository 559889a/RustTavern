...
> 完整的分阶段启动（Shell/Core/Full/APP_READY）链路请看：`docs/state/StartupOptimization.md`。
...
该数组的单条 record 内含大量长字符串（`rawPrompt/finalPrompt/worldInfo/...`），体积通常远大于聊天正文，导致移动端浏览器 renderer 常驻集显著膨胀。
