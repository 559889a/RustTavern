...
- **TT-Sync v2**：远端同步（RustTavern ⇄ TT-Sync 服务端）；与 LAN Sync 共享协议语义，但拓扑是 remote hub。

关键结论（后续改动优先守住这些）：

1. **同步语义以“用户数据一致性”为中心**：scope/exclude、`(size_bytes, modified_ms)` 增量判定、Mirror delete 的时序、原子写入与 mtime 保留。
2. **本地 mutation 全局串行**：LAN Sync、TT-Sync v2 与第三方扩展 install/update/switch/delete/move 共用同一个 composition-root `Semaphore(1)`。所有本地写操作都用 `try_acquire_owned()` fail-fast；只发送远端 pull-request 的作业、extension discovery/version/branches 不占 permit（见 `crates/rusttavern/src/app/composition/services/{mod.rs,sync.rs}`）。
...
- LAN Sync 状态：`default-user/user/lan-sync/`
  ...
  - Sync 偏好：`sync-preferences.json`（保存手动默认 Sync mode 与全局 `overwrite_policy`；由旧 `config.json.sync_mode` 一次迁移，旧文件缺少策略时默认为 `Exact`；见 `crates/tt-adapter-sync/src/lan_sync/store.rs`）
  - 自动同步规则：`automation.json`（运行期自动上传开关、目标、间隔、显式 Sync mode、范围与 bundle 要求；见 `crates/tt-adapter-sync/src/sync_automation_store.rs`）
  - peer 状态：`v2/identity.json` / `v2/peers.json` / TLS 状态（见 `crates/tt-adapter-sync/src/sync/lan/store.rs`）
- TT-Sync v2 状态：`default-user/user/lan-sync/tt-sync-v2/`
  - `identity.json` / `paired-servers.json`（见 `crates/tt-adapter-sync/src/tt_sync/store.rs`）

LAN Sync 与 TT-Sync v2 的 manifest 扫描严格遵循 `ttsync_core::dataset::ResolvedDatasetPolicy`，并且会排除 LAN/TT 同步状态目录（见 `crates/tt-adapter-sync/src/tt_sync/fs.rs`）。当前 RustTavern 默认数据集还会排除 `_tauritavern/prompt-cache/`、`_tauritavern/.ios-policy.json`、`_cache/`、`.staging` 与同步临时文件。
...
默认 RustTavern 数据集已经覆盖 Agent 连续性数据：
...
`default-user/secrets.json`、Agent `model-responses/`、`checkpoints/`、`backups/`、`vectors/`、`thumbnails/` 被保留为独立数据集，不在 RustTavern 推荐默认同步中；用户可以在 Sync Panel 的“Sync content”范围选择弹窗里显式勾选。
...
- LAN Sync：
  ...
  - 应用边界：`crates/tt-application/src/services/lan_sync_service.rs`
  - Tauri event / pairing approval adapter：`crates/rusttavern/src/app/composition/adapters.rs`
  - Axum server lifecycle adapter：`crates/tt-adapter-sync/src/sync/lan/control.rs`
- TT-Sync：
  - 进度/完成/错误：`sync:job`
  - 完成/错误：手动命令通过 `SyncJobReport` 返回；不额外发 TT 专属 completed/error 事件。
  - runtime：`crates/tt-adapter-sync/src/tt_sync/runtime.rs`
- 自动同步：
...
Sync Panel 是 RustTavern 自有设置面板，不属于上游 SillyTavern 事件 ABI。它遵循现有 host wrapper 边界：

- `src/scripts/app/setting/sync-app/**` 只做 Vue 展示组件，不直接访问 Tauri invoke、Popup、扫码服务或 SillyTavern host API。
- `src/scripts/app/setting/setting-panel/sync-popup.js` 拥有 popup / Tauri invoke / QR 扫码能力，并负责把 UI 选择转换为命令参数。
- `sync_get_dataset_catalog` 返回当前 `DatasetPolicy` 版本、支持的数据集 ID、profile ID 与 RustTavern 默认范围；前端只持久化 dataset ID，不持久化路径。
...
入口：`tt_sync_pair`（`crates/rusttavern/src/presentation/commands/tt_sync_commands.rs`）→ `TtSyncService::pair`（`crates/tt-application/src/services/tt_sync_service.rs`）。
...
- `base_url` **必须是 https**，并进行 **SPKI pinning**（见 `crates/tt-adapter-sync/src/sync/http_client.rs`）。
- Pair 只建立信任与权限，不传输用户数据。

### 5.2 TT-Sync v2 Push / Pull（远端同步）

入口：`tt_sync_push` / `tt_sync_pull`（`crates/rusttavern/src/presentation/commands/tt_sync_commands.rs`）。
...
入口仍是现有 LAN Sync 命令面（`crates/rusttavern/src/presentation/commands/lan_sync_commands.rs`）：
...
客户端策略（见 `ttsync_client::ClientSyncEngine` 与 `crates/tt-adapter-sync/src/sync/job_executor.rs`）：
...
- LAN Sync 使用默认并发（桌面 4 / 移动 2）：`crates/tt-adapter-sync/src/sync_transfer.rs`
- TT-Sync 使用更高并发（桌面 16 / 移动 8）：`crates/tt-adapter-sync/src/sync/job_executor.rs`
- 所有写入都走原子写入并保留 mtime：`crates/tt-adapter-sync/src/sync_fs.rs`
...
- RustTavern v2 客户端当前显式偏向 HTTP/1.1。bundle 是单个长流，现有 reqwest/hyper HTTP/2 默认 flow-control 在局域网实测下不如 HTTP/1.1；协议本身仍只要求 HTTPS + SPKI pinning，不把 HTTP 版本暴露为外部契约。
...
2. **原子写入**：每文件都走 `tmp → set mtime → rename`；断线发生在写入过程中只会留下 tmp，不会覆盖目标文件（`crates/tt-adapter-sync/src/sync_fs.rs`）。
...
- 同步 scope 内 **不支持 symlink**（扫描时直接报错，见 `crates/tt-adapter-sync/src/tt_sync/fs.rs`）。
