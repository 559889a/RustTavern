...
实际物理根目录为数据根下的 `_tauritavern/agent-workspaces`。它是 RustTavern 运行产物空间，不属于上游 SillyTavern `default-user` 聊天文件布局。

`<chat-workspace-id>` 必须由稳定聊天身份派生：

```text
stableChatId = window.__RUSTTAVERN__.api.chat.open(chatRef).stableId()
...
MessageExtra(key)
  写入 chat message extra 的 RustTavern namespace。
...
- `rusttavern-settings.agent.retention.auto_prune_enabled` 默认 `false`，控制 Rust 后端是否在 RustTavern 进程运行期间周期性执行自动清理。
- `rusttavern-settings.agent.retention.keep_recent_terminal_runs` 是 terminal run 核心历史窗口，默认 100。
- `rusttavern-settings.agent.retention.keep_full_recent_runs` 是完整 workspace/debug artifacts 窗口，默认 20，必须小于等于 `keep_recent_terminal_runs`。
