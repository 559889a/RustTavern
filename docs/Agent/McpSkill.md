...
当前状态：尚未实现 `window.__RUSTTAVERN__.api.mcp`，MCP 也未贡献到 Agent Tool Catalog。Skill 已落地 `window.__RUSTTAVERN__.api.skill`，并通过 builtin `skill.list` / `skill.search` / `skill.read` 接入现有控制面。当前 Skill 细节以 `docs/Agent/Skill.md` 与 `docs/API/Skill.md` 为准。
