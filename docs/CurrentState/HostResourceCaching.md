...
本文记录 RustTavern 自有 Host Resource 的当前表示、条件请求和 HTTP 交付契约。普通外部 HTTP/HTTPS 资源不属于本模块，由浏览器网络栈处理。
...
Delivery       当前 HTTP 入口能承载的响应形式（axum 直出）
```

缓存职责也按层分离：filesystem 是资源事实，Rust thumbnail cache 是由 source revision 与 recipe 标识的派生产物，Host Resource Service 是浏览器可见表示的新鲜度权威，浏览器 cache 只做机会性存储。mutation 不维护第二套 revision registry，也不依赖浏览器是否真的保存 synthetic response 才能保证正确性。
...
HTTP request/response 直接使用 `http` crate 类型。旧的字符串 status/header DTO 已删除；presentation 只负责 origin gate、delivery capability 和 response header 合并。
...
`no-cache` 允许浏览器存储响应体，但要求复用前验证。它不是 `no-store`。
...
服务器形态下所有 Host Resource 都由 axum 直接以标准 HTTP 交付，浏览器负责缓存与条件请求语义。旧 Wry/Android 的 304 缺失限制与二次 Range workaround 不再适用：

- validator 命中返回标准 `304 Not Modified`；
- `Range` 请求返回 `206 Partial Content` + `Content-Range`（单范围；malformed/multi-range 返回 416）；
- 不得用 204、空 200 或浏览器 patch 模拟 304。

## 7. Origin 与开发态

所有页面与 Host Resource 端点同源（默认 `http://127.0.0.1:8000`）。presentation 在路由前校验请求路径与 data root 边界（`validate_server_path`），外部绝对路径不进入 Host Resource Service。

开发态与生产态使用同一 HTTP 服务器（`pnpm run dev` = Rspack watch + cargo run 并行），因此资源表示、条件请求与 Range 语义天然一致。请求头（`If-None-Match` / `If-Modified-Since` / `Range` / `If-Range`）由浏览器原生发出，无需 IPC 桥接或 custom scheme。
...
- data-root-backed mutable Host Resource 长期保持稳定 URL + `private, no-cache`；Host Resource Service 是浏览器可见表示的新鲜度权威，这不是采用 immutable 前的临时 fallback；
...
- 第一方普通 browser resource 不得通过 base64 read command 或 server-thumbnail Blob cache 绕过本服务；
- mutation 只建立文件事实，既不维护/删除 Rust derived cache，也不预热浏览器 cache；随后只负责触发稳定 URL 的真实 consumer demand，刷新不是 commit 的组成部分，也不得用随机 query 伪造 revision；
- frontend 仅保留有独立派生职责的 background preview cache；Host background 记录必须绑定当前 raw ETag 与 preview recipe；
- 浏览器 synthetic response cache 只是优化，不能成为业务正确性的前提。
