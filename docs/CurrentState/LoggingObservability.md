...
Rust 普通日志统一使用 `tracing::{debug, info, warn, error}` 宏，不再经过 `infrastructure::logging::logger` facade。启动期在 `crates/rusttavern/src/infrastructure/logging/tracing_runtime.rs` 安装全局 subscriber：

- 全局 `EnvFilter` 控制所有 tracing event；
- stdout 与 rolling file 记录普通日志；
- backend log layer 写入 Dev 面板使用的 backend log store；
- user-error layer 只观察 `ERROR + target = rusttavern::user_error`。
...
应用遵循全局 `EnvFilter`。如果用户过滤掉 `rusttavern::user_error` target，对应 backend error toast 也会被过滤；这是用户对观测输出的合理控制。

## 3. 前端桥接

user-error layer 将消息交给 `app/backend_errors.rs::BackendErrorHub`。前端启动时 `src/host/main/bootstrap/backend-error-bridge.js` 先监听事件 `rusttavern-backend-error`（SSE `/__tt/events`），再调用 `backend_error_bridge_ready`：
