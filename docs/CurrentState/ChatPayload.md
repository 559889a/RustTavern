...
这与 SillyTavern 1.18.0 的前端契约一致。RustTavern 不再提供 `chat_history_mode`，也不存在前端 window state、生成时 backfill 或局部 patch 保存。
...
分页是显式查询能力，不参与当前聊天的 `chat[]`、DOM、generation 或保存。`window.__RUSTTAVERN__.api.chat` 的 `history.tail/before/beforePages` 是其公开前端入口。
...
- `src/scripts/app/chat/transport.js`：完整 payload Tauri transport。
- `src/host/main/api/chat.js`：扩展历史分页 API 与 `windowInfo()`。
...
- Rust commands：`rusttavern` presentation 层。
