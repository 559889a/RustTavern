...
- 入口：`src/host/main/services/embedded-runtime/embedded-runtime-service.js`
  - 创建 manager 并挂到 `globalThis.__RUSTTAVERN_EMBEDDED_RUNTIME__`（用于 perf-hud / 调试）。
- 核心实现：`src/host/main/services/embedded-runtime/embedded-runtime-manager.js`
...
- `src/host/main/services/embedded-runtime/embedded-runtime-profiles.js`
  - `compat` / `mobile-safe`
- `src/host/main/services/embedded-runtime/embedded-runtime-profile-state.js`
  - 正式配置：`rusttavern-settings.embedded_runtime_profile = 'off' | 'auto' | 'compat' | 'mobile-safe'`
...
- slot 实现：`src/host/main/adapters/embedded-runtime/managed-iframe-slot.js`
...
- 软停车池：`src/host/main/adapters/embedded-runtime/managed-iframe-parking-lot.js`
...
- JSR：`src/host/main/adapters/embedded-runtime/js-slash-runner-runtime-adapter.js`
- LWB：`src/host/main/adapters/embedded-runtime/littlewhitebox-runtime-adapter.js`
...
- 安装：`src/host/main/services/embedded-runtime/install.js`
  - 在 `APP_READY` 后安装 chat 级 adapter
- 核心：`src/host/main/adapters/embedded-runtime/chat-embedded-runtime-adapter.js`
...
1) `src/host/main/bootstrap.js` 先读取 bootstrap mirror；仅当 profile 不是 `off` 时，才在 main ready 后动态导入 `installEmbeddedRuntime()`
...
- `src/scripts/app/message/mes-text-write.js`
  - `replaceMesTextHtmlWithRuntimePolicy(mesEl, html, options)`
  - `off` 时直接恢复普通 `.mes_text` HTML 写入语义
  - 其余 profile 下委托给渲染事务

- `src/host/main/adapters/embedded-runtime/message-render-transaction.js`
...
- perf-hud：`src/host/main/perf/perf-hud.js`
  - HUD 中 `Runtime:` 行来自 `__RUSTTAVERN_EMBEDDED_RUNTIME__.getPerfSnapshot()`
  - 可用 `Ctrl+Alt+P` 打开/关闭；或 `localStorage tt:perf=1` 自动启用
- 调试入口：`globalThis.__RUSTTAVERN_EMBEDDED_RUNTIME__`
