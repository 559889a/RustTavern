...
> `docs/CurrentState/HttpServerMigrationLog.md`，本文档保留为迁移蓝图与决策依据。
>
...
2. 新增 `crates/rusttavern/src/server/` 模块：
   ...
   - `host_resources.rs`：axum 路由 → `HostResourceService::try_serve(request, HTTP_DELIVERY)`（新 delivery 常量），覆盖 `/thumbnail`、`/characters/*`、`/User Avatars/*`、`/backgrounds/*`、`/assets/*`、`/user/images/*`、`/user/files/*`、`/scripts/extensions/third-party/*`、`/css/user.css`；保留现有 404/ETag/Cache-Control/Range 语义（`docs/CurrentState/HostResourceCaching.md`、`MediaAssetContract.md` 是验收依据）
3. **命令层批量机械改造**（编译器兜底）：
...
- 更新：`docs/BackendStructure.md`（host = axum，presentation = HTTP 分发）、`docs/FrontendHostContract.md`（传输层变更：invoke over HTTP、SSE 事件；`window.__TAURI_*` ABI 语义标注"宿主内核"而非"Tauri"）、`docs/FrontendGuide.md`、`README*.md`（新使用方式：下载/构建 → 启动 → 浏览器访问；局域网/公网部署指引含防火墙与 HTTPS 反代建议）、`AGENTS.md`/`agents.md`、`ExtensionDEV.md`、`docs/CurrentState/*`（DataDirectorySelection 补 server 模式、UpdateChannels、HostResourceCaching、LinuxRepository 等）
- 删除：`docs/AndroidDevelopment.md`、`docs/iOSDevelopment.md`、`docs/CurrentState/iOSPolicy.md`
