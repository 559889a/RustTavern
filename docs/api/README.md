# RustTavern Extension APIs

RustTavern 专属扩展 API 的统一入口是 `window.__RUSTTAVERN__.api`。
...
const host = window.__RUSTTAVERN__;
const api = host?.api;
```
...
| [Migration.md](Migration.md) | 从 SillyTavern 扩展迁移到 RustTavern 的适配指南 |

## 契约说明

- API 类型定义见 `src/types.d.ts`
- 宿主契约与稳定性边界见 `docs/architecture/FrontendHostContract.md`
- Agent 已落地当前 Host ABI、canonical model IR、provider_state continuation、上下文只读工具、Skill tools 与 workspace 读改工具；真实边界见 `docs/api/Agent.md`、`docs/state/AgentFramework.md` 与 `docs/contracts/AgentProviderState.md`
- LLM Connection 管理 API 真实边界见 `docs/api/LlmConnections.md` 与 `docs/architecture/agent/PromptAssembly.md`
- Skill 管理 API 真实边界见 `docs/api/Skill.md` 与 `docs/architecture/agent/Skill.md`
- MCP 仍处于规划阶段；实现前请以 `docs/api/MCP.md` 与 `docs/architecture/FrontendHostContract.md` 的草案约束为准
