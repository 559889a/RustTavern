# `window.__RUSTTAVERN__.api.mcp` — MCP API Draft
...
const mcp = window.__RUSTTAVERN__.api.mcp;
```

## 2. 方法概览

```ts
type RustTavernMcpApi = {
...
MCP prompt 可以成为 PromptComponent，但不能覆盖 RustTavern preset policy。
