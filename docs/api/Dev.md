# `window.__RUSTTAVERN__.api.dev` — API 参考

RustTavern 为开发者工具、调试面板与扩展作者提供的规范化调试 API。

> 设计目标：让调用方依赖稳定宿主 ABI，而不是直接依赖 SSE 事件名、Rust 命令名或某个 Settings 面板的内部实现。

## 0. 快速上手

```js
await (window.__RUSTTAVERN__?.ready ?? window.__RUSTTAVERN_MAIN_READY__);
const dev = window.__RUSTTAVERN__.api.dev;
```
...
- `rusttavern-backend-log`
- `rusttavern-llm-api-log`
- `devlog_*`
