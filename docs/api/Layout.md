# `window.__RUSTTAVERN__.api.layout` — Layout API（Safe‑Area / Viewport / IME）

RustTavern 为扩展作者提供的布局 API：用**稳定契约（CSS vars + attributes）**统一移动端 safe‑area、viewport 与 Android IME（键盘）语义；并提供 `api.layout` 作为读取/订阅入口，外加可选 `layout-kit.js` DX 糖衣。

> **核心理念**：宿主负责“提供契约与最小准入（admission）”，扩展只需几行 opt‑in，就能稳定适配移动端；不引入新的运行时几何系统，不做黑名单堆积。

## 0. 快速上手（推荐：layout-kit.js）

`layout-kit.js` 是 RustTavern 提供的“软 SDK（DX 糖衣）”：**推荐使用**，用于避免字符串拼写错误、减少样板代码。
...
import {
  ...
  SURFACE,
  applySurface,
} from '/scripts/rusttavern/layout-kit.js';
...
const layout = window.__RUSTTAVERN__.api.layout;
...
> 说明：iOS 主要使用 `env(safe-area-inset-*)`，RustTavern 在 `src/style.css` 中提供默认值；Android 以 native 注入为准。
...
移动端浏览器下键盘弹出通常表现为 viewport resize；宿主把 IME 语义归一化为 `--tt-ime-bottom` 并注入到“当前正在输入的 surface root”，通过 `data-tt-ime-*` 标记 active surface，避免不同浏览器键盘语义差异扩散到布局代码。
