...
本文档记录 RustTavern 当前已经落地的角色身份契约，重点覆盖角色头像文件名、聊天目录、旧错误目录兼容、以及后续开发时不能误改的边界。
...
RustTavern 前端来源于 SillyTavern。角色相关接口必须遵循上游契约：
...
- `src/host/main/services/characters/character-identity.js`
...
- `crates/tt-adapter-storage-core/src/chat_directory_identity.rs`
- `crates/tt-adapter-storage-core/src/repositories/file_chat_repository/chat_dir_resolver.rs`
- `crates/tt-adapter-storage-userdata/src/repositories/file_character_repository/helpers.rs`
...
这条规则避免了升级时全量扫描和批量迁移，对低端移动设备更安全：只有用户实际访问相关角色时才做一次小范围解析。
