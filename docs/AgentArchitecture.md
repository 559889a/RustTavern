...
- 后端已经采用 workspace crate 边界：`tt-application` 依赖 `tt-ports/tt-contracts/tt-domain`，concrete IO 位于 `tt-adapter-*`，host crate（axum HTTP 服务器）只负责 composition、commands 与 host-bound infrastructure。见 `docs/BackendStructure.md`。
...
- 前端 Public ABI 的统一入口是 `window.__RUSTTAVERN__`，应保持小而稳定。见 `docs/FrontendHostContract.md` 第 3.6 节、`src/host/main/bootstrap.js:139`。
