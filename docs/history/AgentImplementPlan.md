# RustTavern Agent 实施说明

本文档记录 Agent 的当前实现、守护项和待办能力。历史施工计划已经收敛为当前架构、测试与契约；第 8 节按能力域归档，不作为旧行为的事实来源。

当前事实以 `docs/state/AgentFramework.md` 为准，架构边界以 `docs/architecture/agent/AgentArchitecture.md` 与 `docs/architecture/agent/AgentContract.md` 为准。
...
- 前端 Host ABI 已挂载 `window.__RUSTTAVERN__.api.agent`。
...
启动输入支持可选 `profileId`。Profile 管理、prompt assembly broker 与 tools list 已封装到 `window.__RUSTTAVERN__.api.agent`。
...
- `docs/state/AgentFramework.md` 与 `docs/contracts/AgentProviderState.md` 作为当前事实入口。
...
cargo test --manifest-path Cargo.toml agent --lib
cargo test --manifest-path Cargo.toml skill --lib
cargo test --manifest-path Cargo.toml agent_tools --lib
git diff --check
```
...
```bash
cargo test --manifest-path Cargo.toml agent_model_gateway
cargo test --manifest-path Cargo.toml openai_responses_payload
cargo test --manifest-path Cargo.toml claude_native_content_blocks_are_replayed
cargo test --manifest-path Cargo.toml normalize_
```
...
- 当前事实：`docs/state/AgentFramework.md`
- 架构边界：`docs/architecture/agent/AgentArchitecture.md`
- 硬契约：`docs/architecture/agent/AgentContract.md`
- LLM gateway：`docs/architecture/agent/LlmGateway.md`
- 工具语义：`docs/architecture/agent/ToolSystem.md`
- workspace 语义：`docs/architecture/agent/Workspace.md`
- Skill 语义：`docs/architecture/agent/Skill.md`
- 事件语义：`docs/architecture/agent/RunEventJournal.md`
- 测试矩阵：`docs/architecture/agent/TestingStrategy.md`
- Host ABI：`docs/api/Agent.md`、`docs/api/Skill.md`、`docs/architecture/FrontendHostContract.md`、`src/types.d.ts`
