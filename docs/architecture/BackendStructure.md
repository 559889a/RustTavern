...
本文档说明 RustTavern Rust 后端的 **Clean Architecture 原则、workspace crate 边界、依赖方向和代码落点**。

它不是 API 命令清单，也不是专题实现百科。具体 API 参考 `docs/api/`，当前实现快照参考 `docs/state/`，前端宿主契约参考 `docs/architecture/FrontendHostContract.md`。

## 1. 最高约束：Clean Architecture

Clean Architecture 是 RustTavern 后端最重要的长期约束。它不是一组漂亮的目录名，而是一条依赖规则：

> 越稳定、越靠近业务语义的代码越靠内；越依赖框架、文件系统、网络、平台 API 的代码越靠外；源码依赖只能从外层指向内层。

在本项目里，这条规则有几个直接含义：

- 领域模型和纯规则不能知道 HTTP 服务器框架、文件系统、async runtime 或 repository。
...
| --- | --- | --- | --- |
| Entities / 领域核心 | `tt-domain` | 领域模型、值对象、领域错误、纯规则、纯文本/JSON 工具 | repository trait、async runtime、文件系统、网络、HTTP 服务器框架 |
| Cross-crate contracts | `tt-contracts` | 跨 crate DTO、事件、payload、observability、通用值契约 | concrete IO、application service、HTTP 服务器框架 |
| Application ports | `tt-ports` | repository trait、gateway trait、event sink、runtime port、Host Resource opened-source port | concrete adapter、reqwest/axum/image/zip 实现 |
| Use cases | `tt-application` | service、use case、job coordinator、策略编排 | axum、adapter、reqwest、具体 tokenizer、文件格式实现 |
| Interface adapters | `tt-adapter-*` | HTTP-free 的具体 IO、运行时、持久化、文件格式、外部系统访问 | application service、host glue |
| Frameworks / Drivers | `rusttavern` | axum HTTP server、commands、composition root、host-bound infrastructure | 领域规则、用例实现、adapter repository |
...
workspace 根目录是 `Cargo.toml`，默认 member 是 `crates/rusttavern`。
...
| crate | 当前职责 |
| --- | --- |
| `rusttavern` | HTTP server host（axum）、presentation command 分发、composition root、host-bound infrastructure |
...
```mermaid
flowchart TB
    host["rusttavern<br/>HTTP host / commands / composition root"]
...
- `tt-domain` 不依赖 `tt-ports`、`tt-contracts`、adapter、HTTP 服务器框架、网络或文件系统。
- `tt-contracts` 不依赖 `tt-ports`、`tt-application`、adapter 或 HTTP 服务器框架。
- `tt-ports` 不依赖 adapter、HTTP 服务器框架、网络/文件系统具体实现。
- `tt-application` 不依赖任何 `tt-adapter-*`、`rusttavern`、`reqwest`、`axum`、`zip`、`image`。
- 任何 `tt-adapter-*` 不依赖 `tt-application` 或 `rusttavern`。
- host 的 `infrastructure` 不下沉为 application 或 adapter 的隐式依赖；具体服务装配集中在 `app/composition`。

如果新增代码需要违反这些方向，通常不是边界脚本太严格，而是抽象落点还没有想清楚。

## 5. 上游兼容契约

RustTavern 不复刻 SillyTavern 1.18.0 的 Node/Express 实现细节。它复刻的是上游前端、扩展、脚本和角色卡能观察到的契约：
...
Rust command、stream/channel、repository/service 分层是内部实现，不能当成上游兼容 API 暴露给扩展作者。
...
SillyTavern frontend / extension / script
  ...
  -> src/host/main/interceptors.js
  -> src/host/main/routes/*
  -> context.safeInvoke(...)
  -> src/host-bridge.js（HTTP invoke）
  -> POST /__tt/invoke/{command}
  -> rusttavern presentation command
  -> tt-application service
  -> tt-ports trait
  -> tt-adapter-* concrete implementation
```

新的 RustTavern 能力优先通过 `window.__RUSTTAVERN__.api.*` 暴露。只有行为确实对齐上游 SillyTavern 路由时，才进入 `/api/*` 兼容层。

## 6. Host 边界

`rusttavern` 是最外层 host crate。它可以知道所有 concrete implementation，但只应在明确边界处使用它们。

主要目录：

```text
crates/rusttavern/src/
├── lib.rs                 # host lib entry，只转入 server::run()
├── main.rs                # server entry（rusttavern_lib::run()）
├── server/                # axum HTTP 服务器：run/config/router/dispatch/events/stream/security
├── app/                   # composition root、AppState、EventBus、HostHandles、startup profile
├── presentation/          # command 注册表、CommandError、web resource adapter
└── infrastructure/        # host-bound infrastructure（logging/persistence/repositories/apis）
```

### 6.1 HTTP server shell

`server/*` 是 axum HTTP 服务器：

- `server/mod.rs`：`run()` 入口——CLI（clap）→ config 合并 → runtime paths → logging → 初始化 → 路由装配 → `axum::serve`（graceful shutdown，Ctrl-C/SIGTERM）。
- `server/config.rs`：CLI + `config.yaml` 合并；`--host/--port/--data-root/--config` 等。
- `server/router.rs`：路由表——`POST /__tt/invoke/{command}`、`GET /__tt/events`（SSE）、`GET /__tt/stream/{stream_id}`（SSE）、`GET /__tt/health`、`GET /__tt/file`、静态文件 + SPA fallback。
- `server/dispatch.rs`：`CommandRegistry` + `register!`/`register_sync!` 宏（命令分发）。
- `server/events.rs`：broadcast → SSE 事件流；`server/stream.rs`：流式 Channel 替代（StreamRegistry/StreamSink）。
- `server/security.rs`：认证（basic/session cookie）、IP/CIDR 白名单、CSRF 头强制。
- `server/fs_resources.rs`：`plugin:fs` rid 语义的资源注册表。

这里可以触碰 `axum`、tokio、HTTP 语义、platform API。这里不写业务用例。
...
只有确实依赖 HTTP server、bundled resources、runtime path、平台 API 或 host lifecycle 的代码才留在 `rusttavern/src/infrastructure`。

当前合理留在 host 的例子：

- runtime path 和数据目录选择引导配置。
- bundled resource 读取（exe 相对目录解析）。
- logging / Dev observability。
...
新的 HTTP-free repository 不应再放回 host infrastructure。
...
- `tt-adapter-sync` 与 `tt-adapter-archive` 是独立运行时/执行器边界，host glue 仍留在 host。

如果一个新 adapter 只是为了两个调用点提前抽象，先不要建 crate。等它有明确 bounded context、独立依赖成本或稳定变化原因时再拆。

## 8. Presentation 边界

`presentation` 是前端可调用的 command 与 HTTP 分发边界。

规则：

- command 注册集中在 `presentation/commands/registry.rs`，由 `register!`/`register_sync!` 宏生成分发表；命令不带 `#[tauri::command]` 属性。
- command 做参数解析、policy gate、错误映射和 service 调用，不写复杂业务流程。
- 分发入口是 `POST /__tt/invoke/{command}`；stream/事件走 `GET /__tt/stream/{stream_id}`、`GET /__tt/events`（SSE）。
- 对上游兼容的 `/api/*` 行为主要由前端 route shim 维持，Rust command 名不是第三方稳定 API。
- 浏览器子资源必须通过真实可加载的 URL/Response 语义提供（`GET /__tt/file`、host resource 路由），不用 IPC/base64 假装资源加载。
...
| 新 command | `rusttavern/src/presentation/commands`（经 `register!`/`register_sync!` 注册） | 调 service，不直接操作仓储细节 |
| 新 service/adapter 装配 | `rusttavern/src/app/composition` | 显式构造，避免 DI 容器或自动注册魔法 |
| 需要 HTTP server / platform API | `rusttavern/src/server`、`app` 或 `infrastructure` | 不下沉到 adapter |
...
| LAN/TT Sync runtime | `tt-adapter-sync` | 事件适配器留 host composition |
| Data Archive import/export executor | `tt-adapter-archive` | 浏览器下载兜底在 host composition |

## 10. 添加后端能力的最小流程

新增能力时，先从契约和变化原因出发，再决定文件位置：

1. 判断这是上游兼容行为，还是 RustTavern 新能力。
2. 上游兼容行为保持 `/api/*`、资源路径、stream 和事件语义。
3. 新能力优先走 `window.__RUSTTAVERN__.api.*`。
...
8. 在 `rusttavern/src/app/composition` 装配。
9. 在 `rusttavern/src/presentation` 增加 command（经 `register!`/`register_sync!` 注册）。
...
RustTavern 私有状态放在 `_tauritavern` 下，例如 agent workspace、agent profiles、skills、prompt cache、legacy extension source metadata、LLM connections。

权威代码入口：

- `crates/tt-adapter-storage-core/src/file_system.rs`
- `crates/tt-domain/src/models/user_directory.rs`
- 数据目录选择现状见 `docs/state/DataDirectorySelection.md`
...
| 主题 | 文档 |
| --- | --- |
| 前端 Host ABI、请求拦截、资源契约 | `docs/architecture/FrontendHostContract.md` |
| 前端集成结构 | `docs/architecture/FrontendGuide.md` |
| 扩展作者 API | `docs/api/README.md` |
| Chat payload（完整历史、原子提交、只读分页） | `docs/contracts/ChatPayload.md` |
| Third-party extension 资源和发现 | `docs/state/ThirdPartyExtensions.md` |
| 媒体 Range / browser resource contract | `docs/contracts/MediaAssetContract.md` |
| Logging / Dev observability | `docs/state/LoggingObservability.md` |
| Native provider API formats | `docs/state/NativeApiFormats.md` |
| Sync | `docs/state/Sync.md` |
| Agent 总览 | `docs/architecture/agent/AgentArchitecture.md` |
| Agent 细节 | `docs/architecture/agent/README.md` |
...
cargo test --manifest-path Cargo.toml -p <affected-crate> <focused-test>
```
...
- 不要让 adapter 依赖 `rusttavern`、`AppState` 或 presentation command。
- 不要把 HTTP-free 文件仓储塞回 host infrastructure。
- 不要把 `tt-adapter-storage-core` 变成新的 common 大桶。
- 不要为了两个 ZIP 调用提前抽一个泛化 `common-archive` crate；等第三个真实 bounded context 出现再说。
- 不要把 Rust command 名当作扩展公共 API。
