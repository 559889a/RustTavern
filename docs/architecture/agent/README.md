# RustTavern Agent Detail Docs

本目录保存 Agent 框架的细节设计文档。高层入口仍保留在 `docs/` 根目录：

1. `docs/architecture/agent/AgentArchitecture.md`：系统边界、分层、数据流。
2. `docs/architecture/agent/AgentContract.md`：不可破坏的不变量与 fail-fast 约束。
3. `docs/history/AgentImplementPlan.md`：当前实施基线、下一步计划与验收命令。
...
实时开发进度不写在本目录；请更新 `docs/state/AgentFramework.md`。

当前已落地 canonical model IR、provider native metadata 保真、invocation-scoped provider state、canonical Tool Control Plane、上下文与 workspace 工具循环、Agent Profile、独立 Preset/Model、return-mode SubAgent 和 foreground handoff。真实能力边界以 `docs/state/AgentFramework.md` 为准；专题文档只解释各自契约，不单独维护进度副本。
