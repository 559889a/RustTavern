# RustTavern Agent Handoff Runtime
...
对 RustTavern 来说，handoff 的价值不是抢占式调度，而是让创作者定义不同身份、提示词、preset、模型和工具权限的 Agent，并让这些 Agent 按写作阶段接力。已经 commit 过的 Agent 仍允许 handoff；最终 Agent 可以继续修订、再次 commit 并 `workspace.finish`。
...
```text
crates/tt-application/src/services/agent_runtime_service/loop_runner.rs
```
...
```text
crates/tt-application/src/services/agent_runtime_service/executor.rs
...
```text
crates/tt-application/src/services/agent_tools/agent/descriptors.rs
crates/tt-application/src/services/agent_tools/agent/mod.rs
...
```text
crates/tt-application/src/services/agent_runtime_service/prompt_assembly.rs
src/host/main/api/agent-prompt-assembly-bridge.js
src/host/main/api/agent-prompt-assembly.js
```

Profile validation：

```text
crates/tt-application/src/services/agent_profile_service/validation.rs
...
crates/tt-application/src/services/agent_profile_service/tests.rs
crates/tt-application/src/services/agent_tools/registry.rs
```
