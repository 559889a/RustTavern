# RustTavern Agent Run Event Journal
...
以下小节同时包含当前已落地事件和后续阶段设计事件；实现新事件时必须更新 `docs/state/AgentFramework.md`。
...
Handoff 的 task / invocation 结构、prompt brief 与失败边界见 `docs/architecture/agent/Handoff.md`。
...
```js
const unsubscribe = await window.__RUSTTAVERN__.api.agent.subscribe(runId, event => {});
```
