# RustTavern Agent MCP and Skill
...
当前状态：尚未实现 `window.__RUSTTAVERN__.api.mcp`，MCP 也未贡献到 Agent Tool Catalog。Skill 已落地 `window.__RUSTTAVERN__.api.skill`，并通过 builtin `skill.list` / `skill.search` / `skill.read` 接入现有控制面。当前 Skill 细节以 `docs/architecture/agent/Skill.md` 与 `docs/api/Skill.md` 为准。
...
它们不应自动覆盖 RustTavern preset，也不应绕过创作者 policy。
...
工具权限、MCP server、预算与可见性不由 Skill frontmatter 直接授予；这些能力必须进入 profile/policy/approval 体系。RustTavern 专属 metadata 可放在可选 `agents/rusttavern.json`，schema 无效时应 fail-fast。
