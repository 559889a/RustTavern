# RustTavern Agent Testing Strategy
...
特别要覆盖 `docs/state/NativeApiFormats.md` 中的契约：
...
```text
window.__RUSTTAVERN__.api.agent exists after ready
window.__RUSTTAVERN__.api.skill exists after ready
window.__RUSTTAVERN__.api.mcp exists after ready when MCP Host ABI lands
...
- 后端 `cargo check --manifest-path Cargo.toml` 通过。
- 后端 `cargo test --manifest-path Cargo.toml agent_runtime_service` 通过。
- 后端 `cargo test --manifest-path Cargo.toml -p tt-adapter-storage-userdata file_agent_repository` 通过。
- 后端 `cargo test --manifest-path Cargo.toml -p tt-adapter-storage-userdata file_agent_profile_repository` 通过。
