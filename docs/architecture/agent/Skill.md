# RustTavern Agent Skill

本文档记录当前已落地的 Agent Skill 能力。长期开发以本文、`docs/state/AgentFramework.md`、`docs/architecture/agent/ToolSystem.md` 与 `docs/api/Skill.md` 为准。
...
- `agents/rusttavern.json` 可选；一旦存在，schema 无效就 fail-fast。
...
- Host ABI：`window.__RUSTTAVERN__.api.skill`。
...
- `agents/rusttavern.json` schema 无效。
- 同名冲突但没有用户决策。
