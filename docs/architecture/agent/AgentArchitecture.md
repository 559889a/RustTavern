# RustTavern Agent Architecture

本文档定义 RustTavern Agent 系统的总体架构。它不是功能清单，而是后续实现必须遵守的架构地图。

配套阅读顺序：

1. `docs/state/AgentFramework.md`：当前真实实现状态与手动 smoke。
2. `docs/architecture/agent/AgentArchitecture.md`：系统边界、分层、数据流。
3. `docs/architecture/agent/AgentContract.md`：不可破坏的不变量与 fail-fast 约束。
4. `docs/history/AgentImplementPlan.md`：当前实施基线、后续顺序与验收命令。
5. `docs/architecture/agent/Workspace.md`：Workspace、Artifact、Checkpoint 的存储语义。
6. `docs/architecture/agent/RunEventJournal.md`：Run Event、状态机、恢复/取消语义。
7. `docs/architecture/agent/ProfilesAndPreset.md`：Preset、Agent Profile、Plan Policy。
8. `docs/architecture/agent/ToolSystem.md`：Tool Registry、Tool Result、权限与审批。
9. `docs/architecture/agent/LlmGateway.md`：provider-agnostic LLM gateway 与现有 ChatCompletionService 的复用边界。
10. `docs/architecture/agent/Skill.md`：当前 Skill 格式、存储、导入导出、Agent tool 与安全边界。
11. `docs/architecture/agent/McpSkill.md`：MCP 与 Skill 的边界。
...
15. `docs/architecture/agent/TestingStrategy.md`：测试矩阵与回归守护。
...
- 后端已经采用 workspace crate 边界：`tt-application` 依赖 `tt-ports/tt-contracts/tt-domain`，concrete IO 位于 `tt-adapter-*`，host crate（axum HTTP 服务器）只负责 composition、commands 与 host-bound infrastructure。见 `docs/architecture/BackendStructure.md`。
- 应用服务由 `AppState` 管理并在 `composition::build_services()` 装配。见 `crates/rusttavern/src/app.rs`、`crates/rusttavern/src/app/composition.rs`。
- 当前 LLM 请求经过 `ChatCompletionService`，该服务负责 provider 解析、iOS policy、endpoint override policy、payload build、prompt caching 和取消注册。见 `crates/tt-application/src/services/chat_completion_service/mod.rs:32`、`crates/tt-application/src/services/chat_completion_service/mod.rs:302`、`crates/tt-application/src/services/chat_completion_service/mod.rs:358`。
- 当前 LLM API 日志依赖 composition root 中装配的 `LoggingChatCompletionRepository` wrapper；Agent 不得直接调用 `HttpChatCompletionRepository` 绕过日志、secret 或 policy。Responses WebSocket 建连已复用 `HttpClientPool` 的 ChatCompletion WebSocket profile，不应扩散成第二套 LLM 调用链。见 `crates/rusttavern/src/app/composition/repositories.rs`。
- 当前聊天遵循完整 payload 保存契约；`ChatService` / `GroupChatService` 通过正式 repository 边界提交，前端保存经过统一队列。tail/before 是与写入解耦的只读历史查询。见 `docs/contracts/ChatPayload.md`。
- 前端 Public ABI 的统一入口是 `window.__RUSTTAVERN__`，应保持小而稳定。见 `docs/architecture/FrontendHostContract.md` 第 3.6 节、`src/host/main/bootstrap.js:139`。
...
前端保持旧 Generate 前置语义
  ↓
window.__RUSTTAVERN__.api.agent.startRunFromLegacyGenerate()
  ↓
...
- Agent `provider_state` 已用于 run-scoped continuation；OpenAI Responses 通过 persistent WebSocket、incremental input 与 `previous_response_id` 续接。详见 `docs/contracts/AgentProviderState.md`。
...
```text
  ...
  crates/
    ...
    rusttavern/src/infrastructure/
      apis/
      bundled_resources.rs
      paths.rs

    rusttavern/src/presentation/
      commands/
        agent_commands.rs
        mcp_commands.rs
```
...
- `tt-domain` 定义纯模型与纯规则；repository/gateway/runtime trait 位于 `tt-ports`。两者都不依赖 HTTP 服务器框架、tokio process、浏览器或 HTTP client implementation。
...
详见 `docs/architecture/agent/Workspace.md`。
...
详见 `docs/architecture/agent/ToolSystem.md`。
...
```js
window.__RUSTTAVERN__.api.agent
```

MCP 直连能力必须挂在：

```js
window.__RUSTTAVERN__.api.mcp
```

禁止新增散落的 `window.__RUSTTAVERN_AGENT_*` 全局函数，除非是临时调试且不进入 Public Contract。
...
- 第三方扩展依赖的事件仍按旧语义触发。
- `window.__RUSTTAVERN__.api.chat` 不因 Agent 引入而改变。
...
SillyTavern 上游的事件和 chat message 结构仍是兼容层的基础。Agent metadata 必须放在 `extra` 的 RustTavern namespace 下，新增字段必须向后兼容，不能让旧 UI/扩展读取消息时崩溃。
