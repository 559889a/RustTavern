...
禁止在 provider payload builder 中硬编码 RustTavern Agent 的 prompt 结构。
...
现有前端保存队列见 `src/script.js`。完整链路见 `docs/contracts/ChatPayload.md`。
...
- `installHostAbi()` 创建 `window.__RUSTTAVERN__`。
...
- 新增公共 `window.__RUSTTAVERN_AGENT_*` 散落全局。
- 让第三方直接依赖 Rust command 名。
- 把内部 service object 暴露给 `window.__RUSTTAVERN__.api`。
...
Agent commit 到 chat message 时，metadata 必须放入 `extra` 下的 RustTavern namespace，建议：
