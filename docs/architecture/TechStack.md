...
本文档说明 RustTavern 当前采用的主要技术、架构约束和工程守护。它不是完整依赖清单；精确版本以 `package.json`、`Cargo.toml` 和各 crate 的 `Cargo.toml` 为准。

## 1. 项目定位

RustTavern 将 SillyTavern 1.18.0 前端移植为浏览器 WebUI，并用 Rust HTTP 服务器后端替代上游 Node/Express 后端。终端启动一条命令后，任意设备浏览器即可访问。

核心取舍：

- 保留上游前端体验、数据格式、资源路径和扩展可观察行为。
- 后端采用 Rust workspace crate 拆分，Clean Architecture 是最高层架构约束。
- 前端通过宿主注入层（Host Kernel）拦截 `fetch` 与 `jQuery.ajax`，把上游同源请求转到本地 Rust HTTP 服务器。
- 新的 RustTavern 扩展能力优先暴露在 `window.__RUSTTAVERN__.api.*`，不污染上游 `/api/*` 兼容层。
...
| axum (0.8) | HTTP 服务器、路由、SSE 事件/流、graceful shutdown |
| tower / tower-http | 中间件（认证/白名单/CSRF）、静态文件/SPA fallback |
| clap | CLI 参数（`--host`/`--port`/`--data-root`/`--config`） |
| serde_yaml | `config.yaml` 解析 |
...
| crate | 职责 |
| --- | --- |
| `rusttavern` | HTTP server host（axum）、commands、composition root、host-bound infrastructure |
...
详细边界以 `docs/architecture/BackendStructure.md` 为准。

## 4. Rust 依赖分组

| 依赖 | 主要用途 |
| --- | --- |
| `axum` / `tower` / `tower-http` | HTTP 服务器、中间件、静态文件/SPA fallback |
| `clap` / `serde_yaml` | CLI 与 `config.yaml` |
| `serde` / `serde_json` | DTO、SillyTavern 兼容数据格式 |
...
当前主存储不是 SQLite。用户数据以 SillyTavern 兼容文件布局为主，必要的 RustTavern 私有状态放在 `_tauritavern` 下。
...
RustTavern 自己维护的前端集成层位于 `src/host/main/*`，按 `context/kernel/services/adapters/routes` 拆分：
...
- RustTavern 新能力优先走 `window.__RUSTTAVERN__.api.*`。
- Rust command 名是内部实现细节，不是扩展作者的稳定 API。
- 浏览器子资源使用真实 URL/Response 语义，不用 base64 伪装资源加载。

## 7. 数据与兼容性

RustTavern 的数据兼容目标是浏览器、扩展和用户数据可观察的 SillyTavern 语义，而不是 Node/Express 内部实现。
...
数据目录选择与当前状态见 `docs/state/DataDirectorySelection.md`。
...
- 终端启动：`rusttavern(.exe) [--host 0.0.0.0] [--port 8000] [--data-root <dir>] [--config <file>]`
- 任意设备浏览器访问 WebUI（默认 `http://127.0.0.1:8000`）。
- 分发：`scripts/build-server.mjs` 产出 zip（binary + `src/` + `default/` + `src/scripts/templates/`），见 `release/RustTavern-{platform}-{arch}.zip`。
- Linux：AppImage / DEB / RPM，以及从源码构建的 Nix flake（`nix/package.nix`）。
- flatpak：`packaging/flatpak/`（图标暂缺——已知限制）。

数据目录：默认 `<exe_dir>/data`（dev 下为仓库根 `data/`）；可用 `--data-root` 覆盖，`config.yaml` 的 `dataRoot` 字段优先级其次。

## 10. 相关文档

| 主题 | 文档 |
| --- | --- |
| 后端 Clean Architecture 与 crate 边界 | `docs/architecture/BackendStructure.md` |
| 前端集成结构 | `docs/architecture/FrontendGuide.md` |
| Host ABI / 请求拦截 / 资源契约 | `docs/architecture/FrontendHostContract.md` |
| 当前实现状态 | `docs/state/README.md` |
| Linux 分发与 Nix | `docs/state/LinuxRepository.md` |
| 扩展 API | `docs/api/README.md` |
| Agent 架构 | `docs/architecture/agent/AgentArchitecture.md` |
