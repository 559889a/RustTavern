...
本文档描述 RustTavern 当前前端（基于 SillyTavern 1.18.0）在浏览器 WebUI（HTTP 服务器 + 宿主注入层）形态下的集成架构与开发方式。

宿主层对外契约清单见：`docs/architecture/FrontendHostContract.md`（重构时优先保障其不回归）。

## 1. 目标与原则

- **最小侵入**：尽量保持上游 SillyTavern 前端行为不变。
- **模块化**：将宿主注入逻辑拆分为独立模块，避免单文件膨胀。
- **低耦合**：路由注册、请求拦截、业务上下文分离。
- **入口收敛**：统一走 `init.js -> host-main.js -> tauri/main/*`，减少重复入口。

## 2. 启动链路

当前前端启动顺序如下：

1. `src/init.js` 动态导入：`lib.js` -> `host-main.js` -> `script.js`
2. `src/lib.js` 静态导入 `src/dist/lib.core.bundle.js`，统一提供 ESM 导出；`highlight.js` 通过 `getHljs()` 按需加载
3. `src/host-main.js` 仅调用 `bootstrapTauriMain()`（薄入口）
4. `src/host/main/bootstrap.js` 负责：
   - 创建运行上下文（`context`）
   - 注册前端路由（`router + routes/*`）
   - 安装请求拦截器（`fetch` 与 `jQuery.ajax`）
   - 安装平台 ABI：`window.__RUSTTAVERN__`（小而稳定的宿主对外接口）
   - 安装同源窗口下载桥（浏览器式导出）
   - 安装移动端浏览器兼容层（runtime polyfills + geometry firewall + surface classifier，仅移动端浏览器）
   - 为宿主接管的路由响应注入追踪 header：`x-rusttavern-trace-id`
   - 初始化 bridge 与目录信息
...
```text
src/
├── host-bridge.js            # 低层 bridge：invoke over HTTP/SSE、listen、createChannel、convertFileSrc
├── host-main.js              # 新入口：只做 bootstrap
├── tauri/
│   └── main/
...
│       ├── services/          # 有状态能力（assets/thumbnails/characters…）
...
- 维护对第三方可见的宿主 ABI（`window.__RUSTTAVERN__`）与请求追踪 header。

### 4.2 `context.js`

- `context.js` 仅作为兼容入口（避免外部 import 路径变化）。
- 真实实现位于 `src/host/main/context/index.js`：作为 Host Kernel facade 组装 `kernel + services + adapters`。
- `safeInvoke` 具备可配置的 invoke 策略（dedupe / write-behind / TTL cache），集中在 `src/host/main/kernel/invokes/invoke-policies.js`。
- Host 侧已知的 Rust 命令名收敛为类型：`src/host/main/kernel/invokes/host-commands.js`（`TauriInvokeCommand`）。
- 与第三方直接交互的全局符号（如缩略图 helpers）属于 Public Contract（见 `docs/architecture/FrontendHostContract.md`）。

#### 4.2.1 `window.__RUSTTAVERN__.api.chat`（扩展/记忆类插件 API）

> 这是 RustTavern 独有 API 的**唯一入口**（刻意不做 alias），为扩展提供有界历史读取、后端定位、检索与持久化能力；它增强上游完整 `chat[]` 契约，不替代该契约。

- 安装位置：`src/host/main/api/chat.js`（在 `src/host/main/bootstrap.js` 中安装到 `window.__RUSTTAVERN__.api.chat`）。
...
- 处理同源窗口中的浏览器式下载（如 `blob:` / `data:` / 同源 URL + `a[download]`）。
- 将命中的导出转接到浏览器下载链路（`file-export.js` 的 `downloadBlobWithRuntime`）。
...
3. 命中后交给路由分发到 `routes/*`
4. 路由通过 `context.safeInvoke(...)` 调用 Rust 命令（经 `src/host-bridge.js` 的 HTTP invoke：`POST /__tt/invoke/{command}`）
5. 返回标准 `Response` 给前端调用方

补充：

- `/csrf-token` 在 `system-routes.js` 中返回固定 token，用于通过前端初始化流程中的 CSRF 依赖检查。
- 所有宿主接管的路由响应都会附带 `x-rusttavern-trace-id`，用于将 DevTools Network 与 console/perf-hud 关联定位问题（header 名也可从 `window.__RUSTTAVERN__?.traceHeader` 获取）。
...
- 统一入口：上游只 import `src/scripts/chat-payload-transport.js`，不要直接依赖 `src/scripts/app/chat/*`。
...
完整现状见 `docs/contracts/ChatPayload.md`。
...
- 外部 renderer 通过 `window.__RUSTTAVERN__.api.chatSurface.registerParticipant()` 接入，不直接控制投影，也不依赖伪造消息事件。
...
- `chat_virtualization_enabled=false` 是默认值并保持上游 Show More 语义；RustTavern Settings 的“聊天 DOM 虚拟化”开关直接绑定这个布尔值并在保存后 reload。用户显式开启后，experimental bounded 使用 TanStack 几何提交 `V ∪ T`，true tail 常驻且 `.mes` 上界为 `32 + 1`。
...
- 所有生效切换必须经 `src/scripts/power-user.js` 的统一入口；宿主 appearance adapter 不模拟 `#themes` 的 DOM change 事件。
...
- 将宿主专属逻辑限制在独立 runtime 子模块，减少与上游同步冲突。
...
- `src/host/main/compat/mobile/mobile-runtime-compat.js`：移动端旧浏览器运行时 polyfills（补齐旧浏览器缺失 JS API）。
- `src/host/main/compat/mobile/mobile-overlay-surface-admission.js`：移动端旧浏览器第三方 fixed overlay admission（分类 + 契约输出）。
- `src/host/main/compat/mobile/mobile-overlay-compat-controller.js`：移动端旧浏览器 overlay compat controller（观察 + 有界 settle window，暴露 `window.__RUSTTAVERN_MOBILE_OVERLAY_COMPAT__`）。
- `src/host/main/compat/mobile/mobile-iframe-viewport-contract-bridge.js`：Same-origin iframe 的 viewport/inset contract bridge（`viewport-host` 边界变量同步）。
- `src/scripts/extensions/runtime/resource-paths.js`：扩展资源路径规范化与 third-party 判定。
- `src/scripts/extensions/runtime/host-ready.js`：等待 `__RUSTTAVERN_MAIN_READY__`，避免 bridge 未就绪时提前加载。
- `src/scripts/extensions/runtime/third-party-runtime.js`：第三方扩展样式兼容层（旧浏览器下为样式 URL 附加 `ttCompat=layer`，触发 Rust 端点做 `@layer` 展平；不再走前端预取/Blob 注入）。
...
4. 当扩展为 `third-party/*` 时：
   ...
   - CSS 仅在旧浏览器不支持 `@layer` 时由 runtime 附加 `ttCompat=layer` query；由 Rust 端点返回展平后的 CSS bytes（否则仍走原始 URL）。
5. `/scripts/extensions/third-party/*` 由 Rust 协议层端点提供（HTTP host resource 路由），统一返回 bytes + `Content-Type` + 404 语义。
...
- 面向持续开发的现状说明见 `docs/state/ThirdPartyExtensions.md`；涉及实现边界或改动前，先读该文档，再决定是改前端 runtime 还是改后端资源端点。

### 7.4 契约与约束

- third-party 扩展命名约定为 `third-party/<folder>`，前后端均按该约定解析。
- 扩展命令参数统一使用 camelCase（如 `extensionName`），避免 invoke 参数缺失。
- 客户端版本检查仍遵循上游格式：`SillyTavern:<version>:RustTavern`，用于 `minimum_client_version` 判断。
...
- 新增插件加载能力时，优先扩展 `src/scripts/extensions/runtime/*`，不要把宿主细节回灌到 `extensions.js`。
- 新增插件 API 时，优先在 `src/host/main/routes/extensions-routes.js` 封装，再通过 `context.safeInvoke()` 调 Rust 命令。
- 若调整 third-party 静态资源路径约定，必须同时更新 `resource-paths.js` 与 Rust 协议层端点的前缀解析逻辑。

### 7.7 移动端插件兼容（新增）

#### 7.7.1 JS 运行时兼容（移动端旧浏览器）

- 实现位置：`src/host/main/compat/mobile/mobile-runtime-compat.js`。
- 入口：`src/host/main/bootstrap.js` 中安装（仅移动端旧浏览器，兼容保留）。
- 行为：仅补齐缺失 API，且只执行一次。
- 当前按需补齐：
  - `Array.prototype.at`
  - `String.prototype.at`
...
#### 7.7.2 CSS `@layer` 降级（移动端旧浏览器）

- 实现位置：`src/scripts/extensions/runtime/third-party-runtime.js`（样式加载链路）。
- 触发条件：
  - 样式内容包含 `@layer`；
  - 当前浏览器不支持 CSS Cascade Layers。
- 处理方式：
  - runtime 将样式 URL 改写为 `...?ttCompat=layer`；
  - Rust 协议层端点识别该 query 并移除 `@layer` 包裹（展平层级），返回可被旧浏览器直接应用的 CSS。
...
- 实现位置：
  - 分类/契约输出：`src/host/main/compat/mobile/mobile-overlay-surface-admission.js`
  - 观察与有界 settle window：`src/host/main/compat/mobile/mobile-overlay-compat-controller.js`
  - 同源 iframe bridge：`src/host/main/compat/mobile/mobile-iframe-viewport-contract-bridge.js`
- 入口：`src/host/main/bootstrap.js` 中安装（仅移动端浏览器，兼容保留）。
...
- 若看到 `*.at is not a function`：
  - 检查是否为移动端旧浏览器会话，并确认 `window.__RUSTTAVERN_MOBILE_RUNTIME_COMPAT__ === true`。
- 若插件样式错乱但 CSS 已成功请求：
  - 优先检查是否命中 `@layer` 降级分支；
  - 关注 `resolveStylesheetUrl()` 是否返回带 `ttCompat=layer` 的 URL。
- 若脚本弹窗贴顶到状态栏：
  - 检查脚本是否通过 `<style>` 或行内 `style` 设置了固定定位顶边；
  - 检查 `window.__RUSTTAVERN_MOBILE_OVERLAY_COMPAT__` 是否已安装。
...
- 安装入口：`src/host/main/services/embedded-runtime/install.js`
  - `bootstrap.js` 在 main ready 后按 bootstrap mirror 决定是否加载；在 `APP_READY` 后安装 chat adapters。
- Manager 与 profiles：`src/host/main/services/embedded-runtime/*`
  - 全局调试入口：`globalThis.__RUSTTAVERN_EMBEDDED_RUNTIME__`
  - 配置来源：`rusttavern-settings.embedded_runtime_profile`
...
- DOM detectors：`src/host/main/adapters/embedded-runtime/*-runtime-adapter.js`
  - 已支持：JS-Slash-Runner（`.TH-render`）与 LittleWhiteBox（`.xiaobaix-iframe-wrapper`）。
- 消息写入 facade：`src/scripts/app/message/mes-text-write.js`
  - 上游调用点统一依赖 facade；`off` 时直接恢复普通 `.mes_text` HTML 写入语义。
- 渲染事务：`src/host/main/adapters/embedded-runtime/message-render-transaction.js`
...
更多“当前如何工作/哪些契约不能破坏/回归点”见：`docs/state/EmbeddedRuntime.md`。

## 8. 兼容层策略

- `src/host-main.js`：新主入口（推荐）。
- 新开发统一集中在 `src/host/main/*`，避免重复实现与多处注入链路并存。

## 9. 如何新增一个宿主注入接口

1. 在 Rust 后端新增/确认命令（`crates/rusttavern/src/presentation/commands/*`），并在 `registry.rs` 中经 `register!`/`register_sync!` 注册（经 `POST /__tt/invoke/{command}` 分发）。
2. 若宿主层会调用该命令，将命令名加入 `src/host/main/kernel/invokes/host-commands.js`（`TauriInvokeCommand`，避免字符串拼写漂移）。
3. 若该命令为高频/可合并写入的调用，按需在 `src/host/main/kernel/invokes/invoke-policies.js` 增加/调整策略（dedupe / write-behind）。
4. 在 `src/host/main/routes/` 对应业务域中新增路由：路由层禁止直接引用 `window`，需要浏览器能力时下沉到 `adapters/` 或 `services/`。
...
1. `pnpm run check`
2. `pnpm run dev`（rspack watch + cargo run 并行，浏览器访问 `http://127.0.0.1:8000`）
3. 启动后确认：
   - 首屏加载正常
   - 不再出现 CSRF 初始化错误
   - 角色/聊天/设置等核心接口可用
...
- 启用（需 reload 才能抓启动打点）：
  ...
  - 或 URL：`?ttPerf=1`
- 启用后等待就绪：`await window.__RUSTTAVERN_PERF_READY__`
- 常用导出命令：
  - `window.__RUSTTAVERN_PERF__.downloadReport()` 下载 JSON（便于交给 AI 分析）
  - `window.__RUSTTAVERN_PERF__.exportJson({ includeResources: true })` 直接拿到 JSON 字符串
  - `await window.__RUSTTAVERN_PERF__.copyReport()` 复制到剪贴板（若可用）
- HUD 操作：拖动标题栏移动（位置持久化），点击标题栏展开/收起；桌面端可用 `Ctrl+Alt+P` 切换开关。

## 11. 工程守护（Guardrails + 类型检查）

目标：把宿主层（`src/host/main/*`）限制在**可长期维护**的规模与依赖形态，避免再次回到单文件膨胀与隐式耦合。
...
- Guardrails（`scripts/check-frontend-guardrails.mjs`）：
  ...
  - 路由契约：`src/host/main/routes/*` 禁止直接引用 `window`（需要触碰浏览器/DOM/上游 ST 时，新增 `adapters/*`）。
