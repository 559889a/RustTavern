# `window.__RUSTTAVERN__.api.llmConnections` — LLM Connection API

本 API 是前端/扩展侧管理 Agent 可引用 LLM 连接定义的 Host ABI。它只暴露稳定 DTO，不暴露 Rust repository、文件路径或 Rust command 名。
...
const llmConnections = window.__RUSTTAVERN__.api.llmConnections;
```
...
- Rust command 名 `list_llm_connections` / `save_llm_connection` 等属于 Internal 实现细节，不是 Public Contract。
