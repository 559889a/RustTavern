# `window.__RUSTTAVERN__.api.skill` — Skill API
...
const skill = window.__RUSTTAVERN__.api.skill;
```
...
type RustTavernSkillApi = {
  list(options?: { scope?: RustTavernSkillScopeFilter }): Promise<RustTavernSkillIndexEntry[]>;
...
`RustTavernSkillScope` 分为 `global` / `preset` / `profile` / `character`。未显式传入 scope 的旧无归属操作按 `global` 处理。
