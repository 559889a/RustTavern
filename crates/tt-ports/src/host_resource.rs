...
/// Whole-body async reader over a host resource's bytes.
pub type HostResourceReader = Box<dyn tokio::io::AsyncRead + Send + Unpin>;

pub trait HostResourceBody: Send {
    fn read(self: Box<Self>, range: Option<ByteRange>) -> Result<Vec<u8>, HostResourceStoreError>;

    /// Whole-body reader, when this source can produce one without buffering.
    ///
    /// `None` means the implementation only yields bytes through `read` (derived
    /// variants, thumbnails, synthesized payloads); the caller then falls back to
    /// buffering. Ranged reads always go through `read`.
...
impl OpenedHostResource {
    ...
    /// Whole-body reader for the un-ranged GET, when the source has one.
    pub fn reader(&self) -> Option<HostResourceReader> {
        self.body.reader()
    }
