...
const TEST_USER_AGENT: &str = "RustTavern/test";

fn unique_temp_root() -> PathBuf {
    std::env::temp_dir().join(format!("rusttavern-extension-repo-{}", random::<u64>()))
}
...
#[test]
fn extension_identifier_rejects_folder_names_that_win32_resolves_to_the_parent() {
    // `...` / trailing dot / trailing space collapse to the extensions root on Windows, so a
    // recursive delete or rename on the resolved path would hit every installed extension.
    for identifier in [
        "third-party/...",
        "third-party/..",
        "third-party/ext.",
        ...
        "third-party/CON",
        "third-party/aux",
        "third-party/nested/ext",
        "third-party/",
    ] {
        assert!(
            super::parse_third_party_extension_folder_name(identifier).is_none(),
            "identifier must be rejected: {identifier}"
        );
    }

    assert_eq!(
        super::parse_third_party_extension_folder_name("third-party/my-ext").as_deref(),
        Some("my-ext")
    );
    assert_eq!(
        super::parse_third_party_extension_folder_name("third-party/my.ext.v2").as_deref(),
        Some("my.ext.v2")
    );
}
