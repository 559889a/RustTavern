...
/// Built-in extensions enabled in RustTavern.
...
fn is_valid_extension_folder_segment(segment: &str) -> bool {
    ...
    // Win32 strips trailing dots and spaces from a path component before it reaches the
    // filesystem, so a segment like `...` or `name.` resolves to the *parent* directory and a
    // recursive delete/rename would then hit the whole extensions root. Reuse the same portable
    // component validation the install path already applies to Git tree entries so that every
    // identifier-derived folder name is held to one rule (it also rejects DOS device names).
    self::git_worktree::validate_install_folder(segment).is_ok()
}
