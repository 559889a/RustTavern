...
/// Source paths currently being generated, so concurrent requests for the
/// same image don't each decode and re-encode it.
static THUMBNAIL_IN_FLIGHT: OnceLock<Mutex<HashSet<PathBuf>>> = OnceLock::new();

fn thumbnail_in_flight() -> &'static Mutex<HashSet<PathBuf>> {
    THUMBNAIL_IN_FLIGHT.get_or_init(|| Mutex::new(HashSet::new()))
}

/// Removes the source path from the in-flight set when generation finishes
/// (or fails), so the dedup marker can never leak.
struct ThumbnailGenerationGuard(PathBuf);

impl Drop for ThumbnailGenerationGuard {
    fn drop(&mut self) {
...
fn generate_thumbnail_sync(
    ...
    // Dedup: only one thread generates a given source at a time. A caller
    // that races another generation treats it like a cache miss and serves
    // the original; the next request hits the committed cache.
    {
        let mut in_flight = thumbnail_in_flight().lock().map_err(|_| {
            DomainError::InternalError("Thumbnail in-flight lock is poisoned".to_string())
        })?;
        if !in_flight.insert(original_path.to_path_buf()) {
            return Err(DomainError::Transient(format!(
                "Thumbnail generation already in flight for '{}'",
                original_path.display()
            )));
        }
    }
    let _generation_guard = ThumbnailGenerationGuard(original_path.to_path_buf());
