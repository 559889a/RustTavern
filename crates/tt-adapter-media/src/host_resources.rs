...
use tt_ports::host_resource::{
    HostResourceAssetStore, HostResourceBody, HostResourceReader, HostResourceSourceMetadata,
...
impl HostResourceBody for FileHostResourceBody {
    fn reader(&self) -> Option<HostResourceReader> {
        // Duplicate the handle we already hold rather than reopening the path: it is
        // the one whose metadata the response headers were derived from, and its read
        // cursor is still at 0 because nothing reads through `&self`.
    ...
    fn read(
        ...
        let Some(range) = range else {
            // Reserve the known length up front. Growing a default `Vec` for a large background
            // video or user file reallocates and copies repeatedly, so peak RSS lands well above the
            // file size. (Callers that can stream take `reader` instead; this is the fallback for
            // the paths that must materialise the bytes.)
            let capacity = usize::try_from(self.content_length).unwrap_or(0);
...
impl FilesystemHostResourceStore {
    ...
    fn open_thumbnail_asset(
        ...
        maybe_sweep_thumbnail_dir(thumbnail_root);
...
/// Sweep every N thumbnail opens: keeps each thumbnail directory bounded by
/// deleting the oldest files when the cap is exceeded (thumbnails are
/// regenerable on demand, so age-based eviction is safe).
const THUMBNAIL_SWEEP_EVERY_N_CALLS: u64 = 256;
const THUMBNAIL_DIR_MAX_FILES: usize = 1200;
static THUMBNAIL_SWEEP_COUNTER: AtomicU64 = AtomicU64::new(0);

fn maybe_sweep_thumbnail_dir(root: &Path) {
    let calls = THUMBNAIL_SWEEP_COUNTER.fetch_add(1, Ordering::Relaxed);
    if !calls.is_multiple_of(THUMBNAIL_SWEEP_EVERY_N_CALLS) {
        return;
    }

    let mut files = Vec::new();
    collect_thumbnail_files(root, &mut files);
    if files.len() <= THUMBNAIL_DIR_MAX_FILES {
        return;
    }
    files.sort_by_key(|(_, modified)| *modified);
    let excess = files.len() - THUMBNAIL_DIR_MAX_FILES;
    for (path, _) in files.into_iter().take(excess) {
        let _ = std::fs::remove_file(path);
    }
}

fn collect_thumbnail_files(dir: &Path, out: &mut Vec<(PathBuf, SystemTime)>) {
    ...
    for entry in entries.flatten() {
        ...
        if meta.is_dir() {
            collect_thumbnail_files(&path, out);
...
mod tests {
    ...
    impl TempDirGuard {
        fn new(test_name: &str) -> Self {
            ...
            path.push(format!("rusttavern-{test_name}-{}", uuid::Uuid::new_v4()));
