...
/// Max time without any data from the upstream stream. A provider that keeps
/// the connection open but stops sending would otherwise pin the task and its
/// stream buffer forever (connect timeout does not apply to an established
/// connection).
        ...
        loop {
            ...
            let chunk = tokio::select! {
                ...
                _ = tokio::time::sleep(STREAM_IDLE_TIMEOUT) => {
                    let failure = crate::http_error::idle_timeout_failure(&endpoint);
                    tracing::warn!(
                        provider = provider_name,
                        operation = "stream",
                        code = %failure.code,
                    ...
                    return Err(DomainError::upstream_failure(failure));
                }
            };
