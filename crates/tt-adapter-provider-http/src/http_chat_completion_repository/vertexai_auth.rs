...
/// Upper bound on cached service account authenticators. Cache keys are
/// content hashes of caller-supplied service account JSON; an unbounded map
/// would let repeated configuration changes pin authenticators (each holding
/// a full network stack) forever. The legitimate set is tiny, so a clear on
/// overflow is a cheap one-time rebuild.
const SERVICE_ACCOUNT_CACHE_MAX_ENTRIES: usize = 8;
...
pub(super) async fn get_service_account_access_token(
    ...
    let cached = match cached {
        ...
        None => {
            ...
            if cache.len() >= SERVICE_ACCOUNT_CACHE_MAX_ENTRIES && !cache.contains_key(&cache_key) {
                cache.clear();
            }
