...
/// Sessions idle for longer than this are closed and removed on the next
/// pool access. A run that ends without reaching the explicit close path
/// (client disconnect, host wait timeout) would otherwise pin its WebSocket
/// connection forever.
const WS_SESSION_IDLE_TIMEOUT: Duration = Duration::from_secs(30 * 60);
/// Upper bound on concurrently open WebSocket sessions; when full, the
/// least-recently-used session is evicted on the next access.
const MAX_WS_SESSIONS: usize = 8;

#[derive(Default)]
pub(super) struct ResponsesWsSessionPool {
    sessions: Mutex<HashMap<String, Arc<ResponsesWsSessionEntry>>>,
}

/// Pool entry for one WebSocket session.
///
/// Only the socket sits behind the async mutex. A generation holds that lock for
/// the whole model response (tens of seconds to minutes), so the pool's
/// bookkeeping — the idle sweep, LRU eviction and the connection-key check —
/// must be answerable without taking it. Doing otherwise blocked those passes
/// while they held the pool lock, which queued every other `session()` and
/// `close()` (including the idle sweep and run-timeout cleanup) behind an
/// in-progress generation.
struct ResponsesWsSessionEntry {
    /// Immutable for the entry's lifetime: a config change retires the entry
    /// rather than mutating it.
    connection_key: String,
    session: Mutex<ResponsesWsSession>,
    last_used: std::sync::Mutex<Instant>,
}

impl ResponsesWsSessionEntry {
    fn last_used(&self) -> Instant {
        *self.last_used.lock().expect("ws session clock poisoned")
    }

    fn touch(&self) {
        *self.last_used.lock().expect("ws session clock poisoned") = Instant::now();
    }
}
...
/// Closes a retired session. Always called with the pool lock released.
async fn close_pooled_session(
    session_id: &str,
    entry: Arc<ResponsesWsSessionEntry>,
    reason: &'static str,
) {
    let close_result = entry.session.lock().await.close().await;
...
impl ResponsesWsSessionPool {
    async fn session(
        ...
        endpoint_path: &str,
        session_id: &str,
    ) -> Result<Arc<ResponsesWsSessionEntry>, DomainError> {
        ...
        // Phase 1: all pool bookkeeping under the pool lock, with no `.await`
        // on a session lock inside it.
        let mut retired: Vec<(String, Arc<ResponsesWsSessionEntry>, &'static str)> = Vec::new();
        let reusable = {
            let mut sessions = self.sessions.lock().await;
            let now = Instant::now();

            // Drop sessions idle past the timeout so a run that never reaches
            // the close path cannot pin sockets forever.
            let idle: Vec<String> = sessions
                .iter()
                .filter(|(_, entry)| {
                    now.saturating_duration_since(entry.last_used()) > WS_SESSION_IDLE_TIMEOUT
                })
                .map(|(id, _)| id.clone())
                .collect();
            for id in idle {
                if let Some(entry) = sessions.remove(&id) {
                    retired.push((id, entry, "idle"));
                }
            }

            let existing = sessions.get(session_id).cloned();
            let reusable = match existing {
                Some(entry) if entry.connection_key == connection_key => {
                    entry.touch();
                    Some(entry)
                }
                Some(_) => {
                    // Connection settings changed: retire the stale socket.
                    if let Some(entry) = sessions.remove(session_id) {
                        retired.push((session_id.to_string(), entry, "config changed"));
                    }
                    None
                }
                None => None,
            ...
            // Keep the table bounded by evicting the least-recently-used entry.
            while sessions.len() >= MAX_WS_SESSIONS {
                ...
                match sessions.remove(&oldest_id) {
                    Some(entry) => retired.push((oldest_id, entry, "evicted")),
                    None => break,
                }
            }
        ...
        for (id, entry, reason) in retired {
            close_pooled_session(&id, entry, reason).await;
        }
        ...
        // Phase 2: connect with the pool lock released, so a slow handshake does
        // not serialize the whole pool.
        let socket = connect_responses_ws(client, config, endpoint_path).await?;
        let entry = Arc::new(ResponsesWsSessionEntry {
            connection_key,
            session: Mutex::new(ResponsesWsSession { socket }),
        ...
        if let Some(replaced) = replaced {
            // A concurrent request connected for the same id; close the socket
            // being displaced instead of leaking it.
            close_pooled_session(session_id, replaced, "replaced").await;
        }
    ...
    pub(super) async fn close(&self, session_id: &str) {
        ...
        if let Some(entry) = entry {
            close_pooled_session(session_id, entry, "closed").await;
        }
    }
}
...
async fn generate_persistent_ws(
    ...
    // A long generation must not look idle to the pool's sweep.
    entry.touch();
