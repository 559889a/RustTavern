...
pub(super) async fn list_models(
    ...
    // since RustTavern wants to surface the entire catalog (with a
...
fn rusttavern_bedrock_metadata(id: &str) -> Value {
    ...
    if let Some(items) = foundation.get("modelSummaries").and_then(Value::as_array) {
        for item in items {
            ...
            entries.push(json!({
                ...
                "tauritavern": rusttavern_bedrock_metadata(id),
            }));
        }
    }
...
fn require_supported_family(spec: &BedrockModelSpec) -> Result<(), DomainError> {
    ...
    let reason = spec
        .unsupported_reason()
        .unwrap_or("This Bedrock model family is not wired by RustTavern's built-in adapter yet.");
    Err(DomainError::InvalidData(format!(
        "AWS Bedrock model `{}` is not supported by RustTavern's built-in Bedrock adapter. {reason} Enable the custom template (`aws_bedrock_use_custom_template`) with response paths, or choose a supported Bedrock family.",
        spec.raw_id()
    )))
}
...
fn parse_next_message(buffer: &[u8]) -> Result<ParseStep, DomainError> {
    ...
    // `total_length` comes from the peer; without a cap a bogus 4 GiB frame makes
    // this buffer grow until the upstream stream ends.
    ...
    if total_length > MAX_FRAME_LEN {
        return Err(DomainError::InternalError(format!(
            "{BEDROCK_PROVIDER_NAME} stream returned an oversized EventStream frame ({total_length} bytes)",
        )));
    }
...
mod tests {
    ...
    fn merge_bedrock_models_lists_all_providers_and_tags_each_entry() {
        ...
        // ACTIVE inference profiles for any provider are kept; RustTavern
        ...
        assert_eq!(
            nova.pointer("/rusttavern/bedrock/family")
                .and_then(serde_json::Value::as_str),
            Some("amazon_nova")
        );
        assert_eq!(
            nova.pointer("/rusttavern/bedrock/supported")
                .and_then(serde_json::Value::as_bool),
            Some(true)
        );
        assert_eq!(
            nova.pointer("/rusttavern/bedrock/capabilities/stream")
                .and_then(serde_json::Value::as_bool),
            Some(true)
        );
        ...
        assert_eq!(
            titan
                .pointer("/rusttavern/bedrock/family")
                .and_then(serde_json::Value::as_str),
            Some("unsupported")
        );
        assert_eq!(
            titan
                .pointer("/rusttavern/bedrock/supported")
                .and_then(serde_json::Value::as_bool),
            Some(false)
        );
        assert!(
            titan
                .pointer("/rusttavern/bedrock/unsupportedReason")
                .and_then(serde_json::Value::as_str)
                .is_some()
        );
