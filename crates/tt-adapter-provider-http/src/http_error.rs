...
/// Idle-stream timeout failure: the upstream kept the connection open but
/// stopped sending data past the configured window.
pub(crate) fn idle_timeout_failure(endpoint: &Url) -> UpstreamFailure {
    UpstreamFailure::network(
        UPSTREAM_NETWORK_TIMEOUT,
        Some(sanitize_endpoint(endpoint)),
