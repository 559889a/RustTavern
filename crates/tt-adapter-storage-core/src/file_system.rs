...
impl DataDirectory {
    ...
    pub fn new(root: PathBuf) -> Self {
        ...
        let rusttavern = root.join("_tauritavern");
        let extension_sources = rusttavern.join("extension-sources");
    ...
    pub async fn initialize(&self) -> Result<(), DomainError> {
        ...
        self.create_directory(&self.rusttavern).await?;
...
/// Open `path` for write access and flush its data to disk. The extra open is
/// needed because callers drop their write handle before publishing.
async fn sync_file_data(path: &Path) -> Result<(), DomainError> {
...
pub async fn replace_file_with_fallback(
    ...
    // Sync the temp file's data before the rename swaps it into place: after
    // the rename the old target is gone, so a crash in between would leave
    // the target holding unflushed content with nothing to recover from.
    sync_file_data(temp_path).await?;
...
mod tests {
    ...
    fn unique_temp_root() -> PathBuf {
        use rand::random;
        std::env::temp_dir().join(format!("rusttavern-file-system-{}", random::<u64>()))
    }
