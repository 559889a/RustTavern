...
pub struct FileSettingsRepository {
    rusttavern_settings_file: PathBuf,
...
fn parse_tauritavern_settings(
    path: &Path,
    contents: &str,
) -> Result<RustTavernSettings, DomainError> {
    RustTavernSettings::from_json_str_with_compat(contents).map_err(|error| {
...
impl FileSettingsRepository {
    pub fn new(settings_dir: PathBuf) -> Self {
        let rusttavern_settings_file = settings_dir.join("tauritavern-settings.json");
    ...
    pub fn load_rusttavern_settings_sync(&self) -> Result<RustTavernSettings, DomainError> {
        if !self.rusttavern_settings_file.exists() {
            let default_settings = RustTavernSettings::default();
            self.save_tauritavern_settings_sync(&default_settings)?;
            return Ok(default_settings);
        }

        tracing::debug!(
            "Loading RustTavern settings from {}",
            self.rusttavern_settings_file.display()
        );

        let contents =
            std::fs::read_to_string(&self.rusttavern_settings_file).map_err(|error| {
                map_tauritavern_settings_read_error(&self.rusttavern_settings_file, error)
            })?;

        parse_tauritavern_settings(&self.rusttavern_settings_file, &contents)
    }
    ...
    fn save_tauritavern_settings_sync(
        ...
        write_json_file_sync(&self.rusttavern_settings_file, settings)
    }

    async fn ensure_directory_exists(&self) -> Result<(), DomainError> {
        if let Some(parent) = self.rusttavern_settings_file.parent()
            && !parent.exists()
        {
...
impl SettingsRepository for FileSettingsRepository {
    async fn save_tauritavern_settings(
        ...
        write_json_file(&self.rusttavern_settings_file, settings).await?;
        Ok(())
    }
    ...
    async fn load_tauritavern_settings(&self) -> Result<RustTavernSettings, DomainError> {
        ...
        let contents = fs::read_to_string(&self.rusttavern_settings_file)
            .await
            .map_err(|error| {
                map_tauritavern_settings_read_error(&self.rusttavern_settings_file, error)
            })?;
...
mod tests {
    ...
    async fn load_tauritavern_settings_reads_disk_each_time() {
        ...
        let _ = repository
            ...
            .await
            .expect("load default rusttavern settings");
        ...
        let second = repository
            .load_tauritavern_settings()
            .await
            .expect("load externally updated rusttavern settings");
        assert_eq!(
            second
                .updates
                .startup_popup
    ...
    #[test]
    fn load_rusttavern_settings_sync_creates_default_file() {
        let dir = TestDir::new();
        let repository = FileSettingsRepository::new(dir.path().to_path_buf());

        repository
            .load_rusttavern_settings_sync()
            .expect("load default rusttavern settings synchronously");
