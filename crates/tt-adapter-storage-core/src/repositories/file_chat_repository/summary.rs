...
use tt_domain::models::chat::{
    normalize_chat_file_stem, parse_message_timestamp_value, strip_jsonl_extension,
};
...
// Bumped to 2 when `SearchFingerprint::hash_trigram` stopped using SipHash: the
// persisted bits are only comparable within one hash function, so an index written
// by the old build must be discarded rather than mixed with new entries.
...
impl SearchFingerprint {
    ...
    /// Mix a trigram into a 64-bit hash.
    ///
    /// The signature is a 4096-bit set, so the hash only has to spread trigrams
    /// evenly across it. Spinning up a `DefaultHasher` (SipHash) and three
    /// `Hash::hash` calls per trigram costs an order of magnitude more than the
    /// multiply-xor below, and this runs once per character of every scanned chat.
    ///
    /// Changing this function changes the persisted bits, so `INDEX_SCHEMA_VERSION`
    ...
    fn hash_trigram(chars: [char; 3]) -> u64 {
        ...
        // Final avalanche: the low bits pick the bit index and would otherwise
        // stay biased towards the last character.
        hash ^= hash >> 32;
...
impl FileChatRepository {
    ...
    /// File stem of the character's most recently updated chat, or `None` when
    /// the character has no chat on disk.
    ///
    /// Ordering uses the same last-message date the chat list is sorted by, so
    /// a card whose stored pointer is unusable resolves to the chat a client
    /// would pick. Every read goes through the summary cache the chat stats
    /// scan already populates.
    pub async fn latest_character_chat_stem(
        &self,
        character_name: &str,
        ...
        .await?;
        let chat_dir = self.chats_dir.join(dir_key);
        let files = list_files_with_extension(&chat_dir, "jsonl").await?;
        ...
        for path in files {
            ...
            let date = Self::get_chat_stats_date(&self.summary_cache, &descriptor).await?;
            let is_newer = match latest.as_ref() {
                Some((best_date, _)) => date > *best_date,
                None => true,
            };
            if is_newer {
                latest = Some((date, file_name));
            }
        }
        ...
        Ok(latest.and_then(|(_, file_name)| normalize_chat_file_stem(&file_name)))
    }
...
mod tests {
    ...
    async fn backup_summary_streams_zstd_and_rejects_raw_bytes_with_zstd_suffix() {
        let root =
            std::env::temp_dir().join(format!("rusttavern-zstd-summary-{}", random::<u64>()));
