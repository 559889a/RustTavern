...
impl FileChatRepository {
    ...
    pub(super) async fn record_current_content_signature(
        ...
        // Bounded like path_write_locks: one entry per chat ever written would
        // otherwise grow for the life of the process. This map is only a "we
        // wrote this, so the hash is known" cache — dropping entries costs a
        // re-hash on the next backup check, never correctness.
        const MAX_RETAINED_CONTENT_SIGNATURES: usize = 4096;

        let mut signatures = self.current_content_signatures.lock().await;
        if signatures.epoch != expected_epoch {
            return;
        }
        if signatures.entries.len() >= MAX_RETAINED_CONTENT_SIGNATURES {
            signatures.entries.clear();
        }
