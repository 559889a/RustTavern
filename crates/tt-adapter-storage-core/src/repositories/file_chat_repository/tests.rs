...
use tt_ports::repositories::chat_payload_commit_repository::{
    ChatPayloadBaseline, ChatPayloadCommitRepository, ChatPayloadTarget, CommittedChatPayload,
};
...
use super::chat_payload_commit::{CHAT_COMMIT_SESSION_TTL, MAX_ACTIVE_CHAT_COMMIT_SESSIONS};

fn unique_temp_root() -> PathBuf {
    std::env::temp_dir().join(format!("rusttavern-chat-repo-{}", random::<u64>()))
}
...
#[tokio::test]
async fn chat_commit_rejects_a_stale_baseline_instead_of_overwriting() {
    let (repository, root) = setup_repository().await;
    let target = character_target("alice", "baseline");
    let payload = payload_to_jsonl(&payload_with_integrity("shared-slug"));

    // Client A would have loaded this file and captured its signature; in the
    // meantime client B commits a newer version of the same chat.
    let first = commit_payload_bytes(&repository, target.clone(), payload.as_bytes(), false)
        .await
        .expect("first commit");

    // A stale baseline (file changed after this client loaded it) must be
    // rejected: the integrity slug is identical on both sides and would have
    // let the older copy silently drop B's messages.
    let stale = ChatPayloadBaseline {
        size: first.size,
        mtime_sec: first.mtime_sec.saturating_sub(1),
    };
    let stale_session = repository
        .begin(target.clone(), false, Some(stale))
        .await
        .expect("begin stale commit");
    repository
        .append(&stale_session.session_id, 0, payload.as_bytes())
        .await
        .expect("append stale payload");
    assert!(matches!(
        repository
            .finish(&stale_session.session_id, payload.len() as u64)
            .await,
        Err(DomainError::Conflict(_))
    ));

    // A baseline matching the file on disk commits and reports the published
    // signature so the client can adopt it without a re-read.
    let matching = ChatPayloadBaseline {
        size: first.size,
        mtime_sec: first.mtime_sec,
    };
    let matching_session = repository
        .begin(target, false, Some(matching))
        .await
        .expect("begin matching commit");
    repository
        .append(&matching_session.session_id, 0, payload.as_bytes())
        .await
        .expect("append payload");
    let second = repository
        .finish(&matching_session.session_id, payload.len() as u64)
        .await
        .expect("commit with matching baseline");
    assert!(second.mtime_sec >= first.mtime_sec);
...
async fn forced_chat_commit_still_rejects_an_invalid_header() {
    ...
    let session = repository
        .begin(character_target("alice", "session"), true, None)
        .await
...
async fn same_target_sessions_are_complete_and_last_finish_wins() {
    ...
    let session_a = repository
        ...
        .await
        .expect("begin a");
    let session_b = repository.begin(target, false, None).await.expect("begin b");
    repository
...
#[tokio::test]
async fn stale_chat_commit_sessions_are_reaped_before_the_limit_applies() {
    let (repository, root) = setup_repository().await;
    let target = character_target("alice", "stale");
    ...
    // Clients that vanished mid-upload: the sessions sit in the map and nothing
    // will ever finish or abort them.
    {
        let sessions = repository.chat_commit_sessions.lock().await;
        assert_eq!(sessions.len(), MAX_ACTIVE_CHAT_COMMIT_SESSIONS);
        for session in sessions.values() {
            let mut entry = session.lock().await;
            entry.created_at = Instant::now() - CHAT_COMMIT_SESSION_TTL - Duration::from_secs(1);
        }
    }
    ...
    let replacement = repository
        .begin(target, false, None)
        .await
        .expect("stale sessions should be reaped instead of blocking the cap");

    let staged = std::fs::read_dir(&repository.chat_commit_staging_dir)
        ...
        .count();
    assert_eq!(staged, 1, "only the live session should keep a stage file");

    repository
        .abort(&replacement.session_id)
        .await
        .expect("abort replacement session");
    let _ = fs::remove_dir_all(root).await;
}
...
async fn non_c2_mutation_fails_closed_and_group_uses_the_same_guard() {
    ...
    let write_guard = repository
        ...
        .await;
    repository
        .write_payload_to_path_locked(&write_guard, &current_path, &payload, false)
        .await
