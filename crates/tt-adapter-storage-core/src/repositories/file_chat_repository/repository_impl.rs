...
impl ChatRepository for FileChatRepository {
    ...
    async fn save_with_options(&self, chat: &Chat, force: bool) -> Result<(), DomainError> {
        ...
        if let Some(file_name) = &chat.file_name {
            ...
            self.write_chat_file_locked(&write_guard, &path, chat, force)
                .await?;
    ...
    async fn add_message(
        ...
        // Read-modify-write under the payload mutation lock: reading outside
        // it let two concurrent add_message calls resolve from the same base
        // and the second save silently dropped the first message.
        let path = self
            .resolve_character_chat_path(character_name, file_name)
            .await?;
        let write_guard = self.acquire_payload_mutation_lock(&path).await;
        ...
        self.write_chat_file_locked(&write_guard, &path, &chat, false)
            .await?;
        self.remove_summary_cache_for_path(&path).await;
    ...
    async fn set_character_chat_metadata_extension(
        ...
        // Computed before the write, mirroring the commit path: the header
        // rewrite bypasses the payload write path, so the cached Chat must be
        // dropped afterwards or a cached read would resurrect the old header
        // on the next read-modify-write (add_message).
        let cache_key = self.get_cache_key(character_name, file_name)?;
        let path = self
            .resolve_character_chat_path(character_name, file_name)
            .await?;
