...
use tt_ports::repositories::chat_payload_commit_repository::{
    ChatPayloadBaseline, ChatPayloadCommitBegin, ChatPayloadCommitRepository, ChatPayloadTarget,
    CommittedChatPayload,
};
use uuid::Uuid;
...
/// How long an untouched upload session may sit before `begin` reaps it. Long
/// enough for a slow upload of a large chat, short enough that an abandoned
/// session does not outlive the user's patience.
pub(super) const CHAT_COMMIT_SESSION_TTL: Duration = Duration::from_secs(600);
...
pub(super) struct CommitSession {
    ...
    /// When this upload started; only `begin`'s reaping sweep reads it.
...
impl FileChatRepository {
    ...
    async fn strict_publish_chat_payload(
        ...
        // Through the shared helper, like every other atomic write in this crate:
        // it fsyncs the stage before the swap and falls back to copy+remove when
        // rename is unreliable (Android external app storage), failing fast on an
        // ambiguous post-error state. The bare rename here aborted the whole save
        // instead, and left the stage unsynced.
        crate::file_system::replace_file_with_fallback(stage_path, target_path)
            .await
            .map_err(|error| {
                tracing::error!(
...
impl ChatPayloadCommitRepository for FileChatRepository {
    async fn begin(
        ...
        // Reap sessions whose client vanished mid-upload (tab closed, navigation,
        // crash) before judging the cap below. Nothing else ever removes them, so
        // without this sweep eight abandoned uploads fill the cap for the life of
        // the process and every later save fails with Conflict until a restart.
        let now = Instant::now();
        let mut expired_session_ids = Vec::new();
        sessions.retain(|session_id, session| {
            // A session whose guard cannot be taken right now is mid-append and
            // therefore alive; only an idle session can be judged stale.
            let is_stale = session
                .try_lock()
                .is_ok_and(|entry| now.duration_since(entry.created_at) > CHAT_COMMIT_SESSION_TTL);
            if is_stale {
        ...
        for expired_session_id in expired_session_ids {
            let stage_path = self
                .chat_commit_staging_dir
                .join(format!("{expired_session_id}.partial"));
            self.remove_chat_commit_stage(&stage_path).await;
        }
    ...
    async fn finish(
        ...
        let result = async {
            ...
            // Sync before the rename publishes the staged file: without this,
            // a crash right after the rename can leave the target holding
            // unflushed content while the old file is already gone.
            file.sync_all().await.map_err(|error| {
                DomainError::InternalError(format!(
                    "Failed to sync chat commit session {session_id}: {error}"
                ))
            })?;
            drop(file);
            ...
            if !force {
                if let Some(baseline) = baseline {
                    // Concurrent-editor guard: the integrity slug never
                    // rotates, so two clients editing from the same loaded
                    // copy would otherwise pass the slug check below and the
                    // later save would silently drop the earlier one's
                    // messages. A missing file has nothing to overwrite and
                    // is allowed through.
                    match fs::metadata(&target_path).await {
                        Ok(metadata) => {
                            let mtime_sec = metadata
                                ...
                                .map(|d| d.as_secs())
                                .unwrap_or(0);
                            if metadata.len() != baseline.size || mtime_sec != baseline.mtime_sec
                            {
                                return Err(DomainError::Conflict(format!(
                                    "Chat integrity conflict: {} changed since it was loaded \
                                     (expected {} bytes / mtime {}, found {} bytes / mtime {}). \
                                     Reload the chat before saving.",
                                    target_path.display(),
                                    baseline.size,
                                    baseline.mtime_sec,
                                    metadata.len(),
            ...
            let published_mtime_sec = fs::metadata(&target_path)
                .await
                .ok()
...
mod tests {
    ...
    async fn chat_commit_missing_stage_never_guesses_success_from_target_size() {
        let root = std::env::temp_dir().join(format!(
            "rusttavern-chat-strict-publish-{}",
            Uuid::new_v4()
        ));
