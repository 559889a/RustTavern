...
impl FileChatRepository {
    ...
    /// `write_payload_to_path_locked` for callers already holding the payload
    /// mutation lock for `path` (the guard argument is proof — it cannot be
    /// constructed otherwise).
    pub(super) async fn write_payload_to_path_locked(
        &self,
    ...
    /// `write_chat_file` for callers already holding the payload mutation
    /// lock for `path` (the guard argument is proof — it cannot be
    /// constructed otherwise).
    pub(super) async fn write_chat_file_locked(
        &self,
        ...
        self.write_payload_to_path_locked(write_guard, path, &objects, force)
            .await?;
