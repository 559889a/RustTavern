...
impl FileChatRepository {
    ...
    pub(super) async fn update_character_chat_store_json_value(
        ...
        let dir = self
            .resolve_character_chat_store_dir(character_name, file_name, namespace)
            .await?;
        // Read-merge-write under a lock: two concurrent updates to the same
        // entry would otherwise both read the old value and the later write
        // would silently drop the earlier merge.
        let _write_guard = self.acquire_payload_mutation_lock(&dir).await;
    ...
    pub(super) async fn update_group_chat_store_json_value(
        ...
        let dir = self
            .resolve_group_chat_store_dir(chat_id, namespace)
            .await?;
        // See the character variant: read-merge-write needs the lock.
        let _write_guard = self.acquire_payload_mutation_lock(&dir).await;
        update_store_json_entry(&dir, &key, value).await
    }
