    ...
    T: Serialize + ?Sized,
{
    ...
    let result = write_temp_then_replace(&tmp_path, path, &contents).await;
    if result.is_err() {
        // The temp name carries a fresh UUID, so a leftover is never reused or overwritten: without
        // this the sync state directory would gain one invisible file per failed write, forever.
        let _ = tokio::fs::remove_file(&tmp_path).await;
    }
    result
}

async fn write_temp_then_replace(
    tmp_path: &Path,
    path: &Path,
    contents: &[u8],
) -> Result<(), DomainError> {
...
mod tests {
    ...
    fn temp_root() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("rusttavern-sync-json-{}", Uuid::new_v4()))
    }
