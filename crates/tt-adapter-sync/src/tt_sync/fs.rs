...
mod tests {
    ...
    fn unique_temp_root() -> PathBuf {
        std::env::temp_dir().join(format!("rusttavern-tt-sync-{}", random::<u64>()))
    }
    ...
    fn scan_manifest_excludes_all_sync_state_files() {
        ...
        assert!(
            ...
            "agent profiles are part of the RustTavern default dataset"
        );
        assert!(
            paths.contains(
            ...
            "agent persistent state is part of the RustTavern default dataset"
        );
        assert!(
