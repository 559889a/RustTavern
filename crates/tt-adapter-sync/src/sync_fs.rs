...
pub(crate) async fn write_file_atomic(
    ...
    let result = write_temp_then_rename(path, &tmp_path, data, modified_ms).await;
    if result.is_err() {
        // The temp name embeds the target's extension and is never reused, so leaving it behind
        // would accumulate invisible garbage in the user's data tree on every failed transfer.
...
async fn write_temp_then_rename(
    ...
    // `flush()` only drains the Rust-side buffer. The incremental planner compares just
    // (size, mtime) — a file whose metadata survived a crash but whose bytes did not would match
    // forever and never be re-fetched, so the content must be durable before the rename.
    file.sync_all()
        .await
        .map_err(|error| FileMutationError::unchanged(SyncError::Internal(error.to_string())))?;
    drop(file);
    ...
    tokio::task::spawn_blocking(move || set_file_modified_ms(&tmp_owned, modified_ms))
        .await
        .map_err(|error| FileMutationError::unchanged(SyncError::Internal(error.to_string())))?
        .map_err(FileMutationError::unchanged)?;
...
mod tests {
    ...
    fn unique_temp_root() -> std::path::PathBuf {
        use rand::random;
        std::env::temp_dir().join(format!("rusttavern-sync-fs-{}", random::<u64>()))
    }
