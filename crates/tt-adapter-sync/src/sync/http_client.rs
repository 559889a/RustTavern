...
/// Deadline for a sync control-plane request (status, session open, pairing, pull-request notify).
///
/// `SyncClientOptions` exposes no timeout and reqwest defaults to none, so a peer that accepts the
/// connection and then stops responding would leave these awaits pending forever — stalling the
/// sync automation loop, which awaits its jobs inline. Bulk transfers are not covered here; they
/// are guarded by the progress watchdog in `job_executor`.
pub(crate) const SYNC_CONTROL_REQUEST_TIMEOUT: Duration = Duration::from_secs(30);

pub(crate) async fn with_control_timeout<F, T>(label: &str, request: F) -> Result<T, DomainError>
where
    F: Future<Output = Result<T, DomainError>>,
{
    match tokio::time::timeout(SYNC_CONTROL_REQUEST_TIMEOUT, request).await {
        Ok(result) => result,
        Err(_) => Err(DomainError::Transient(format!(
            "{label} did not respond within {} seconds",
            SYNC_CONTROL_REQUEST_TIMEOUT.as_secs()
        ))),
    }
}
...
impl SyncHttpClient {
    ...
    pub async fn pair_complete(
        ...
        with_control_timeout("TT-Sync pairing", async {
            self.inner
    ...
    pub async fn status(&self) -> Result<StatusResponse, DomainError> {
        with_control_timeout("Sync status request", async {
    ...
    pub async fn open_session(
        ...
        with_control_timeout("Sync session open", async {
            self.inner
