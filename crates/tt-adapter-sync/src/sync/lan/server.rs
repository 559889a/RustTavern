...
pub async fn spawn_lan_sync_server(
    ...
    status.server = "rusttavern-lan".to_string();
...
mod tests {
    ...
    fn temp_default_user_dir() -> PathBuf {
        std::env::temp_dir().join(format!("rusttavern-lan-server-{}", Uuid::new_v4()))
    }
    ...
    async fn pull_plan_and_file_download_use_session_and_dataset_scope() {
        ...
        let workspace = Arc::new(RustTavernSyncWorkspace::new(target_root.clone(), None));
        let mut options =
