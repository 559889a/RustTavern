    ...
    fn from(error: DomainError) -> Self {
        match error {
            ...
            DomainError::WorkspaceWriteConflict { path, kind } => {
                // An optimistic-concurrency conflict is a 409 the caller can
                // retry after re-reading, not a malformed request. Keep `path`
                // so the client can say which file lost the race.
                CommandError::Conflict(format!("Workspace write conflict at {path}: {kind}"))
            }
        }
    }
}
