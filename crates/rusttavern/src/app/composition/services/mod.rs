...
pub(super) async fn build(
    ...
    let rusttavern_settings = &startup_profile.rusttavern_settings;
    let repositories = repositories::build(
        app_context,
        data_directory,
        rusttavern_settings.chat_backups,
    )
    .await?;
    let ios_policy = startup_profile.ios_policy.clone();

    let http_client_pool = app_context.http_client_pool.clone();
    ...
    let secret_service = Arc::new(SecretService::new(
        repositories.secret_repository.clone(),
        rusttavern_settings.allow_keys_exposure,
    ));
    ...
    let data_archive_service = archive::build(app_context, data_change_reconciler.clone());
