...
use tt_domain::models::settings::RustTavernSettings;

#[derive(Debug, Clone)]
pub(crate) struct StartupProfile {
    pub rusttavern_settings: RustTavernSettings,
...
impl StartupProfile {
    pub(crate) fn load(data_root: &Path) -> Result<Self, DomainError> {
        ...
        let rusttavern_settings = settings_repository.load_rusttavern_settings_sync()?;
        rusttavern_settings
            .chat_backups
            .validate()
        ...
        let raw_policy = if scope == IosPolicyScope::Ios {
            resolve_effective_raw_policy_sync(data_root, rusttavern_settings.ios_policy.as_ref())?
        } else {
            None
        };
...
mod tests {
    ...
    impl TestDir {
        fn new() -> Self {
            let path = std::env::temp_dir().join(format!(
                "rusttavern-startup-profile-test-{}-{}",
                std::process::id(),
    ...
    #[test]
    fn load_creates_default_rusttavern_settings_file() {
        let dir = TestDir::new();
