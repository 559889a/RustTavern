...
pub(crate) struct FilesystemDataArchiveFileGateway {
    runtime_paths: Arc<RuntimePaths>,
}

impl FilesystemDataArchiveFileGateway {
    pub(crate) fn new(runtime_paths: Arc<RuntimePaths>) -> Self {
        Self { runtime_paths }
    }
}

impl DataArchiveFileGateway for FilesystemDataArchiveFileGateway {
    fn prepare_incoming_import_archive_path(&self) -> Result<PathBuf, DomainError> {
        let incoming_dir = self.runtime_paths.archive_imports_root.join("incoming");
    ...
    fn prepare_user_backup_archive(
        ...
        let runtime_paths = self.runtime_paths.clone();
        let export_root = resolve_user_backup_export_root(&runtime_paths)?;
        fs::create_dir_all(&export_root).map_err(|error| {
            DomainError::InternalError(format!("Failed to create export directory: {}", error))
        })?;
        cleanup_stale_exports(&export_root, protected_paths);
    ...
    fn save_export(&self, archive_path: &Path, file_name: &str) -> Result<PathBuf, DomainError> {
        save_staged_archive_to_downloads(archive_path, file_name)
    }
    ...
    fn save_user_backup(
        ...
        let source_path =
            resolve_staged_user_backup_archive_path(&self.runtime_paths, archive_path)?;
        save_staged_archive_to_downloads(&source_path, file_name)
    }
...
/// Resolve a writable "downloads" directory for server-side archive saves.
/// Falls back to the archive exports root (always writable) when the OS
/// downloads directory cannot be determined.
fn resolve_download_dir() -> Result<PathBuf, DomainError> {
    ...
    // Fallback: keep the archive in the exports staging root.
...
fn candidate_user_backup_export_roots(
    ...
    roots.push(runtime_paths.archive_exports_root.clone());
...
fn resolve_user_backup_export_root(
    ...
    let roots = candidate_user_backup_export_roots(runtime_paths)?;
...
mod tests {
    ...
    fn temp_root(name: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "rusttavern-data-archive-adapters-{}-{}",
            name,
            Uuid::new_v4().simple()
        ))
    }
