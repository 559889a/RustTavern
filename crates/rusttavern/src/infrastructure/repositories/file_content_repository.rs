...
use crate::infrastructure::assets::{ResourceRoots, list_default_content_files_under};
...
impl FileContentRepository {
    ...
    ///
    /// * `resources` - resource roots for reading bundled default content
    ...
    pub fn new(resources: Arc<ResourceRoots>, data_root: PathBuf, user_content_dir: PathBuf) -> Self {
        Self {
            resources,
            data_root,
            user_content_dir,
    ...
    async fn seed_content_scope(
        ...
        for item in scoped_items {
            ...
            for resource_entry in resource_entries {
                ...
                self.resources
                    .copy_resource_to_file(&resource_path, &dest_path)
                    .await?;
            }
...
impl ContentRepository for FileContentRepository {
    ...
    async fn get_content_index(&self) -> Result<Vec<ContentItem>, DomainError> {
        let index_items: Vec<ContentIndexItem> =
            self.resources.read_json("default/content/index.json")?;
