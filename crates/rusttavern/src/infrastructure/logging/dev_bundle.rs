...
const BUNDLE_ROOT_DIR: &str = "rusttavern-dev-bundle";
...
pub fn export_dev_log_bundle(
    ...
    let output_dir = resolve_bundle_output_dir(runtime_paths)?;
...
#[cfg(target_os = "ios")]
fn resolve_bundle_output_dir(_runtime_paths: &RuntimePaths) -> Result<PathBuf, DomainError> {
    let temp_dir = std::env::temp_dir();
    Ok(temp_dir.join(IOS_EXPORT_STAGING_ROOT_NAME))
}

#[cfg(not(target_os = "ios"))]
fn resolve_bundle_output_dir(runtime_paths: &RuntimePaths) -> Result<PathBuf, DomainError> {
    // Server mode: bundles land in the log root so they are always writable
    // and easy to locate next to the logs they contain.
    Ok(runtime_paths.log_root.clone())
}

fn default_bundle_file_name() -> String {
    let ts = Utc::now().format("%Y%m%d-%H%M%S").to_string();
    let suffix = uuid::Uuid::new_v4().simple().to_string();
    format!("rusttavern-dev-bundle-{}-{}.zip", ts, &suffix[..8])
}
