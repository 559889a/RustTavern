...
impl ChatCompletionRepository for LoggingChatCompletionRepository {
    ...
    async fn generate_stream(
        ...
        // Raw SSE log writer with its own bounded queue: the stream must
        // never be backpressured (or accumulated in memory) by disk speed.
        // When the writer falls behind, raw chunks are dropped — the readable
        // text collector below still captures the full visible stream.
        let (log_tx, mut log_rx) = tokio::sync::mpsc::channel::<String>(32);
        ...
        let forward_task = tokio::spawn(async move {
            ...
            while let Some(chunk) = rx.recv().await {
                ...
                if log_tx.try_send(chunk).is_err() {
                    tracing::debug!("LLM API raw log queue full; dropping chunk");
                }
            }
