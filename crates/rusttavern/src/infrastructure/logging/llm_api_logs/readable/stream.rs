...
/// Max collected chars per buffer. A pathological stream (or a client that
/// never terminates) must not pin unbounded strings in memory while being
/// logged; the tail is truncated with an explicit marker.
const STREAM_READABLE_MAX_CHARS: usize = 256 * 1024;

/// Push `text` into `buffer`, truncating at the cap with a marker instead of
/// growing without bound.
fn push_limited(buffer: &mut String, text: &str) {
    if buffer.len() >= STREAM_READABLE_MAX_CHARS {
        return;
    }
    let remaining = STREAM_READABLE_MAX_CHARS - buffer.len();
...
impl StreamReadableCollector {
    ...
    fn push_openai_like(&mut self, trimmed: &str) {
        ...
        for choice in choices {
            ...
            if let Some(text) = delta.get("reasoning_content").and_then(Value::as_str) {
                push_limited(&mut self.reasoning_buffer, text);
            }
            ...
            if let Some(text) = delta.get("content").and_then(Value::as_str) {
                push_limited(&mut self.text_buffer, text);
            }
        }
    }
    ...
    fn push_cohere(&mut self, trimmed: &str) {
        ...
        if let Some(tool_plan) = message.get("tool_plan").and_then(Value::as_str) {
            push_limited(&mut self.text_buffer, tool_plan);
        }
    }
