...
/// Generic outbound requests (update checks and similar small calls). reqwest
/// applies no timeout of its own, so without these an unresponsive upstream
/// keeps the task and its buffers resident for the process lifetime.
pub const DEFAULT_CONNECT_TIMEOUT: Duration = Duration::from_secs(15);
pub const DEFAULT_REQUEST_TIMEOUT: Duration = Duration::from_secs(60);
/// Downloads (character cards, assets, skill imports) are larger but still
/// bounded; the byte limits live with the callers.
pub const DOWNLOAD_CONNECT_TIMEOUT: Duration = Duration::from_secs(20);
pub const DOWNLOAD_REQUEST_TIMEOUT: Duration = Duration::from_secs(10 * 60);
...
mod tests {
    ...
    fn clients_are_cached_per_profile() {
        ...
        pool.client(HttpClientProfile::Download).unwrap();
    ...
    fn git_blocking_builder_uses_product_user_agent() {
        ...
        assert!(
            request
                .lines()
                .any(|line| line.eq_ignore_ascii_case("user-agent: RustTavern/test"))
        );
        server.finish();
    }
