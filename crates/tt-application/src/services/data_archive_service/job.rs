...
/// How long a finished job stays queryable before the registry drops it. The
/// frontend polls job status until a terminal state; afterwards the entry is
/// only needed while its export artifact is still protected (see
/// `is_sweepable`).
...
impl DataArchiveJobRegistry {
    ...
    /// Drop finished jobs that are past their query window and no longer
    /// protect an export artifact. Lazy (called from `insert`/`get`) so the
    /// table stays bounded without a background task.
    fn sweep(&self) {
        ...
        let expired: Vec<String> = jobs
            .iter()
            .filter(|(_, job)| job.is_sweepable(TERMINAL_JOB_TTL))
            .map(|(id, _)| id.clone())
            .collect();
        for id in expired {
            jobs.remove(&id);
...
impl DataArchiveJobHandle {
    ...
    /// True when the job finished more than `ttl` ago and its export artifact
    /// (if any) has been disposed, so the registry can drop it. Export jobs
    /// keep their entry while the artifact path is still protected, even past
    /// the TTL.
    pub(crate) fn is_sweepable(&self, ttl: Duration) -> bool {
        let expired = match self.terminal_at.lock() {
        ...
        match self.export_artifact_path.lock() {
    ...
    fn update_status(
        ...
        let mut status = self
            ...
            .lock()
            .map_err(|_| DomainError::InternalError("Failed to lock job status".to_string()))?;
        let was_terminal = is_terminal_state(&status.state);
        update(&mut status);
        if !was_terminal
            && is_terminal_state(&status.state)
            && let Ok(mut terminal_at) = self.terminal_at.lock()
        {
...
fn is_terminal_state(state: &str) -> bool {
    state == DATA_ARCHIVE_STATE_COMPLETED
        || state == DATA_ARCHIVE_STATE_FAILED
        || state == DATA_ARCHIVE_STATE_CANCELLED
}
