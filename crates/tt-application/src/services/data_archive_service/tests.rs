...
fn service_resolves_completed_export_artifact() {
    ...
    job.mark_completed_export(
        "rusttavern-data.zip".to_string(),
        PathBuf::from("/tmp/rusttavern-data.zip"),
    )
    ...
    assert_eq!(artifact.file_name, "rusttavern-data.zip");
}

#[tokio::test]
async fn start_import_runs_executor_initializer_reconciler_and_cleanup() {
    let jobs = Arc::new(DataArchiveJobRegistry::new());
    let data_root = PathBuf::from("/tmp/rusttavern-data-root");
    let workspace_root = PathBuf::from("/tmp/rusttavern-import-workspace");
...
async fn start_import_with_no_local_mutation_skips_initializer_and_reconciler() {
    let jobs = Arc::new(DataArchiveJobRegistry::new());
    let data_root = PathBuf::from("/tmp/rusttavern-noop-data-root");
    let workspace_root = PathBuf::from("/tmp/rusttavern-noop-import-workspace");
    let files = Arc::new(RecordingFiles::with_import(ImportArchiveExecutionRequest {
        data_root,
        archive_path: PathBuf::from("/tmp/import.archive"),
        workspace_root,
...
async fn partial_import_failure_initializes_reconciles_and_reports_local_mutation() {
    let jobs = Arc::new(DataArchiveJobRegistry::new());
    let data_root = PathBuf::from("/tmp/rusttavern-partial-data-root");
    let workspace_root = PathBuf::from("/tmp/rusttavern-partial-import-workspace");
    let files = Arc::new(RecordingFiles::with_import(ImportArchiveExecutionRequest {
        data_root: data_root.clone(),
        archive_path: PathBuf::from("/tmp/import.archive"),
        workspace_root,
...
async fn partial_import_cancel_initializes_reconciles_and_reports_local_mutation() {
    let jobs = Arc::new(DataArchiveJobRegistry::new());
    let data_root = PathBuf::from("/tmp/rusttavern-partial-cancel-data-root");
    let files = Arc::new(RecordingFiles::with_import(ImportArchiveExecutionRequest {
        data_root: data_root.clone(),
        archive_path: PathBuf::from("/tmp/import.archive"),
        workspace_root: PathBuf::from("/tmp/rusttavern-partial-cancel-workspace"),
    }));
    let initializer = Arc::new(RecordingInitializer::default());
    let reconciler = Arc::new(RecordingReconciler::default());
    let local_applied = import_local_applied();
...
async fn partial_import_failure_reports_reconcile_error() {
    let jobs = Arc::new(DataArchiveJobRegistry::new());
    let data_root = PathBuf::from("/tmp/rusttavern-partial-reconcile-data-root");
    let files = Arc::new(RecordingFiles::with_import(ImportArchiveExecutionRequest {
        data_root,
        archive_path: PathBuf::from("/tmp/import.archive"),
        workspace_root: PathBuf::from("/tmp/rusttavern-partial-reconcile-workspace"),
    }));
    let local_applied = import_local_applied();
    let service = DataArchiveService::new(
        jobs,
...
async fn start_export_runs_executor_and_marks_completed() {
    let jobs = Arc::new(DataArchiveJobRegistry::new());
    let output_path = PathBuf::from("/tmp/rusttavern-data.zip");
    let files = Arc::new(RecordingFiles::with_export(ExportArchiveExecutionRequest {
        data_root: PathBuf::from("/tmp/data-root"),
        output_path: output_path.clone(),
        file_name: "rusttavern-data.zip".to_string(),
    }));
    let service = DataArchiveService::new(
        jobs,
        test_runtime_handle(),
        Arc::new(RecordingExecutor::export_ok("rusttavern-data.zip")),
        files,
        Arc::new(UnusedInitializer),
        Arc::new(UnusedReconciler),
    );
    ...
    assert_eq!(result.file_name.as_deref(), Some("rusttavern-data.zip"));
    assert_eq!(
...
async fn start_export_cleans_partial_archive_on_failure() {
    let jobs = Arc::new(DataArchiveJobRegistry::new());
    let output_path = PathBuf::from("/tmp/partial-rusttavern-data.zip");
    let files = Arc::new(RecordingFiles::with_export(ExportArchiveExecutionRequest {
        data_root: PathBuf::from("/tmp/data-root"),
        output_path: output_path.clone(),
        file_name: "rusttavern-data.zip".to_string(),
...
async fn start_export_protects_claimed_completed_artifact_from_stale_cleanup() {
    ...
    existing_job
        .mark_completed_export("rusttavern-data.zip".to_string(), existing_path.clone())
        .expect("mark completed export");
    existing_job
...
async fn save_export_marks_artifact_disposed_with_saved_path() {
    ...
    let saved_path = PathBuf::from("/Downloads/rusttavern-data.zip");
    let service = DataArchiveService::new(
        jobs,
        test_runtime_handle(),
        Arc::new(UnusedExecutor),
    ...
    assert_eq!(
        ...
        Some("/Downloads/rusttavern-data.zip")
    );
...
fn cleanup_export_marks_artifact_disposed_and_is_idempotent() {
    ...
    job.mark_completed_export("rusttavern-data.zip".to_string(), archive_path.clone())
        .expect("mark completed export");
    jobs.insert("job-1", job).expect("insert job");
