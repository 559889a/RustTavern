...
/// Upper bound on cached template texts. Template names are caller-supplied
/// (validated only for path segments), so an unbounded cache would let an
/// abusive caller pin arbitrary strings in memory forever. The legitimate
/// template set is far smaller; the cache is rebuilt cheaply after a clear.
const TEMPLATE_CACHE_MAX_ENTRIES: usize = 128;
...
impl BundledTemplateService {
    ...
    fn read_resource_text(&self, resource_path: &str) -> Result<String, DomainError> {
        {
            let mut cache = self.cache.lock().expect("bundled template cache lock poisoned");
            if let Some(content) = cache.get(resource_path).cloned() {
                return Ok(content);
            }
