...
use crate::dto::settings_dto::{
    ...
    UpdateAgentSettingsDto, UpdateChatBackupSettingsDto, UpdateRustTavernSettingsDto,
...
struct SettingsAggregateCacheEntry {
    ...
    /// When `signature` was last read from disk; see
    /// `current_sillytavern_settings_signature`.
    signature_checked_at: Instant,
}
...
impl SettingsService {
    ...
    pub async fn get_rusttavern_settings(
        &self,
    ) -> Result<RustTavernSettingsDto, ApplicationError> {
        tracing::debug!("Getting RustTavern settings");

        let settings = self.settings_repository.load_tauritavern_settings().await?;

        Ok(RustTavernSettingsDto::from(settings))
    }
    ...
    pub async fn update_rusttavern_settings(
        ...
        tracing::debug!("Updating RustTavern settings");
    ...
    /// The on-disk aggregate signature, reusing a recent one.
    ///
    /// Computing it opens settings.json and walks every preset, theme and
    /// lorebook file (twelve directories of them), and callers ask for the
    /// aggregate in bursts — boot, world-info refresh, quick replies. Only a
    /// change made by something other than this process can be missed, and only
    /// for this window; a save through the app clears the entry outright.
    async fn current_sillytavern_settings_signature(
        &self,
    ) -> Result<SettingsAggregateSignature, ApplicationError> {
        const SIGNATURE_REUSE_WINDOW: Duration = Duration::from_secs(3);

        if let Some(entry) = self.sillytavern_settings_cache.lock().await.as_ref()
            && entry.signature_checked_at.elapsed() < SIGNATURE_REUSE_WINDOW
        {
    ...
    pub async fn get_sillytavern_settings(
        ...
        let signature = self.current_sillytavern_settings_signature().await?;
    ...
    async fn build_sillytavern_settings_response(
        ...
        let (
            (settings_json, rusttavern_settings_revision),
            (
...
mod tests {
    ...
    use tt_domain::models::settings::{SettingsSnapshot, RustTavernSettings, UserSettings};
    ...
    #[tokio::test]
    async fn rusttavern_settings_update_applies_request_proxy_runtime() {
        ...
        let updated = service
            .update_rusttavern_settings(UpdateRustTavernSettingsDto {
    ...
    #[tokio::test]
    async fn rusttavern_settings_update_persists_chat_virtualization_switch() {
        let service = SettingsService::new(
    ...
    #[tokio::test]
    async fn rusttavern_settings_update_persists_and_applies_chat_backup_policy() {
        let repository = Arc::new(TestSettingsRepository::default());
        let backup_runtime = Arc::new(TestChatBackupRuntime::default());
        let service = SettingsService::new(
            repository,
    ...
    async fn compression_only_update_applies_and_schedules_format_reconciliation() {
        ...
        let update = UpdateRustTavernSettingsDto {
            chat_backups: Some(UpdateChatBackupSettingsDto {
                automatic_enabled: None,
                zstd_compression_enabled: Some(true),
                max_files_per_prefix: None,
        ...
        service
            .update_rusttavern_settings(update.clone())
            .await
        ...
        service
            .update_rusttavern_settings(update)
            .await
    ...
    async fn sillytavern_settings_aggregate_uses_cache_until_signature_changes() {
        ...
        assert_eq!(
            first.rusttavern_settings_revision.hash_algorithm,
            USER_SETTINGS_HASH_ALGORITHM
        );
        assert_eq!(
            first.rusttavern_settings_revision.settings_hash,
    ...
    struct TestSettingsRepository {
        settings: Mutex<RustTavernSettings>,
    ...
    impl SettingsRepository for TestSettingsRepository {
        async fn save_tauritavern_settings(
            &self,
            settings: &RustTavernSettings,
        ) -> Result<(), DomainError> {
            *self.settings.lock().await = settings.clone();
            Ok(())
        }

        async fn load_tauritavern_settings(&self) -> Result<RustTavernSettings, DomainError> {
