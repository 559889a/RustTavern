...
/// Wall-clock budget for one batch. `regress` is a backtracking engine and the
/// patterns come from imported character cards and presets, so a crafted
/// pattern can spin indefinitely. The budget guarantees the caller always gets
/// an answer.
const BATCH_TIME_BUDGET: Duration = Duration::from_secs(5);
/// Input caps applied before compiling anything. They reject the shapes that
/// make catastrophic backtracking reachable at all, which is cheaper and more
/// legible than trying to interrupt the engine mid-match.
const MAX_PATTERN_CHARS: usize = 4096;
const MAX_TEXT_BYTES: usize = 2 * 1024 * 1024;
const MAX_MATCHES_PER_SCRIPT: usize = 100_000;
...
impl NativeRegexService {
    ...
    pub async fn apply_batch(
        ...
        // The permit is held by this future, not by the blocking closure: a
        // pattern that outruns its budget leaves the worker thread spinning,
        // and if that thread also owned the permit the semaphore would stay
        // exhausted forever and every later batch would wait on it with no
        // error. Releasing here keeps the feature usable after a bad script.
        let _permit = self.jobs.clone().acquire_owned().await.map_err(|error| {
            ApplicationError::InternalError(format!("Native regex queue closed: {error}"))
        })?;
        let cache = Arc::clone(&self.cache);

        let job = tokio::task::spawn_blocking(move || apply_batch_blocking(cache, dto));

        match tokio::time::timeout(BATCH_TIME_BUDGET, job).await {
            Ok(joined) => joined.map_err(|error| {
                ApplicationError::InternalError(format!("Native regex task failed: {error}"))
            })?,
            Err(_) => Err(ApplicationError::ValidationError(format!(
                "Native regex batch exceeded its {} second budget; check the script patterns for catastrophic backtracking",
                BATCH_TIME_BUDGET.as_secs()
            ))),
        }
    }
}
...
fn apply_script(
    ...
    if script.pattern.chars().count() > MAX_PATTERN_CHARS {
        return Err(script_error(
            script,
            format!("pattern exceeds {MAX_PATTERN_CHARS} characters"),
        ));
    }
    ...
    if text.len() > MAX_TEXT_BYTES {
        return Err(script_error(
            script,
            format!("input text exceeds {MAX_TEXT_BYTES} bytes"),
        ));
    }
...
fn replace_matches(
    ...
    for mat in regex.find_iter(text) {
        ...
        match_count += 1;
        if match_count > MAX_MATCHES_PER_SCRIPT {
            return Err(script_error(
                script,
                format!("produced more than {MAX_MATCHES_PER_SCRIPT} matches"),
            ));
        }
    }
