...
pub(crate) struct TestSettingsRepository {
    rusttavern_settings: Mutex<RustTavernSettings>,
}

impl TestSettingsRepository {
    pub(crate) fn new() -> Arc<Self> {
        Arc::new(Self {
            rusttavern_settings: Mutex::new(RustTavernSettings::default()),
        })
    }

    pub(crate) async fn store_tauritavern_settings(&self, settings: RustTavernSettings) {
        *self.rusttavern_settings.lock().await = settings;
    }
}
...
impl SettingsRepository for TestSettingsRepository {
    ...
    async fn load_tauritavern_settings(&self) -> Result<RustTavernSettings, DomainError> {
        Ok(self.rusttavern_settings.lock().await.clone())
    }
