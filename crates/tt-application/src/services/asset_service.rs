...
/// Hosts allowed to serve imported content.
///
/// Shared with `content_service`: every server-side fetch driven by a
/// client-supplied URL goes through this list. Without it the server would
/// proxy arbitrary http/https addresses and hand the body back to the caller,
/// which reaches internal services the browser cannot.
