...
use crate::services::asset_service::is_import_host_whitelisted;
...
/// Upper bound for an imported character card. A PNG card with an embedded
/// lorebook stays well under this; without a bound the whole response is
/// buffered in memory, which a 1 GB machine cannot absorb.
const MAX_EXTERNAL_IMPORT_BYTES: usize = 32 * 1024 * 1024;
        ...
        let downloaded = self
            ...
            .fetch_bytes(
                ...
                Some(DownloadByteLimit {
                    label: "Imported character card",
                    max_bytes: MAX_EXTERNAL_IMPORT_BYTES,
                }),
            )
            .await?;
...
fn parse_external_import_url(raw: &str) -> Result<Url, ApplicationError> {
    ...
    // The server performs this fetch on the caller's behalf, so an arbitrary
    // host would turn the endpoint into a proxy into the machine's own network.
    // Same whitelist the asset import path already enforces.
    let host = url
        .host_str()
        .ok_or_else(|| ApplicationError::ValidationError("Import URL host is required".to_string()))?
        .to_ascii_lowercase();
    if !is_import_host_whitelisted(&host) {
        return Err(ApplicationError::ValidationError(format!(
            "Import host is not whitelisted: {host}"
        )));
    }
...
mod tests {
    ...
    async fn download_external_import_url_returns_png_payload() {
        ...
        let result = service
            .download_external_import_url("https://files.catbox.moe/share")
            .await
    ...
    #[tokio::test]
    async fn download_external_import_url_rejects_non_whitelisted_host() {
        let service = ContentService::new(
            Arc::new(TestContentRepository),
            Arc::new(TestExternalImportDownloader {
                bytes: vec![1, 2, 3],
        ...
        // An internal address must be refused before any request is issued.
        let error = service
            .download_external_import_url("http://169.254.169.254/latest/meta-data/")
            .await
            .expect_err("non-whitelisted host must be rejected");
        assert!(matches!(error, ApplicationError::ValidationError(_)), "got {error:?}");
    }
