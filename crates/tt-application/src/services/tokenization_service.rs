...
/// Fallback for an empty model name. Every name resolves to the same tokenizer
/// (see `MiktikTokenizerRepository::canonical_model`), so this only has to be a
/// non-empty string.
...
impl TokenizationService {
    ...
    pub async fn count_openai_tokens(
        ...
        // Tokenizing is synchronous CPU work; on the async runtime it would
        // stall one of only two worker threads for the length of the message.
        ...
        .await
        .map_err(|error| {
            ApplicationError::InternalError(format!("Token count task failed: {error}"))
        })??;
    ...
    pub async fn encode_openai_tokens(
        ...
        self.tokenizer_repository
            .ensure_model_ready(model.as_ref())
            .await?;
        // Encode plus the per-token decode pass is synchronous CPU work; keep it
        // off the async worker threads (only two of them exist).
        let tokenizer_repository = Arc::clone(&self.tokenizer_repository);
        let model = model.into_owned();
        let text = dto.text;
        let (ids, chunks) = tokio::task::spawn_blocking(move || {
            let ids = tokenizer_repository
                .encode(&model, &text)
                .map_err(ApplicationError::from)?;
            ...
            Ok::<_, ApplicationError>((ids, chunks))
        })
        .await
        .map_err(|error| {
            ApplicationError::InternalError(format!("OpenAI encode task failed: {error}"))
        })??;
    ...
    pub async fn decode_openai_tokens(
        ...
        let model = model.into_owned();
        let ids = dto.ids;
        let (text, chunks) = tokio::task::spawn_blocking(move || {
            let text = tokenizer_repository
                .decode(&model, &ids)
                .map_err(ApplicationError::from)?;
            ...
            Ok::<_, ApplicationError>((text, chunks))
        })
        .await
        .map_err(|error| {
            ApplicationError::InternalError(format!("OpenAI decode task failed: {error}"))
        })??;
