...
impl AgentRuntimeService {
    ...
    pub(super) async fn perform_host_chat_commit(
        ...
        let host_result = tokio::select! {
            ...
            _ = tokio::time::sleep(super::HOST_WAIT_TIMEOUT) => {
                self.active_chat_commits.write().await.remove(&commit_id);
                return Err(ApplicationError::Transient(format!(
                    "agent.chat_commit_timeout: host did not resolve commit `{commit_id}` within the wait window"
                )));
            }
        };
        ...
        let host_result = tokio::select! {
            ...
            _ = tokio::time::sleep(super::HOST_WAIT_TIMEOUT) => {
                self.active_persistent_state_metadata_updates
                    ...
                    .remove(&update_id);
                return Err(ApplicationError::Transient(format!(
                    "agent.persistent_state_metadata_update_timeout: host did not resolve update `{update_id}` within the wait window"
                )));
            }
        };
