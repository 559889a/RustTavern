    ...
    async fn finalize_agent_loop_run_result(
        ...
        match result {
            ...
            Err(ApplicationError::Cancelled(message)) => {
                ...
                report_finalization_failure(
                    run_id,
                    "cancel_unfinished_child_tasks",
                    self.cancel_unfinished_child_tasks(run_id).await,
                );
                report_finalization_failure(
                    run_id,
                    "finish_root_invocation",
                ...
                report_finalization_failure(
                    ...
                    self.transition_status(run_id, AgentRunStatus::Cancelled)
                        .await,
                );
                report_finalization_failure(
                    run_id,
            ...
            Err(error) => {
                ...
                if commit_ledger.is_empty() {
                    report_finalization_failure(
                        ...
                        self.transition_status(run_id, AgentRunStatus::Failed).await,
                    );
                    report_finalization_failure(
                        run_id,
                        "append_run_failed_event",
                        self.event(
                            run_id,
                    ...
                    report_finalization_failure(
                        run_id,
                        "transition_status(partial_success)",
                        self.transition_status(run_id, AgentRunStatus::PartialSuccess)
                            .await,
                    );
                    report_finalization_failure(
                        run_id,
                        "append_run_partial_success_event",
                        self.event(
                            run_id,
                            AgentRunEventLevel::Warn,
                            "run_partial_success",
    ...
    fn close_model_session_after_run(&self, run_id: String) {
        ...
        tokio::spawn(async move {
            let session_ids = match invocation_repository.list_invocations(&run_id).await {
                ...
                Err(error) => {
                    // Only the root session id is derivable without the invocation list, so any
                    // sub-agent / handoff sockets stay open until the pool's idle sweep reclaims
                    // them. That is bounded but not free, so report it rather than warn quietly.
                    tracing::error!(
                        target: tt_contracts::observability::USER_VISIBLE_ERROR,
                        "Agent run {run_id} could not enumerate invocations to close its model sessions ({error}); \
                         only the root session was closed and any sub-agent sessions will be reclaimed by the idle sweep"
                    );
...
/// Terminal cleanup deliberately continues past a failing step, but a discarded failure here
/// leaves the run in a non-terminal status forever: the timeline keeps waiting and the retention
/// planner (which only prunes terminal runs) never reclaims its workspace. Surface it instead.
fn report_finalization_failure<T>(run_id: &str, step: &str, result: Result<T, ApplicationError>) {
    if let Err(error) = result {
        tracing::error!(
            target: tt_contracts::observability::USER_VISIBLE_ERROR,
            "Agent run {run_id} could not complete finalization step '{step}': {error}"
        );
    }
}
