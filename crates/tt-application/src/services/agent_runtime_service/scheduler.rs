...
impl AgentTaskScheduler {
    ...
    async fn cancel_unfinished_tasks(
        ...
        // Cancel every task even if one write fails, and always wake the waiters. Returning early
        // used to leave the remaining tasks running and skip `notify_change`, so a parent blocked in
        // `agent.await` only unblocked when its own timeout expired (up to five minutes) while the
        // user already saw a cancel error.
