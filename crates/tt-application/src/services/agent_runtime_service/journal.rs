...
impl AgentRuntimeService {
    pub(super) async fn transition_status(
        ...
        // A terminal status is final. `cancel_run` awaits several repository calls before it writes
        // `Cancelling`, so without this guard a run that finished inside that window would be
        // pushed back to a non-terminal status that nothing can ever move again (the loop task is
        // gone and its handle has left `active_runs`), leaving the timeline waiting forever and the
        // retention planner unable to prune the run. Mirrors `finish_invocation`.
        if run.status.is_terminal() {
            tracing::debug!(
                "Ignoring agent run {} status transition to {:?}: already terminal ({:?})",
                run_id,
                status,
                run.status
            );
