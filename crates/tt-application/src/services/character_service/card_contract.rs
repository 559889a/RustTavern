...
fn sanitize_agent_profile_package_at_path(
    ...
    let Some(rusttavern) = extensions.get_mut("tauritavern") else {
        return Ok(());
    };
    let Some(rusttavern) = rusttavern.as_object_mut() else {
        return Err(DomainError::InvalidData(
            "Character payload extensions.tauritavern must be an object".to_string(),
        ));
    };
    let Some(package) = rusttavern.get_mut("agentProfiles") else {
        return Ok(());
    };
...
fn sanitize_agent_profile_package(package: &mut Value) -> Result<(), DomainError> {
    let Some(package) = package.as_object_mut() else {
        return Err(DomainError::InvalidData(
            "Character payload rusttavern.agentProfiles must be an object".to_string(),
        ));
    };
    if package.get("version").and_then(Value::as_u64) != Some(EMBEDDED_AGENT_PROFILES_VERSION) {
        return Err(DomainError::InvalidData(
            "Character payload rusttavern.agentProfiles.version must be 1".to_string(),
        ));
    }
    let Some(items) = package.get_mut("items") else {
        return Err(DomainError::InvalidData(
            "Character payload rusttavern.agentProfiles.items must be an array".to_string(),
        ));
    };
