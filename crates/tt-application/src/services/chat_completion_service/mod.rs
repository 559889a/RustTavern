...
use tt_domain::models::settings::{PromptCacheTtl, RustTavernSettings};
...
impl ChatCompletionService {
    ...
    pub fn register_stream(&self, stream_id: &str) -> watch::Receiver<bool> {
        self.active_streams.register(stream_id)
    }

    pub fn cancel_stream(&self, stream_id: &str) -> bool {
        self.active_streams.cancel(stream_id)
    }

    pub fn complete_stream(&self, stream_id: &str) {
        self.active_streams.complete(stream_id);
    }

    pub fn register_generation(&self, request_id: &str) -> watch::Receiver<bool> {
        self.active_generations.register(request_id)
    }

    pub fn cancel_generation(&self, request_id: &str) -> bool {
        self.active_generations.cancel(request_id)
    }

    pub fn complete_generation(&self, request_id: &str) {
        self.active_generations.complete(request_id);
    }
    ...
    async fn load_tauritavern_settings(&self) -> Result<RustTavernSettings, ApplicationError> {
