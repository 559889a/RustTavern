...
use super::shared::{
    message_content_to_text, message_name, prefix_prompt_example_name, resolve_prompt_name_label,
};
...
fn build_google_payload(
    ...
    let (contents, system_prompt) = convert_messages(
        ...
        use_system_prompt,
        use_vertex_ai,
        &PromptNames::from_payload(&payload),
    )?;
...
fn convert_messages(
    ...
    if use_system_prompt && entries.len() > 1 {
        while start_index < entries.len().saturating_sub(1) {
            ...
            // Upstream substitutes the example markers here before the text enters
            // `system_instruction`; a plain `name` on a leading system message is not folded in.
            let content_text =
                prefix_prompt_example_name(&content_text, message_name(message), names);
    ...
    for entry in entries.iter().skip(start_index) {
        ...
        let mut parts = if matches!(role.as_str(), "tool" | "function") {
            ...
            // Gemini has no `name` field, so upstream folds the label into every text part. Skipping
            // this made "Character Names Behavior: Message Completion" a no-op on Gemini/Vertex:
            // the model received an unlabeled group transcript. Provider-native parts are left
            // untouched so round-tripped content (and its thought signatures) stays byte-faithful.
            if native_gemini_parts.is_none() {
                apply_google_name_label(&mut parts, message, names);
            }
...
/// Fold a message's `name` into each of its text parts, the way Gemini requires.
fn apply_google_name_label(
    parts: &mut [Value],
    message: &Map<String, Value>,
    names: &PromptNames,
) {
    let Some(label) = resolve_prompt_name_label(message_name(message), names) else {
        return;
    };
...
mod tests {
    ...
    #[test]
    fn makersuite_folds_message_names_into_text_parts() {
        let payload = json!({
            "model": "gemini-2.5-pro",
            "use_sysprompt": true,
            "char_name": "Seraphina",
            ...
            "messages": [
                ...
                { "role": "system", "name": "example_user", "content": "Hi there." },
                { "role": "system", "name": "example_assistant", "content": "Greetings." },
                { "role": "user", "name": "Bob", "content": "Where are we?" },
                { "role": "assistant", "name": "Seraphina", "content": "Seraphina: In the glade." }
            ]
        })
        .as_object()
        .cloned()
        ...
        let upstream = build(payload).expect("build should succeed").1;

        // Leading system run keeps the example markers substituted, not the marker text.
        let system_text = upstream
            .pointer("/systemInstruction/parts/0/text")
            .and_then(Value::as_str)
            .unwrap_or_default();
        assert!(system_text.contains("Persona."), "got {system_text:?}");
        assert!(system_text.contains("Alex: Hi there."), "got {system_text:?}");
        assert!(
            system_text.contains("Seraphina: Greetings."),
            "got {system_text:?}"
        );

        // Group / sendas labels must reach the model, and an already-labelled text is not doubled.
        assert_eq!(
            upstream
                .pointer("/contents/0/parts/0/text")
                .and_then(Value::as_str),
            Some("Bob: Where are we?")
        );
        assert_eq!(
            upstream
                .pointer("/contents/1/parts/0/text")
                .and_then(Value::as_str),
            Some("Seraphina: In the glade.")
        );
        assert!(
            !upstream.to_string().contains("example_user"),
            "internal role markers must not reach the provider"
        );
    }
}
