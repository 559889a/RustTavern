...
pub(super) fn build(mut payload: Map<String, Value>) -> Result<(String, Value), ApplicationError> {
    ...
    // DeepSeek has no structured-output mode: upstream asks for a JSON object and appends the
    // schema as a user message. Leaving `json_schema` in place would make `openai::build` emit the
    // OpenAI-only `response_format: {type: "json_schema"}` shape, which DeepSeek rejects.
        ...
        && let Some(body) = upstream_payload.as_object_mut()
    {
        ...
        if let Some(schema) = json_schema {
            apply_json_schema_as_user_message(body, &schema)?;
        }
    }
...
/// Mirror upstream's JSON-schema handling: request a JSON object and hand the schema to the model as
/// a trailing user message.
fn apply_json_schema_as_user_message(
    body: &mut Map<String, Value>,
    schema: &Value,
) -> Result<(), ApplicationError> {
    let rendered = serde_json::to_string_pretty(schema).map_err(|error| {
        ApplicationError::ValidationError(format!(
            "DeepSeek could not render the response JSON schema: {error}"
        ))
    })?;
    ...
    let Some(messages) = messages.as_array_mut() else {
        return Err(ApplicationError::ValidationError(
            "DeepSeek request messages must be an array".to_string(),
        ));
    };
    messages.push(serde_json::json!({
        "role": "user",
        "content": format!("JSON schema for the response:\n{rendered}"),
    }));
...
mod tests {
    ...
    #[test]
    fn deepseek_json_schema_becomes_json_object_plus_schema_message() {
        let payload = json!({
            "model": "deepseek-chat",
            "messages": [{"role": "user", "content": "hello"}],
            "chat_completion_source": "deepseek",
        ...
        // DeepSeek has no structured-output mode, so the OpenAI-only json_schema shape must not leak.
        assert_eq!(
        ...
        let last = messages.last().expect("schema message must be appended");
        assert_eq!(last.get("role").and_then(Value::as_str), Some("user"));
        assert!(
            last.get("content")
                .and_then(Value::as_str)
                .unwrap_or_default()
                .starts_with("JSON schema for the response:"),
            "got {last:?}"
        );
    }
}
