...
use super::shared::{insert_if_present, message_content_to_text, move_if_present};
...
fn build_text_completion_payload(
    ...
    if let Some(prompt) = payload.remove("prompt").filter(|value| !value.is_null()) {
...
fn build_chat_completion_payload(
    ...
    // `messages` is the bulk of the request (it carries every message plus any inline base64
    // media), so move it instead of cloning a second copy of the whole prompt per send.
    move_if_present(&mut request, &mut payload, "messages");
    ...
    if payload.get("tools").is_some_and(Value::is_array) {
        move_if_present(&mut request, &mut payload, "tools");
        insert_if_present(&mut request, &payload, "tool_choice");
    }

    map_chat_logprobs(&mut request, &payload);

    if let Some(response_format) = resolve_response_format(&mut payload) {
...
fn resolve_response_format(payload: &mut Map<String, Value>) -> Option<Value> {
    ...
    build_response_format_from_json_schema(payload)
}

fn build_response_format_from_json_schema(payload: &mut Map<String, Value>) -> Option<Value> {
    let json_schema = payload.remove("json_schema")?;
    let mut json_schema = match json_schema {
        Value::Object(object) => object,
        _ => return None,
    };
    let schema_value = json_schema.remove("value")?;
    ...
    json_schema_object.insert(
        "strict".to_string(),
        json_schema.remove("strict").unwrap_or(Value::Bool(true)),
    );
