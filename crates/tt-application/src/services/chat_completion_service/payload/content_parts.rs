...
mod tests {
    ...
    fn rejects_invalid_data_url() {
        ...
        // Not base64: the payload is percent-encoded text, so forwarding it as base64 would
        // corrupt the attachment.
        let error = parse_openai_chat_content(Some(&json!([{
            "type": "audio_url",
            "audio_url": { "url": "data:audio/wav,hello" }
        }])))
        .expect_err("non-base64 data URL should fail");
        assert!(error.to_string().contains("invalid data URL"));
    }

    #[test]
    fn accepts_data_url_with_extra_parameters() {
        // Browsers and third-party extensions emit parameters before the encoding marker; the
        // attachment must not be refused because of them.
        let parts = parse_openai_chat_content(Some(&json!([{
            "type": "image_url",
            "image_url": { "url": "data:image/png;charset=utf-8;base64,AAAA" }
        }])))
        .expect("data URL with parameters should parse");
