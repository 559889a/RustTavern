...
use super::shared::{
    message_content_to_text, message_name, prefix_prompt_example_name, prefix_prompt_name,
...
fn build_gemini_interactions_payload(
    ...
    let (input, system_instruction) = build_input_and_system_instruction(
    ...
    for (index, entry) in entries.iter().enumerate() {
        ...
        match role.as_str() {
            "system" | "developer" => {
                ...
                // The example role markers must reach the model as the persona / character name.
                let text = prefix_prompt_example_name(&text, message_name(message), names);
            ...
            "assistant" => {
                ...
                let text = message_content_to_text(message.get("content"));
                let text = prefix_prompt_name(&text, message_name(message), names);
                if !text.trim().is_empty() {
                    steps.push(json!({
                        "type": "model_output",
            ...
            "user" => {
                ...
                // Gemini has no `name` field: group / sendas labels only reach the model if they are
                // folded into the text.
                let content = apply_name_label_to_blocks(content, message_name(message), names);
                steps.push(json!({
                    "type": "user_input",
                    "content": content,
                }));
...
/// Fold a message `name` into each text block of a converted content array.
fn apply_name_label_to_blocks(
    mut blocks: Vec<Value>,
    name: Option<&str>,
    names: &PromptNames,
) -> Vec<Value> {
    let Some(label) = resolve_prompt_name_label(name, names) else {
        return blocks;
    };
