    ...
    for message in normalized {
        ...
        if can_merge {
            ...
            if let Some(last_object) = merged.last_mut().and_then(Value::as_object_mut) {
                // Append in place. Building a fresh `format!("{previous}\n\n{current}")` here made
                // merging quadratic in total prompt bytes, which is what `single` mode does to
                // every message in a long chat.
                match last_object.get_mut("content") {
                    Some(Value::String(previous_content)) => {
                        previous_content.reserve(current_content.len() + 2);
                        previous_content.push_str("\n\n");
                        previous_content.push_str(&current_content);
                    }
                    _ => {
                        last_object.insert(
                            "content".to_string(),
                ...
                match part {
                    ...
                    Value::Object(object) => match object.get("type").and_then(Value::as_str) {
                        ...
                        Some("image_url") | Some("video_url") | Some("audio_url") => {
                            let token = format!("__RUSTTAVERN_MEDIA_TOKEN_{}__", Uuid::new_v4());
