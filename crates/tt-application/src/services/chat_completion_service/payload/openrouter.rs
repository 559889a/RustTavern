...
use super::openai;
use super::shared::{insert_if_present, provider_option_source};

pub(super) fn build(payload: Map<String, Value>) -> Result<(String, Value), ApplicationError> {
    let source_payload = provider_option_source(&payload);
...
fn apply_openrouter_overrides(
    ...
    // `openai::build` only forwards `verbosity` for OpenAI/custom GPT-5 models, but OpenRouter
    // routes it itself, so upstream forwards it unconditionally here. Without this the Verbosity
    // control silently has no effect on OpenRouter.
    if let Some(verbosity) = source_payload
        .get("verbosity")
...
mod tests {
    ...
    fn openrouter_payload_includes_provider_specific_fields() {
        ...
        // OpenRouter routes verbosity itself, so it is forwarded regardless of model family.
