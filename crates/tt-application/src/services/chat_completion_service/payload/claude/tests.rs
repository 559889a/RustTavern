...
#[test]
fn claude_substitutes_example_role_markers_with_real_names() {
    let mut payload = claude_payload("claude-sonnet-4-5");
    payload.insert("char_name".to_string(), json!("Seraphina"));
    payload.insert("user_name".to_string(), json!("Alex"));
    payload.insert("use_sysprompt".to_string(), json!(true));
    payload.insert(
        "messages".to_string(),
        json!([
    ...
    // Leading system run: example markers get the persona / character label, never the literal
    // marker text.
    assert_eq!(
        upstream.pointer("/system/0/text").and_then(Value::as_str),
        Some("Persona.")
    );
    assert_eq!(
        upstream.pointer("/system/1/text").and_then(Value::as_str),
        Some("Alex: Hi there.")
    );
    assert_eq!(
        upstream.pointer("/system/2/text").and_then(Value::as_str),
        Some("Seraphina: Greetings.")
    );

    // A plain `name` on a regular message is still folded in verbatim.
    let text = upstream
        .pointer("/messages/0/content")
        .and_then(Value::as_array)
    ...
    assert!(
        ...
        "expected the message name to be folded in, got {text:?}"
    );
    assert!(
...
#[test]
fn claude_substitutes_example_markers_after_the_system_run() {
    let mut payload = claude_payload("claude-sonnet-4-5");
    payload.insert("char_name".to_string(), json!("Seraphina"));
    payload.insert("user_name".to_string(), json!("Alex"));
    payload.insert("use_sysprompt".to_string(), json!(false));
    payload.insert(
        "messages".to_string(),
        json!([
            { "role": "system", "name": "example_user", "content": "Hi there." },
    ...
    // Both become `user` turns and are merged block by block.
    assert_eq!(
        upstream
