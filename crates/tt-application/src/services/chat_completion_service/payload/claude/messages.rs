...
use super::super::prompt_post_processing::PromptNames;
use super::super::shared::{
    PromptNameLabel, message_content_to_text, message_name, prefix_prompt_example_name,
    resolve_prompt_name_label,
};
...
pub(super) fn convert_messages(
    ...
    if use_system_prompt {
        while start_index < entries.len() {
            ...
            // Upstream only substitutes the example markers here; a plain `name` on a leading
            // system message is not folded into the system prompt.
            let content_text =
    ...
    for (index, entry) in entries.iter().enumerate().skip(start_index) {
        ...
        let raw_name = message_name(message);
        // Upstream substitutes the example markers on `system` messages (then drops `name` so the
        // generic path cannot re-add it), and folds a plain `name` into every non-system message.
        let name = if role == "system" {
            resolve_prompt_name_label(
                raw_name.filter(|value| matches!(*value, "example_user" | "example_assistant")),
                names,
            )
        } else {
            resolve_prompt_name_label(raw_name, names)
        };
        ...
        match role.as_str() {
            "assistant" => {
                    ...
                    message_native_claude_content(message)?
                {
                    ...
                    let mut blocks =
                        convert_message_content_to_claude_blocks(message.get("content"), name, names)?;
                    if use_tools {
...
fn prefix_name(text: &str, name: Option<PromptNameLabel<'_>>, names: &PromptNames) -> String {
    match name {
...
fn convert_message_content_to_claude_blocks(
    ...
    for part in &parts {
        if let Some(block) = render_claude_content_part(part, name, names)? {
            blocks.push(block);
        }
    }
...
fn render_claude_content_part(
    ...
    match part {
        InputPart::Text(text) => Ok(Some(normalize_claude_text_block(&prefix_name(
            text, name, names,
        )))),
