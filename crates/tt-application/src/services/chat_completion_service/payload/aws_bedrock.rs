...
fn unsupported_model_message(spec: &BedrockModelSpec) -> String {
    ...
    format!(
        "AWS Bedrock model `{}` is not supported by RustTavern's built-in Bedrock adapter. {reason} Enable the custom template (`aws_bedrock_use_custom_template`) for this model, or choose one of the supported families: Anthropic Claude, Amazon Nova, Meta Llama, Mistral, DeepSeek, Cohere Command R/R+, AI21 Jamba.",
        spec.raw_id()
    )
}
