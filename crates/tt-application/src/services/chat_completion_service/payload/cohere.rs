...
pub(super) fn build(payload: Map<String, Value>) -> Result<(String, Value), ApplicationError> {
    ...
    // Move the message array out: it is the bulk of the payload and the converter takes ownership.
    let messages = convert_messages(payload.remove("messages"), &names)?;
    ...
    {
        let mut tools = std::mem::take(tools);
