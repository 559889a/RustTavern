...
/// Max time a run waits for the frontend host to resolve a chat commit or a
/// prompt assembly request. A host that disconnects or stops replying would
/// otherwise leave the run (and its pending entry, background tasks, and
/// model WebSocket session) hanging forever.
pub(super) const HOST_WAIT_TIMEOUT: Duration = Duration::from_secs(60);
