...
use super::{HostResourceDeliveryCapabilities, HostResourcePayload, HostResourceResponse};
...
pub(super) fn not_modified(metadata: &RepresentationMetadata) -> HostResourceResponse {
    representation_response(StatusCode::NOT_MODIFIED, metadata, Vec::new().into(), None, false)
}

pub(super) fn head(metadata: &RepresentationMetadata) -> HostResourceResponse {
...
pub(super) fn ok(metadata: &RepresentationMetadata, body: Vec<u8>) -> HostResourceResponse {
    let content_length = body.len() as u64;
    representation_response(StatusCode::OK, metadata, body.into(), Some(content_length), true)
}

/// Whole-body response whose payload is streamed rather than buffered.
///
/// `content_length` comes from the source metadata (the bytes have not been read
/// yet), so the header stays correct without materialising the body.
pub(super) fn ok_streamed(
    metadata: &RepresentationMetadata,
    reader: tt_ports::host_resource::HostResourceReader,
    content_length: u64,
) -> HostResourceResponse {
    representation_response(
        StatusCode::OK,
        metadata,
        HostResourcePayload::from_reader(reader),
        Some(content_length),
        true,
    )
}
...
pub(super) fn no_content(allowed_methods: &'static str) -> HostResourceResponse {
...
pub(super) fn method_not_allowed(allowed_methods: &'static str) -> HostResourceResponse {
...
pub(super) fn error(status: StatusCode, message: &str) -> HostResourceResponse {
...
fn base_response(status: StatusCode, body: Vec<u8>) -> HostResourceResponse {
...
pub(super) fn with_accept_ranges(mut response: HostResourceResponse) -> HostResourceResponse {
    response
        .headers_mut()
...
pub(super) fn range_not_satisfiable(message: &str, total_size: u64) -> HostResourceResponse {
