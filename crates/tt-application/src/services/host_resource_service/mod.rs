...
use tt_ports::host_resource::{HostResourceAssetStore, HostResourceReader};

static NEXT_TRACE_SEQUENCE: AtomicU64 = AtomicU64::new(0);
const RUSTTAVERN_TRACE_ID: HeaderName = HeaderName::from_static("x-rusttavern-trace-id");

/// Payload of a host resource response.
///
/// Small payloads (thumbnails, user CSS, error text, every ranged read) stay
/// buffered. A whole-file payload carries a reader so the HTTP layer streams it:
/// buffering a several-hundred-MB background video or attachment would cost the
/// same in RSS, which is fatal on the 1-2GB devices this server targets.
pub enum HostResourcePayload {
    Bytes(Vec<u8>),
    Reader(HostResourceReader),
}

impl HostResourcePayload {
    pub fn from_reader(reader: HostResourceReader) -> Self {
        Self::Reader(reader)
    }

    /// Drop the body, keeping only the headers (`HEAD` requests).
    pub fn clear(&mut self) {
        *self = Self::Bytes(Vec::new());
    }

    /// The buffered bytes.
    ///
    /// Panics on a streaming payload: only paths that materialise the body should
    /// call this, so reaching it with a reader is a bug rather than a case to
    /// handle — silently yielding nothing would ship an empty response.
    pub fn bytes(&self) -> &[u8] {
        match self {
            Self::Bytes(bytes) => bytes,
            Self::Reader(_) => panic!("host resource payload is streamed, not buffered"),
        }
    }

    /// The buffered bytes, consuming the payload. Panics on a streaming payload,
    /// for the same reason as [`Self::bytes`].
    ...
    /// Whether this payload carries no buffered bytes. A streamed payload is not
    /// empty: its bytes exist, they are simply not held here.
    pub fn is_empty(&self) -> bool {
        matches!(self, Self::Bytes(bytes) if bytes.is_empty())
    }
}

impl From<Vec<u8>> for HostResourcePayload {
    fn from(bytes: Vec<u8>) -> Self {
        Self::Bytes(bytes)
    }
}

impl std::fmt::Debug for HostResourcePayload {
...
impl<const N: usize> PartialEq<&[u8; N]> for HostResourcePayload {
    fn eq(&self, other: &&[u8; N]) -> bool {
        matches!(self, Self::Bytes(bytes) if bytes.as_slice() == other.as_slice())
    }
}

pub type HostResourceResponse = Response<HostResourcePayload>;
...
mod tests {
    ...
    fn facade_dispatches_known_routes_and_ignores_frontend_assets() {
        ...
        assert!(
            user_css_response.headers()[&RUSTTAVERN_TRACE_ID]
                .to_str()
                .ok()
        ...
        assert_ne!(
            user_css_response.headers()[&RUSTTAVERN_TRACE_ID],
            third_party_response.headers()[&RUSTTAVERN_TRACE_ID]
        );
        ...
        assert!(fallback.headers().contains_key(&RUSTTAVERN_TRACE_ID));
    }
