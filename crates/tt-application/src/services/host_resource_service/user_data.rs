...
fn read_full_response(
    ...
    // Stream whole files. Buffering is what makes an un-ranged request for a large
    // background video or user file cost its own size in RSS, which is fatal on the
    // 1-2GB devices this server targets. Sources without a reader (thumbnails,
    // derived variants) keep the buffered path.
    if let Some(reader) = opened.reader() {
        return response::with_accept_ranges(response::ok_streamed(
            metadata,
            reader,
