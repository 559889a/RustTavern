...
impl From<DomainError> for ApplicationError {
    fn from(error: DomainError) -> Self {
        match error {
            ...
            DomainError::WorkspaceWriteConflict { path, kind } => {
                // Optimistic-concurrency conflict: report it as a conflict (so
                // the command layer answers 409, not 400) and keep `path` so the
                // caller knows which file lost the race and must be re-read.
                ApplicationError::Conflict(format!("Workspace write conflict at {path}: {kind}"))
            }
        }
    }
}
