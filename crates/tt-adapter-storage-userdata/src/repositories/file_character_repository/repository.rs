...
impl FileCharacterRepository {
    ...
    /// Per-card write lock (see `path_write_locks`): prune the Weak table
    /// when it grows past the retention bound, then hand out the live lock.
    async fn character_file_lock(&self, path: &Path) -> Arc<tokio::sync::Mutex<()>> {
        const MAX_RETAINED_LOCK_ENTRIES: usize = 1024;
...
impl CharacterRepository for FileCharacterRepository {
    async fn save(&self, character: &Character) -> Result<(), DomainError> {
        ...
        // The card is a read-merge-write over the PNG: without a per-file
        // lock, two concurrent saves of the same card both read the old
        // bytes and the later write silently drops the earlier merge.
        let _write_guard = self
            .character_file_lock(&file_path)
            .await
            .lock_owned()
            .await;
