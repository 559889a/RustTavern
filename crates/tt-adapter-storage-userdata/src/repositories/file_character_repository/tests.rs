...
fn unique_temp_root() -> PathBuf {
    std::env::temp_dir().join(format!("rusttavern-character-import-{}", random::<u64>()))
}
...
async fn create_with_avatar_png_without_crop_preserves_png_metadata_fast_path() {
    ...
    fs::write(
        &avatar_path,
        insert_text_chunk_before_iend(build_distinct_png(), "rusttavern-fast-path", "preserve me"),
    )
    .await
    ...
    assert!(
        text_chunks.iter().any(|chunk| {
            chunk.keyword == "rusttavern-fast-path" && chunk.text == "preserve me"
        })
    );
