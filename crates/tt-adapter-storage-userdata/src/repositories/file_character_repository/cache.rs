...
pub(crate) struct CharacterShallowIndexCache {
    ...
    /// When the on-disk signature behind this cache was last confirmed. The
    /// confirmation costs a full walk of every card and chat file, so a cache
    /// this fresh is served as-is; see `load_shallow_character_index`.
