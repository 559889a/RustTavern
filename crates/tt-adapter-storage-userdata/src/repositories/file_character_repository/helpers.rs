...
use tt_domain::models::chat::{normalize_chat_file_stem, parse_message_timestamp};
...
impl FileCharacterRepository {
    ...
    pub(crate) async fn read_character_from_file(
        ...
        // A card stored without a usable chat pointer must not be handed a
        // freshly generated, time-dependent name: that name differs on every
        // load, so it stops matching the chat file the previous session wrote
        // and the client is left pointing at nothing. Report the most recent
        // chat that exists instead, and let the normalizer synthesize a name
        // only when the character has no chat at all.
        if normalize_chat_file_stem(&character.chat).is_none()
            && let Some(latest_chat) = self
                .chat_repository
                .latest_character_chat_stem(&file_name)
                .await?
        {
            character.chat = latest_chat;
        }
