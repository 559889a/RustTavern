...
impl FileCharacterRepository {
    ...
    pub(crate) async fn load_shallow_character_index(&self) -> Result<Vec<Character>, DomainError> {
        ...
        // Confirming the signature means walking every card and every chat file's
        // metadata, and callers ask for this list several times in a row (boot,
        // character switch, each character route). A cache confirmed a moment ago
        // is served as-is: this repository's own edits go through
        // clear_shallow_index_cache, so only a change made outside the app can be
        // seen late, and only for this window.
        const REVALIDATE_AFTER: Duration = Duration::from_secs(3);
        if let Some(cache) = self.shallow_index_cache.lock().await.as_ref()
            && cache.validated_at.elapsed() < REVALIDATE_AFTER
        {
        ...
        {
            self.mark_shallow_index_validated().await;
            return Ok(Self::shallow_index_characters(cache));
        }
    ...
    /// Record that the cached index was just confirmed against the filesystem.
    async fn mark_shallow_index_validated(&self) {
        if let Some(cache) = self.shallow_index_cache.lock().await.as_mut() {
            cache.validated_at = Instant::now();
        }
    }
