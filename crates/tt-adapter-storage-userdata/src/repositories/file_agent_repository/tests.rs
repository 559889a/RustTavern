...
fn temp_root() -> PathBuf {
    std::env::temp_dir().join(format!("rusttavern-agent-repo-{}", Uuid::new_v4()))
}
...
#[tokio::test]
async fn event_seq_comes_from_the_journal_tail_and_survives_large_lines() {
    let root = temp_root();
    let repository = FileAgentRepository::new(root.clone());
    let run = sample_run();
    let manifest = sample_manifest(&run);
    ...
    // A single event larger than the tail probe window must still yield the right next seq: the
    // append path only reads the file's tail instead of parsing the whole journal.
    let big_payload = Value::String("x".repeat(64 * 1024));
    ...
    // `after_seq` at or beyond the high-water mark answers "nothing new" without a full parse.
    let none = repository
        .read_events(
            &run.id,
            AgentRunEventReadQuery {
