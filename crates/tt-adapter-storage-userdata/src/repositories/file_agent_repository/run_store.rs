...
use super::event_journal::last_event_seq_in_file;
...
impl AgentRunRepository for FileAgentRepository {
    ...
    async fn append_event(
        ...
        let seq = last_event_seq_in_file(&events_path)
            .await?
            .map(|last| last + 1)
            .unwrap_or(1);
    ...
    async fn read_events(
        ...
        // The timeline UI polls with `after_seq` set to the highest seq it already holds (and with
        // an out-of-range value when it only wants the high-water mark), so answer "nothing new"
        // from the journal tail instead of parsing the whole file on every structural event.
        if query.before_seq.is_none()
            && let Some(after_seq) = query.after_seq
            && self
                .last_event_seq(run_id)
                .await?
