...
pub(super) struct RustTavernSidecar {
    ...
    #[serde(default)]
    pub(super) recommended_context: Option<RustTavernRecommendedContext>,
    #[serde(default)]
    pub(super) tags: Vec<String>,
    #[serde(default)]
...
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub(super) struct RustTavernRecommendedContext {
    #[serde(default)]
...
pub(super) fn read_sidecar(root: &Path) -> Result<Option<RustTavernSidecar>, DomainError> {
    ...
    if metadata.file_type().is_symlink() {
        return Err(DomainError::InvalidData(
            "agents/rusttavern.json cannot be a symlink".to_string(),
        ));
    }
    ...
    let sidecar: RustTavernSidecar = serde_json::from_str(&text).map_err(|error| {
        DomainError::InvalidData(format!("Invalid agents/rusttavern.json: {error}"))
    })?;
    if sidecar.version != SIDECAR_VERSION {
        return Err(DomainError::InvalidData(format!(
            "Unsupported agents/rusttavern.json version {}",
            sidecar.version
        )));
    }
