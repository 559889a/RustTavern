...
fn default_ios_policy_seed() -> Option<Value> {
    ...
    let profile = env!("RUSTTAVERN_IOS_POLICY_PROFILE").trim();
    if profile.is_empty() {
        return None;
    }
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RustTavernSettings {
    pub updates: RustTavernUpdateSettings,
...
impl Default for RustTavernSettings {
    fn default() -> Self {
        Self {
            updates: RustTavernUpdateSettings::default(),
...
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct RustTavernUpdateSettings {
...
mod tests {
    ...
    fn avatar_persona_original_images_enabled_migrates_legacy_thumbnail_setting() {
        let settings = RustTavernSettings::from_json_str_with_compat(
    ...
    fn chat_virtualization_defaults_to_disabled_and_accepts_enabled() {
        let older = RustTavernSettings::from_json_str_with_compat(
            r#"{"updates":{"startup_popup":{"dismissed_release_token":null}}}"#,
        )
        .expect("parse older settings");
        assert!(!older.chat_virtualization_enabled);

        let enabled = RustTavernSettings::from_json_str_with_compat(
    ...
    fn agent_retention_defaults_to_recent_terminal_history_policy() {
        let settings = RustTavernSettings::default();
    ...
    fn chat_backup_settings_accept_documented_sentinels_and_reject_lower_values() {
        let mut settings = RustTavernSettings::default().chat_backups;
