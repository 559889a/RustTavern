...
pub struct ChatMetadata {
    ...
    /// Chat-local variables (`/setvar`, `{{getvar::}}`, extension state).
    ///
    /// The frontend stores its own values as strings, but this map is writable by
    /// any extension and real chats contain object values. Typing it as `String`
    /// made the whole chat unreadable ("invalid type: map, expected a string"),
    /// so keep the value opaque and round-trip it unchanged — no Rust code
    /// interprets it.
    #[serde(default)]
