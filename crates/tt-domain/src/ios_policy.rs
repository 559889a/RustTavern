...
fn ios_build_default_profile() -> Option<IosPolicyProfile> {
    ...
    match env!("RUSTTAVERN_IOS_POLICY_PROFILE").trim() {
        "" => None,
        ...
        value => panic!(
            "RUSTTAVERN_IOS_POLICY_PROFILE embedded value {value:?} is unsupported. Expected one of: full, ios_internal_full, ios_external_beta."
        ),
    }
}
